...
void QPatternConverter::formatAndAppend(QExclusiveTextStream& output, const QLoggingEvent& event)
{
	...
	while(nPos < m_strPattern.length())
	{
		...
		case LITERAL_STATE :
			...
			if(c == ESCAPE_CHAR)
			{
				...
				switch (m_strPattern[nPos].toLatin1())
				{
				case ESCAPE_CHAR:
					m_strCurrentLiteral += c;
					nPos++; // move pointer
		...
		case DOT_STATE:
			...
			{
				loglog()->error("Error occurred in position %d.\n Was expecting digit, instead got char \"%c\".", nPos, c.toLatin1());
				m_state = LITERAL_STATE;
			}
			break;
		case MAX_STATE:
...
not_implemented:;
	default:
		loglog()->error("Unexpected char [%c] at position %d in conversion patterrn.", c.toLatin1(), m_nPos);
...
void QPatternLogLayout::init(const QString& pattern, quint32 ndcMaxDepth)
{
	...
	for(int i=0; i<m_patterns.size(); i++)
	{
		if( m_patterns[i] == 0 )
		{
			...
			QPatternConverter* pPattern=m_patterns[i];
			m_patterns[i] = new QLiteralPatternConverter("");

			SAFE_DELETE_CLASS(pPattern);
		}
	}

	if(m_patterns.isEmpty())
	{
		loglog()->warning("PatternLayout pattern is empty.  Using default...");
...
void QPatternLogLayout::formatAndAppend(QExclusiveTextStream& output, const QLoggingEvent& event)
{
	for(int i=0; i<m_patterns.size(); i++)
	{
...
Q_EXPORT_REGISTER_LOG_LAYOUT_FACTORY(QPatternLogLayout, QPatternLogLayoutFactory)
