...
QMutex QLogLevelManager::m_mutex;
QLogLevelManagerPtr QLogLevelManager::m_pInstance;
...
QLogLevelManagerPtr QLogLevelManager::instance()
{
	if (m_pInstance == 0)
	{
		m_mutex.lock();
		if (m_pInstance == 0)
		{
			m_pInstance=QLogLevelManagerPtr(new QLogLevelManager);
		}
		m_mutex.unlock();
	}
