...
com.bitc.xmis.salaryExcelImportWin = Ext.extend(Ext.Window, {
	/**
	...
	initComponent : function() {
		...
		var infoPanel = this.infoPanel = new Ext.Panel({
			...
			html : String
					.format(
							'说明：请<a href="{0}/bbzFinancyMain.do?method=downloadTemplateFileBankDesposit" class="red-td-data">'
									+ '点击这里下载模板文件</a>'
									+ '（不要使用迅雷等下载工具下载），'
									+ '保存为<span class="red-td-data">Excel97-2003格式</span>的xls文件，'
									+ '<span class="red-td-data">关闭Excel后</span>将文件上传。',
							__cxtPath)+"</br>上传的工资文件中必须含有“月份”列。"
		});
			...
			items : [{
						...
						items : {
							...
							handler : function() {
								...
								Ext.Ajax.request({
									url : __cxtPath
											+ '/bbzFinancyMain.do?method=existValidate',
									scope : this,
									params : {
										type : 3,
										fileName : fileName,
																...
																if (buttonId == 'yes') {
																	...
																	Ext.Ajax
																			.request(
																					{
																						url : __cxtPath
																								+ '/bbzFinancyMain.do?method=delYearSheetInfoByAttID',
																						method : 'post',
																						params : {
																							attID: attID
																						},
		...
		com.bitc.xmis.salaryExcelImportWin.superclass.initComponent.call(this);
