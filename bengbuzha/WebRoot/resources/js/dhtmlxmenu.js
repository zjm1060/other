		...
		if ((this.idPull[FC] != null) && (this.idPull[id] != null)) {
			...
			if (this.limit > 0) {
				if (this.limit < itemCount) {
					this.idPull[FC].style.height = this.idPull[FC].childNodes[0].offsetHeight * this.limit + "px";
			...
			var srcX = (this.idPull[id].tagName != null ? ej(this.idPull[id]) : this.idPull[id][0]);
			var srcY = (this.idPull[id].tagName != null ? dg(this.idPull[id]) : this.idPull[id][1]);
			...
			if ((this.sxDacProc != null) && (this.idPull["sxDac_" + id] != null)) {
				...
				if (this._isIE6) {
					...
					this.idPull[pIdIE6].style.width = this.idPull[FC].offsetWidth + "px";
					this.idPull[pIdIE6].style.height = this.idPull[FC].offsetHeight + "px";
	...
	this._redistribSubLevelSelection = function (id, parentId) {
		...
		if (this.itemPull[id]["state"] == "enabled") {
			...
			if (this.itemPull[id]["complex"] && this.dLoad && (this.itemPull[id]["loaded"] == "no")) {
				...
				bA.loadXML(this.dLoadUrl + this.dLoadSign + "action=loadMenu&parentId=" + id.replace(this.idPrefix, "") + "&etc=" + new Date().getTime());
			}
		...
		if (main_self.dLoad) {
			...
			if (parentId == null) {
				...
				if (main_self.menuSelected == main_self.idPrefix + parentId) {
					...
					var gR = ((isTop && (!main_self.context)) ? main_self.dirTopLevel : main_self.dirSubLevel);
					var isShow = false;
					if (isTop && main_self.menuModeTopLevelTimeout && main_self.menuMode == "web" && !main_self.context) {
						...
						if (item._mouseOver == true) {
							var amY = main_self.menuModeTopLevelTimeoutTime - (new Date().getTime() - item._dynLoadTM);
							if (amY > 1) {
...
dhtmlXMenuObject.prototype._countVisiblePolygonItems = function (id) {
	...
	for (var a in this.itemPull) {
		...
		if (this.idPull[a] != null) {
			if (asv == id && (tp == "item" || tp == "radio" || tp == "checkbox") && this.idPull[a].style.display != "none") {
				count++;
			}
		}
	}
	...
	m.onmouseover = function () {
		...
		if (main_self.itemPull[this.id]["state"] == "enabled") {
			...
			if (main_self.dLoad && (main_self.itemPull[this.id]["loaded"] == "no")) {
				...
				bA.loadXML(main_self.dLoadUrl + main_self.dLoadSign + "action=loadMenu&parentId=" + this.id.replace(main_self.idPrefix, "") + "&etc=" + new Date().getTime());
			}
...
dhtmlXMenuObject.prototype._addItemIntoGlobalStrorage = function (itemId, itemParentId, itemText, itemType, disabled, img, imgDis) {
	var item = {id:itemId, title:itemText, imgen:(img != null ? img : ""), imgdis:(imgDis != null ? imgDis : ""), type:itemType, state:(disabled == true ? "disabled" : "enabled"), parent:itemParentId, complex:false, hotkey:"", tip:""};
	this.itemPull[item.id] = item;
};
...
dhtmlXMenuObject.prototype._renderSeparator = function (id, pos) {
	var gR = (this.context ? "SubLevelArea" : (this.itemPull[id]["parent"] == this.idPrefix + this.topId ? "TopLevel" : "SubLevelArea"));
...
(function () {
	dhtmlx.extend_api("dhtmlXMenuObject", {ahY:function (obj) {
		return [obj.parent, obj.skin];
	}, align:"setAlign", top_text:"setTopText", context:"renderAsContextMenu", icon_path:"setIconsPath", open_mode:"setOpenMode", rtl:"setRTL", skin:"setSkin", dynamic:"enableDynamicLoading", xml:"loadXML", items:"items", overflow:"setOverflowHeight"}, {items:function (arr, parent) {
		var pos = 100000;
		var lastItemId = null;
