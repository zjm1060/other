Ext.onReady(function() {
	...
	var invoiceDetailStore = new Ext.data.Store({
				proxy : new Ext.data.HttpProxy({
							url : __cxtPath
									+ '/contractMain.do?method=getInvoiceList'
						}),
				remoteSort : false,
				sortInfo : {
	...
	invoiceDetailStore.on('load', function(store, records, options) {
		...
		for (var i = 0; i < records.length; i++) {
			...
			invoiceAmt += formatCurrency("null", record
							.get(invoiceCurrencyExchangeRate), "")
					* formatCurrency("null", record.get(invoiceInvoiceAmt), "");
		}

		invoiceDetailGrid.setTitle("已开发票金额合计约:&nbsp;&nbsp;&nbsp;&nbsp;"
				+ formatCurrency("", invoiceAmt.toString(), "blue"));
	}, this, {
		delay : 100
	})
	...
	var invoiceDetailColM = new Ext.grid.ColumnModel([
			...
			{
				header : "开票金额",
				dataIndex : invoiceInvoiceAmt,
				sortable : true,
				renderer : function(value, cellMeta, record, rowIndex,
						columnIndex, store) {
					return formatCurrency(record.data[invoiceCurrencyGraph],
							value, 'blue');
				},
				align : 'right'
			},
