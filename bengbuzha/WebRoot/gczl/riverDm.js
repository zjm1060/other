Ext.onReady(function() {
	...
	var infoPanel = new Ext.form.FormPanel({
		...
		items : [{
			...
			items : [{
						...
						id : 'pillar_code',
						// name : 'mptp_name',
						hiddenName : 'pillar_code',
						value : 'cs01',
						valueField : 'pillar_code',
						displayField : 'pillar_name',
						mode : 'remote',
						triggerAction : 'all',
						editable : false,
						store : new Ext.data.Store({
							proxy : new Ext.data.HttpProxy({
										url : __cxtPath
												+ '/gczlAction.do?method=getRiverDmList'
									}),
							autoLoad : true,
							...
							listeners : {
								...
								'load' : function() {
									Ext.getCmp('pillar_code').setValue(Ext
											.getCmp('pillar_code').value);
								}
							}
						})
					}, {
			...
			items : [{
				...
				handler : function() {
					...
					infoPanel.getForm().submit({
						url : __cxtPath + '/gczlAction.do?method=doSaveRiverDmInfo',
						params : {
				...
				handler : function() {
					Ext.MessageBox.confirm('提示', '是否删除', function(btn, text) {
						if (btn == 'yes') {
							infoPanel.getForm().submit({
								url : __cxtPath
										+ '/gczlAction.do?method=doDeleteRiverDmInfoById',
								params : {
	...
	var cm = new Ext.grid.ColumnModel([new Ext.grid.RowNumberer({
				...
				header : '桩号ID',
				hidden:true,
				dataIndex : 'pillar_code',
				sortable : true
			},{
				header : '桩号',
				dataIndex : 'pillar_name',
				sortable : true
			}, {
				header : '距离',
				dataIndex : 'distance',
	...
	var storeGYS = new Ext.data.Store({
		proxy : new Ext.data.HttpProxy({
					url : __cxtPath
							+ '/gczlAction.do?method=getRiverDmInfoListWithGroupQuery',
					method : "POST"
				}),
		reader : new Ext.data.JsonReader({
					root : 'rows',
