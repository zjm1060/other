Ext.onReady(function() {
	...
	var infoPanel = new Ext.form.FormPanel({
		...
		items : [{
			...
			items : [{
						...
						id : 'sky_status',
						// name : 'mptp_name',
						hiddenName : 'sky_status',
						value : '0001',
						valueField : 'weather_code',
						displayField : 'weather_state',
						mode : 'remote',
						triggerAction : 'all',
						editable : false,
						store : new Ext.data.Store({
							proxy : new Ext.data.HttpProxy({
										url : __cxtPath
												+ '/gczlAction.do?method=getWeatherList'
									}),
							autoLoad : true,
							reader : new Ext.data.JsonReader({
										...
										fields : [{
													...
													name : 'weather_state',
													mapping : 'weather_state'
												}]
									}),
							listeners : {
								// 解决在Form中setValue时显示的是valueField而不是displayField的问题
								'load' : function() {
									Ext.getCmp('sky_status').setValue(Ext
											.getCmp('sky_status').value);
								}
							}
						})
					}, {
			...
			items : [{
				...
				handler : function() {
					...
					infoPanel.getForm().submit({
						url : __cxtPath + '/gczlAction.do?method=doSaveWeatherInfo',
						params : {
				...
				handler : function() {
					Ext.MessageBox.confirm('提示', '是否删除', function(btn, text) {
						if (btn == 'yes') {
							infoPanel.getForm().submit({
								url : __cxtPath
										+ '/gczlAction.do?method=doDeleteWeatherInfoById',
								params : {
	...
	var storeGYS = new Ext.data.Store({
		proxy : new Ext.data.HttpProxy({
					url : __cxtPath
							+ '/gczlAction.do?method=getWeatherInfoListWithGroupQuery',
					method : "POST"
				}),
		reader : new Ext.data.JsonReader({
					root : 'rows',
