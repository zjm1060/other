Ext.onReady(function() {
	...
	var infoPanel = new Ext.form.FormPanel({
		...
		items : [{
			...
			items : [{
						...
						fieldLabel : '测量部位',
						id : 'conc_seam_code',
						hiddenName : 'conc_seam_code',
						value : '01',
						valueField : 'conc_seam_code',
						displayField : 'conc_seam_name',
						mode : 'remote',
						triggerAction : 'all',
						editable : false,
						store : new Ext.data.Store({
							proxy : new Ext.data.HttpProxy({
										url : __cxtPath
												+ '/gczlAction.do?method=getConcSeamCodeList'
									}),
							autoLoad : true,
							reader : new Ext.data.JsonReader({
										...
										fields : [{
													name : 'conc_seam_code',
													mapping : 'conc_seam_code'
												}, {
													name : 'conc_seam_name',
													mapping : 'conc_seam_name'
												}]
									}),
							listeners : {
								// 解决在Form中setValue时显示的是valueField而不是displayField的问题
								'load' : function() {
									Ext.getCmp('conc_seam_code').setValue(Ext
											.getCmp('conc_seam_code').value);
								}
							}
						})
					}, {
						...
						fieldLabel : '测量点',
						id : 'conc_mp_code',
						hiddenName : 'conc_mp_code',
						value : '01',
						valueField : 'conc_mp_code',
						displayField : 'conc_mp_name',
						mode : 'remote',
						triggerAction : 'all',
						editable : false,
						store : new Ext.data.Store({
							proxy : new Ext.data.HttpProxy({
										url : __cxtPath
												+ '/gczlAction.do?method=getConcMpCodeList'
									}),
							baseParams:{conc_seam_code:'01'},
							autoLoad : true,
							reader : new Ext.data.JsonReader({
										root : 'rows',
										fields : [{
													name : 'conc_mp_code',
													mapping : 'conc_mp_code'
												}, {
													name : 'conc_mp_name',
													mapping : 'conc_mp_name'
												}]
									}),
							listeners : {
								// 解决在Form中setValue时显示的是valueField而不是displayField的问题
								'load' : function() {
									Ext.getCmp('conc_mp_code').setValue(Ext
											.getCmp('conc_mp_code').value);
								}
							}
						})
			...
			items : [{
				...
				handler : function() {
					...
					infoPanel.getForm().submit({
						url : __cxtPath + '/gczlAction.do?method=doSaveConcSeamInfo',
						params : {
				...
				handler : function() {
					Ext.MessageBox.confirm('提示', '是否删除', function(btn, text) {
						if (btn == 'yes') {
							infoPanel.getForm().submit({
								url : __cxtPath
										+ '/gczlAction.do?method=doDeleteConcSeamInfoById',
								params : {
	...
	var conc_seam_code = infoPanel.getForm().findField("conc_seam_code");
	var conc_mp_code = infoPanel.getForm().findField("conc_mp_code");
	conc_seam_code.on('select', function() {
		conc_mp_code.setValue('');
		conc_mp_code.store.setBaseParam('conc_seam_code',conc_seam_code.getValue());
		conc_mp_code.store.load();
	});
	var cm = new Ext.grid.ColumnModel([new Ext.grid.RowNumberer({
				...
				header : '测量部位ID',
				hidden:true,
				dataIndex : 'conc_seam_code',
				sortable : true
			},{
				header : '测量部位',
				dataIndex : 'conc_seam_name',
				sortable : true
			}, {
				header : '测量点ID',
				hidden:true,
				dataIndex : 'conc_mp_code',
				sortable : true
			},{
				header : '测量点',
				dataIndex : 'conc_mp_name',
				sortable : true
			},{
				header : '气温',
				dataIndex : 'weather',
	...
	var storeGYS = new Ext.data.Store({
		proxy : new Ext.data.HttpProxy({
					url : __cxtPath
							+ '/gczlAction.do?method=getConcSeamInfoListWithGroupQuery',
					method : "POST"
				}),
		reader : new Ext.data.JsonReader({
					root : 'rows',
