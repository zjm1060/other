Ext.onReady(function() {
	...
	var infoPanel = new Ext.form.FormPanel({
		...
		items : [{
			...
			items : [{
						...
						fieldLabel : '河流名称',
						id : 'river_code',
						hiddenName : 'river_code',
						value : '01',
						valueField : 'river_code',
						...
						store : new Ext.data.Store({
							proxy : new Ext.data.HttpProxy({
										url : __cxtPath
												+ '/gczlAction.do?method=getRiverCodeList'
									}),
							autoLoad : true,
							...
							listeners : {
								...
								'load' : function() {
									Ext.getCmp('river_code').setValue(Ext
											.getCmp('river_code').value);
						...
						store : new Ext.data.Store({
							proxy : new Ext.data.HttpProxy({
										url : __cxtPath
												+ '/gczlAction.do?method=getPlaceCodeList'
									}),
							autoLoad : true,
							reader : new Ext.data.JsonReader({
										root : 'rows',
			...
			items : [{
				...
				handler : function() {
					...
					infoPanel.getForm().submit({
						url : __cxtPath + '/gczlAction.do?method=doSaveOphContentInfo',
						params : {
				...
				handler : function() {
					Ext.MessageBox.confirm('提示', '是否删除', function(btn, text) {
						if (btn == 'yes') {
							infoPanel.getForm().submit({
								url : __cxtPath
										+ '/gczlAction.do?method=doDeleteOphContentInfoById',
								params : {
	...
	var storeGYS = new Ext.data.Store({
		proxy : new Ext.data.HttpProxy({
					url : __cxtPath
							+ '/gczlAction.do?method=getOphContentInfoListWithGroupQuery',
					method : "POST"
				}),
		reader : new Ext.data.JsonReader({
					root : 'rows',
