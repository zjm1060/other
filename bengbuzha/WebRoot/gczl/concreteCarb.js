Ext.onReady(function() {
			...
			items : [{
						...
						store : new Ext.data.Store({
							proxy : new Ext.data.HttpProxy({
										url : __cxtPath
												+ '/gczlAction.do?method=getConcreteCarbList'
									}),
							autoLoad : true,
			...
			items : [{
				...
				handler : function() {
					...
					infoPanel.getForm().submit({
						url : __cxtPath + '/gczlAction.do?method=doSaveConcreteCarbInfo',
						params : {
				...
				handler : function() {
					Ext.MessageBox.confirm('提示', '是否删除', function(btn, text) {
						if (btn == 'yes') {
							infoPanel.getForm().submit({
								url : __cxtPath
										+ '/gczlAction.do?method=doDeleteConcreteCarbInfoById',
								params : {
	...
	var cm = new Ext.grid.ColumnModel([new Ext.grid.RowNumberer({
				...
				header : '测量部位',
				dataIndex : 'concrete_place',
				sortable : true
			}, {
				header : '碳化深度',
				dataIndex : 'carbonize_depth',
				sortable : true
			},{
				header : '允许深度',
				dataIndex : 'allow_carb_depth',
				sortable : true
			},{
				header : '测量人ID',
				width : 80,
	...
	var storeGYS = new Ext.data.Store({
		proxy : new Ext.data.HttpProxy({
					url : __cxtPath
							+ '/gczlAction.do?method=getConcreteCarbInfoListWithGroupQuery',
					method : "POST"
				}),
		reader : new Ext.data.JsonReader({
					root : 'rows',
