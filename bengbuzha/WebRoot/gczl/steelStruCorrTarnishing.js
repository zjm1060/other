Ext.onReady(function() {
			...
			items : [{
						...
						store : new Ext.data.Store({
							proxy : new Ext.data.HttpProxy({
										url : __cxtPath
												+ '/gczlAction.do?method=getSteelSCTList'
									}),
							autoLoad : true,
							reader : new Ext.data.JsonReader({
										...
										fields : [{
													...
													name : 'steel_stru_place',
													mapping : 'steel_stru_place'
												}]
									}),
							listeners : {
			...
			items : [{
				...
				handler : function() {
					...
					infoPanel.getForm().submit({
						url : __cxtPath + '/gczlAction.do?method=doSaveSteelSCTInfo',
						params : {
				...
				handler : function() {
					Ext.MessageBox.confirm('提示', '是否删除', function(btn, text) {
						if (btn == 'yes') {
							infoPanel.getForm().submit({
								url : __cxtPath
										+ '/gczlAction.do?method=doDeleteSteelSCTInfoById',
								params : {
	...
	var cm = new Ext.grid.ColumnModel([new Ext.grid.RowNumberer({
				...
				header : '测量部位',
				dataIndex : 'steel_stru_place',
				sortable : true
			}, {
				header : '锈蚀深度',
				dataIndex : 'tarnish_depth',
				sortable : true
			},{
				header : '原深度',
				dataIndex : 'steel_stru_thick',
				sortable : true
			},{
				header : '测量人ID',
				width : 80,
	...
	var storeGYS = new Ext.data.Store({
		proxy : new Ext.data.HttpProxy({
					url : __cxtPath
							+ '/gczlAction.do?method=getSteelSCTInfoListWithGroupQuery',
					method : "POST"
				}),
		reader : new Ext.data.JsonReader({
					root : 'rows',
