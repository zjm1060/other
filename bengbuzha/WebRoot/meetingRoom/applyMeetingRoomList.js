Ext.onReady(function() {
	...
	var store = new Ext.data.Store({
		proxy : new Ext.data.HttpProxy({
					url : __cxtPath
							+ '/applyManageMaint.do?method=getAppMeetingRoomFormList',
					method : "POST"
				}),
		baseParams : {
			limit : 20
	...
	var cm = new Ext.grid.ColumnModel([new Ext.grid.RowNumberer(), {
				...
				header : "会议召集人",
				width : 90,
				dataIndex : "sponsor_surname",
				renderer : function(value, metaData, record) {
					return record.get("sponsor_surname")
				...
				items : [{
					...
					handler : function(grid, rowIndex, colIndex) {
						var form_id = grid.store.getAt(rowIndex).data.id;
						com.bitc.xmis.ApplyMeetRoomWinShow(grid, 'agree',
								form_id, com.bitc.xmis.Combos.STATUS.agree);
				...
				items : [{
					getClass : function(v, meta, rec) {
						if (((rec.data.sponsor_id == __emp.userID || rec.data.app_user_id == __emp.userID) && (rec.data.status_id == 1 || rec.data.status_id == 3))
					...
					handler : function(grid, rowIndex, colIndex) {
						...
						var form_id = grid.store.getAt(rowIndex).data.id;
						com.bitc.xmis.ApplyMeetRoomWinShow(grid, 'update',
								form_id, com.bitc.xmis.Combos.STATUS.update);
				...
				items : [{
					getClass : function(v, meta, rec) {
						if (((rec.data.sponsor_id == __emp.userID || rec.data.app_user_id == __emp.userID) && (rec.data.status_id == 1
					...
					handler : function(grid, rowIndex, colIndex) {
						var form_id = grid.store.getAt(rowIndex).data.id;
						com.bitc.xmis.ApplyMeetRoomWinShow(grid, 'cancel',
								form_id, com.bitc.xmis.Combos.STATUS.cancel);
	...
	var gridPanel = new Ext.grid.GridPanel({
				...
				tbar : {
					...
					items : [{
						...
						iconCls : 'save',
						handler : function() {
							com.bitc.xmis.ApplyMeetRoomWinShow(gridPanel,
									'create', null,
