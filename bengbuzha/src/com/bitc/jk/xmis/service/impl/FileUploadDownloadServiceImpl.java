...
public class FileUploadDownloadServiceImpl implements FileUploadDownloadService {
	...
	public boolean doSaveAttachment(AttachmentFile attFile,boolean isDocument) {
		logger.info("---> doSaveAttachment ? fileName = "
				+ attFile.getFileName() + "&REFID = " + attFile.getRefID());
		return fileUploadDownloadDAO.doSaveAttachment(attFile,isDocument);
	}
	...
	public AttachmentFile getAttachmentByID(int attachmentID) {
		return fileUploadDownloadDAO.getAttachmentByID(attachmentID);
	}
	...
	public void doUpdateAttachmentContentByID(int attachementID ) throws FileNotFoundException{
		...
		attFile.setId(attachementID);
		AttachmentFile attFileObj=fileUploadDownloadDAO.getAttachmentByID(attachementID);
		attFile.setFilePath(attFileObj.getFilePath());
		attFile.setFileName(attFileObj.getFileName());
		fileUploadDownloadDAO.doUpdateAttachmentContentByID(attFile);
