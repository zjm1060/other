...
public class WebUtil {
	...
	/*
	 * public static String generateSQLStr(String sqlStr, PageInfo pageInfo) {
	 * sqlStr = "SET NOCOUNT ON \n" + sqlStr; String orderByStr =
	 * (pageInfo.getSort() == null || "".equals(pageInfo .getSort())) ? "" :
	 * "\n order by " + pageInfo.getSort() + " " + pageInfo.getDir();
	 * 
	 * return (pageInfo.getStart() == 0 && pageInfo.getLimit() == 0) ? (sqlStr +
	 * SQLScript.getResultStrWithoutPaging + orderByStr) : (sqlStr +
	 * SQLScript.getResultStrWithPaging + orderByStr +
	...
	public static String getOrderStr(String fields, String direction) {
		...
		if (direction.indexOf(",") < 0) {
			...
			for (int i = 0; i < size; i++) {
				result.append(fieldsArray[i]).append(" ")
						.append(directionArray[i]).append(",");
			}
	...
	public static String buildStoreForJsonWithoutPageInfoWithDateFormat(
			List aList) {
		logger.info("---> buildStoreForJsonWithoutPageInfoWithDateFormat"
				+ aList.size());
		String totalStr = "{\"success\":\"true\",\"totalCount\":\"";
		StringBuilder store = new StringBuilder();
		if (aList != null && aList.size() > 0) {
	...
	public static String getOrderSqlByPageInfo(PageInfo pageInfo) {
		return (pageInfo.getSort() == null || "".equals(pageInfo.getSort())) ? ""
				: "\n order by " + pageInfo.getSort() + " " + pageInfo.getDir();
	}
