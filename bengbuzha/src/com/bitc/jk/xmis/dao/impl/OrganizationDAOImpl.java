...
public class OrganizationDAOImpl extends BaseDAOImpl implements OrganizationDAO {
	...
	// Logger.getLogger(OrganizationDAOImpl.class);
	...
	public EmployeeObj userValidete(final String loginName,
		...
		String sqlStr = "select e.人员ID 'user_id',RTRIM(e.姓氏)+RTRIM(e.名字) 'emp_name',\n"
				...
				// bbz mysql
				// + ",        CONVERT(e.入职日期,CHAR(12))'joinDate'\n"
				...
				+ "where e.登录名=? and  e.密码=? ";
		// + "where e.登录名=? and ( e.密码=? or UPPER(dbo.MD5(e.密码))=?)"; //xmis
		// bbz mysql
		// + "where e.登录名=? and ( e.密码=?)";
		...
		Object o = jt.execute(sqlStr, new PreparedStatementCallback() {
			public Object doInPreparedStatement(PreparedStatement ps)
				...
				// ps.setString(3, password); //bbz mysql 不需要
	...
	public void doSaveEmployeeInfo(final EmployeeObj emp,
		...
		jt.update(SQLScript.updateEmployeeInfo, new PreparedStatementSetter() {
			...
			public void setValues(PreparedStatement ps) throws SQLException {
				...
				ps.setInt(51, emp.getUserID());
				// 蚌埠闸
				// 在职类型
				if (emp.getType() == null || emp.getType().getId() == 0) {
					ps.setNull(32, Types.INTEGER);
				} else {
					ps.setInt(32, emp.getType().getId());
				}
				if ("".equals(emp.getNation()))
					ps.setNull(33, Types.VARCHAR);
				else
					ps.setString(33, emp.getNation());

				if (emp.getGetqualificationtime() == null)
					ps.setNull(34, Types.DATE);
				else
					ps.setDate(34, emp.getGetqualificationtime());

				if (emp.getText1() == null)
					ps.setNull(35, Types.DATE);
				else
					ps.setDate(35, emp.getText1());

				if (emp.getWagelevel() == 0)
					ps.setNull(36, Types.INTEGER);
				else
					ps.setInt(36, emp.getWagelevel());

				// 参加工作年月
				if (emp.getJoinworkdate() == null)
					ps.setNull(37, Types.DATE);
				else
					ps.setDate(37, emp.getJoinworkdate());
				// 入党年月
				if (emp.getJoinpartydate() == null)
					ps.setNull(38, Types.DATE);
				else
					ps.setDate(38, emp.getJoinpartydate());
				// 入团年月
				if (emp.getJoingroupdate() == null)
					ps.setNull(39, Types.DATE);
				else
					ps.setDate(39, emp.getJoingroupdate());
				// 何时何校何专业毕业
				if ("".equals(emp.getThetimeandschoolandprofessional()))
					ps.setNull(40, Types.VARCHAR);
				else
					ps.setString(40, emp.getThetimeandschoolandprofessional());
				// 备注1
				if ("".equals(emp.getRemark1()))
					ps.setNull(41, Types.VARCHAR);
				else
					ps.setString(41, emp.getRemark1());
				// 备注2
				if ("".equals(emp.getRemark2()))
					ps.setNull(42, Types.VARCHAR);
				else
					ps.setString(42, emp.getRemark2());
				// 职务级别
				if (emp.getDutylevel() == null
						|| emp.getDutylevel().getId() == 0)
					ps.setNull(43, Types.INTEGER);
				else
					ps.setInt(43, emp.getDutylevel().getId());
				// 技术职务
				if (emp.getEchnicalposition() == null
						|| emp.getEchnicalposition().getId() == 0) {
					ps.setNull(44, Types.INTEGER);
				} else {
					ps.setInt(44, emp.getEchnicalposition().getId());
				}
				// 专业技术级别
				if (emp.getProfessionallevel() == null
						|| emp.getProfessionallevel().getId() == 0) {
					ps.setNull(45, Types.INTEGER);
				} else {
					ps.setInt(45, emp.getProfessionallevel().getId());
				}
				// 现有文化程度
				if (emp.getDegree() == null || emp.getDegree().getId() == 0) {
					ps.setNull(46, Types.INTEGER);
				} else {
					ps.setInt(46, emp.getDegree().getId());
				}
				// 工种
				if (emp.getWorktype() == null || emp.getWorktype().getId() == 0) {
					ps.setNull(47, Types.INTEGER);
				} else {
					ps.setInt(47, emp.getWorktype().getId());
				}
				// 备注3
				if ("".equals(emp.getRemark3()))
					ps.setNull(48, Types.VARCHAR);
				else
					ps.setString(48, emp.getRemark3());

				// 身份
				if (emp.getIdentity() == null || emp.getIdentity().getId() == 0) {
					ps.setNull(49, Types.INTEGER);
				} else {
					ps.setInt(49, emp.getIdentity().getId());
				}
				// 退休时间
				if (emp.getRetirementdate() == null)
					ps.setNull(50, Types.DATE);
				else
					ps.setDate(50, emp.getRetirementdate());
	...
	public List getOrgTree(final int type, final int id) {
		...
		if (type == 1) {
			final String sql = "select h.公司ID as compID,h.名称缩写 as compName,COUNT(h.人员ID) as num from "
					+ " (select a.公司ID,a.名称缩写,e.人员ID from [xmis].[org_公司] a  "
					...
					+ "      OR e.在职类型 IS NULL\n"
					+ "    )\n"
					+ " where a.公司ID = ? " + ") h group by h.公司ID,h.名称缩写 ";
	...
	public void doUpdateEmpDeptBatch(final Map<String, String> mappingEmplMap,
			final int updateID) {
		...
		jt.batchUpdate(SQLScript.setEmpBelongToDept, setter);
	}

	public void doUpdateEmpRoleBatch(final Map<String, String> mappingEmplMap,
			final int updateID) {
		BatchPreparedStatementSetter setter = null;
		setter = new BatchPreparedStatementSetter() {
			public int getBatchSize() {
		...
		jt.batchUpdate(SQLScript.setEmpRole, setter);
	}
	...
	// bbz
	public List getBizloigcDataListForBengBuZha(final String paramStr,
			final String type) throws DataAccessException, SQLException {

		if (type.equals(WebKeys.Biz_Logic_Department_Type)) {
			return (List) jt.execute(new ConnectionCallback() {

				public List doInConnection(Connection conn)
						throws SQLException, DataAccessException {

					CallableStatement cs = conn
							.prepareCall("{call xmis.getBizlogicDeptList(?)}");
					cs.setString(1, paramStr);
	...
	public List getOrganizationTree(int type, final int id) {
		// TODO Auto-generated method stub
		if (type == 3) {
			final String sql = "select xmis.org_部门.部门ID,xmis.org_部门.部门名称 from xmis.org_部门归属部门表"
					+ " left join xmis.org_部门 on xmis.org_部门.部门ID = xmis.org_部门归属部门表.部门ID"
	...
	public boolean getDeptOfUserLimit(final int id) {
		...
		// final String sql =
		// "select b.[父部门ID] from xmis.org_部门归属公司表 a,[xmis].[org_部门归属部门表] b where a.部门ID=? and a.部门ID=b.部门ID";
		// bbz mysql
		final String sql = "select b.父部门ID from xmis.org_部门归属公司表 a,xmis.org_部门归属部门表 b where a.部门ID=? and a.部门ID=b.部门ID;";
		...
		return (limit != null && limit.size() > 0 && Type.getBoolean(limit
	...
	public List getAllEmpInfo() {
		logger.info("---->addressBook(dao--getAllEmpInfo)");
		String sqlStr = SQLScript.getAllEmpInfo;
		logger.info("---->addressBook(dao--getAllEmpInfo)" + sqlStr);
		PreparedStatementCallback ps = new PreparedStatementCallback() {
			public Object doInPreparedStatement(PreparedStatement arg0)
			throws SQLException, DataAccessException {
				ResultSet rs = arg0.executeQuery();
	...
	public void updateDisableReason(int empId, String reason, String disable) {
		String sql = "update hr_人员 set 禁用原因=?,禁用=?,禁用时间=getdate() where 人员ID=?";
		jt.update(
				sql,
				new Object[] { new String(reason),
						new Integer(Type.GetInt(disable)), new Integer(empId) });
	...
	public boolean isHasExistAttendanceWarnInfo(String msg, String nowdate) {
		...
		int count = jt.queryForInt(sql, new Object[] { new String(msg),
				new String(nowdate) });
		if (count > 0) {
			return true;
		}
	...
	public int doSavePostMaterials(String materialsID, String imgName) {
		return jt.update(SQLScript.savePostMaterials, new Object[] { imgName,
				materialsID });
	}

	public List getEmpsByDeptLeaderMapping(int deptId,
			int roleFlag) {
		String sql =SQLScript.getEmpsByDeptLeaderMapping;
	return jt.queryForList(sql, new Object[] { deptId, roleFlag });
	}

	@Override
	public List getDutyPlanList(PageInfo pageInfo) {
		String sql =SQLScript.getDutyPlanList;
		return jt.queryForList(sql);
	}

	@Override
	public void saveDutyPlan(T_OnWatch_schedule tOsc) {
		String sql = "select count(*) from T_OnWatch_schedule where id='" +tOsc.getId()+"'";
		int temp = jt.queryForInt(sql);
		if(temp>0){
		String updatesql = "update T_OnWatch_schedule set job_type_id = '"+tOsc.getJobTypeId()+"',on_watch_id='"+tOsc.getOnWatchId()+"'"
				+ ",on_watch_date='"+tOsc.getOnWatchDate()+"',begin_time ='"+tOsc.getBeginTime()+"',end_date='"+tOsc.getEndDate()+"',end_time='"+tOsc.getEndTime()+"'"
						+ ",reason='"+tOsc.getReason()+"',last_updated_id='"+tOsc.getLastUpdatedId()+"',last_updated_datetime='"+tOsc.getLastUpdatedDatetime()+"'"
								+ " where id='" +tOsc.getId()+"'";
			jt.update(updatesql);
		}else{
			ht.save(tOsc);
	...
	@Override
	public void deleteDutyPlan(final String id) {
		
		String sql = "delete from T_OnWatch_schedule where id=?" ;
		jt.update(sql, new PreparedStatementSetter() {
			public void setValues(PreparedStatement ps) throws SQLException {
				ps.setString(1,id);
			}
	...
	@Override
	public List getDutyPlanListByDate(PageInfo pageInfo, String onWatchDate) {
		String sql =SQLScript.getDutyPlanListByDate;
		return jt.queryForList(sql,new Object[] {onWatchDate,onWatchDate});
	}

	@Override
	public List getDutyPlanListByPart(PageInfo pageInfo, String department) {
		String sql =SQLScript.getDutyPlanListByPart;
		return jt.queryForList(sql,new Object[]{department});
	}

	@Override
	public List getDutyPlanListByPartAndDate(PageInfo pageInfo,
			String department, String onWatchDate) {
		String sql =SQLScript.getDutyPlanListByPartAndDate;
		return jt.queryForList(sql,new Object[]{department,onWatchDate,onWatchDate});
	}

	@Override
	public List getSexCount() {
		String sql =SQLScript.getSexCount;
		List list =jt.queryForList(sql);
		final Map<String, String> map = new HashMap<String, String>();
		map.put("男性", 1+"");
		map.put("女性", 2+"");
		java.util.Collections.sort(list, new Comparator<Map<String,Object>>() {
			@Override
			public int compare(Map<String, Object> o1, Map<String, Object> o2) {
				
				return Type.GetInt(map.get(o1.get("sex")))-Type.GetInt(map.get(o2.get("sex")));
			}
		} );
		return list;
	}
	...
	@Override
	public List getSexList(String sex, PageInfo pageInfo) {
		String sql =SQLScript.getSexList;
		if("男性".equals(sex)){
			sql = sql+" and 性别ID = 1";
	...
	@Override
	public List getZzmmCount() {
		String sql =SQLScript.getZzmmCount;
		List list =jt.queryForList(sql);
		final Map<String, String> map = new HashMap<String, String>();
		map.put("中央党员", 1+"");
		map.put("共青团员", 2+"");
		...
		java.util.Collections.sort(list, new Comparator<Map<String,Object>>() {
			...
			public int compare(Map<String, Object> o1, Map<String, Object> o2) {
				...
				return Type.GetInt(map.get(o1.get("zzmm")))-Type.GetInt(map.get(o2.get("zzmm")));
			}
		} );
		return list;
	}
	...
	@Override
	public List getZzmmList(String zzmm, PageInfo pageInfo) {
		String sql =SQLScript.getZzmmList;
		if("普通群众".equals(zzmm)){
			sql = sql+ " and  (政治面貌ID = '3' or 政治面貌ID is null) order by 出生日期   desc";
		}else if("中央党员".equals(zzmm)){
			sql = sql+ " and 政治面貌ID = '1' order by 出生日期   desc";
		}else if("共青团员".equals(zzmm)){
			sql = sql+ " and 政治面貌ID = '2' order by 出生日期   desc";
		}
		List list =  jt.queryForList(sql);
		return list;
	}
	...
	public List getAgeCount() {
		String sql =SQLScript.getAgeCount;
		return jt.queryForList(sql);
	}

	@Override
	public List getAgeList(String age, PageInfo pageInfo) {
		String sql =SQLScript.getAgeList;
		if("30岁及以下".equals(age)){
			sql = sql+ " AND DATEDIFF(yyyy,出生日期,GETDATE()) <=30 order by 出生日期   desc";
		}else if("31岁到40岁".equals(age)){
			sql = sql+ " AND DATEDIFF(yyyy,出生日期,GETDATE()) >30 AND DATEDIFF(yyyy,出生日期,GETDATE()) <=40 order by 出生日期   desc";
		}else if("41岁到50岁".equals(age)){
			sql = sql+ " AND DATEDIFF(yyyy,出生日期,GETDATE()) >40 AND DATEDIFF(yyyy,出生日期,GETDATE()) <=50 order by 出生日期   desc";
		}else if("50岁及以上".equals(age)){
			sql = sql+ " AND DATEDIFF(yyyy,出生日期,GETDATE()) >50 order by 出生日期   desc";
		}else if("出生日期为空".equals(age)){
			sql = sql+ " AND 出生日期  is null";
		}
		return jt.queryForList(sql);
	}

	@Override
	public List getDutyCount() {
		String sql =SQLScript.getDutyCount;
		List list =jt.queryForList(sql);
		final Map<String, String> map = new HashMap<String, String>();
		map.put("高层", 1+"");
		map.put("中层", 2+"");
		...
		java.util.Collections.sort(list, new Comparator<Map<String,Object>>() {
			...
			public int compare(Map<String, Object> o1, Map<String, Object> o2) {
				...
				return Type.GetInt(map.get(o1.get("duty")))-Type.GetInt(map.get(o2.get("duty")));
			}
		} );
		return list;
	}
	...
	@Override
	public List getDutyList(String duty, PageInfo pageInfo) {
		String sql =SQLScript.getDutyList;
		if("高层".equals(duty)){
			sql = sql+" and (dp.职务 = 1 or dp.职务 = 2 or dp.职务 = 3)";
		}else if("中层".equals(duty)){
			sql = sql+" and (dp.职务 = 4 or dp.职务 = 5)";
		}else if("普通职工".equals(duty)){
			sql = sql+" and (dp.职务 =7 or dp.职务  is null)";
		}else if("班组长".equals(duty)){
			sql = sql+" and dp.职务 =6";
		}
		return jt.queryForList(sql);
	}
	...
	@Override
	public List getTitlesCount() {
		String sql =SQLScript.getTitlesCount;
		List list =jt.queryForList(sql);
		final Map<String, String> map = new HashMap<String, String>();
		map.put("高级职称", 1+"");
		map.put("中级职称", 2+"");
		...
		java.util.Collections.sort(list, new Comparator<Map<String,Object>>() {
			...
			public int compare(Map<String, Object> o1, Map<String, Object> o2) {
				...
				return Type.GetInt(map.get(o1.get("titles")))-Type.GetInt(map.get(o2.get("titles")));
			}
		} );
		return list;
	}
	...
	@Override
	public List getTitlesList(String titles, PageInfo pageInfo) {
		String sql =SQLScript.getTitlesList;
		if("高级职称".equals(titles)){
			sql = sql+" and (p.技术职务 = 3 OR p.技术职务 =4 OR p.技术职务 =5)";
		}else if("中级职称".equals(titles)){
			sql = sql+" and (p.技术职务 = 6 OR p.技术职务 =12 OR p.技术职务 =15)";
		}else if("高级工".equals(titles)){
			sql = sql+" and p.技术职务= 2";
		}else if("中级工".equals(titles)){
			sql = sql+" and p.技术职务= 14";
		}else if("其他".equals(titles)){
			sql = sql+" and  (p.技术职务 = 8 OR p.技术职务 =7 OR p.技术职务 =13 OR p.技术职务 =11 OR p.技术职务 =10 OR p.技术职务  is null)";
		}else if("初级".equals(titles)){
			sql = sql+" and (p.技术职务 = 16  OR p.技术职务 =17 OR p.技术职务 =18 OR p.技术职务 =19 OR p.技术职务 =20 OR p.技术职务 =21 OR p.技术职务 =22 OR p.技术职务 =9  OR p.技术职务 =1)";
		}
		return jt.queryForList(sql);
	}

	@Override
	public List getJsjbCount() {
		String sql =SQLScript.getJsjbCount;
		return jt.queryForList(sql);
	}

	@Override
	public List getJsjbList(String jsjb, PageInfo pageInfo) {
		String sql =SQLScript.getJsjbList;
		if("高级".equals(jsjb)){
			sql = sql+" and p.专业技术级别 = 1";
		}else if("中级".equals(jsjb)){
			sql = sql+" and p.专业技术级别 = 2";
		}else if("助理级及其他".equals(jsjb)){
			sql = sql+" and (p.专业技术级别 =3  or p.专业技术级别  is null)";
		}
		return jt.queryForList(sql);
	}

	@Override
	public List getWhcdCount() {
		String sql =SQLScript.getWhcdCount;
		List list =jt.queryForList(sql);
		final Map<String, String> map = new HashMap<String, String>();
		map.put("硕士及研究生", 1+"");
		map.put("本科", 2+"");
		...
		java.util.Collections.sort(list, new Comparator<Map<String,Object>>() {
			...
			public int compare(Map<String, Object> o1, Map<String, Object> o2) {
				...
				return Type.GetInt(map.get(o1.get("whcd")))-Type.GetInt(map.get(o2.get("whcd")));
			}
		} );
		return list;
	}
	...
	@Override
	public List getWhcdList(String whcd, PageInfo pageInfo) {
		String sql =SQLScript.getWhcdList;
		if("硕士及研究生".equals(whcd)){
			sql = sql+" and (p.现有文化程度 = 2 OR  p.现有文化程度 =8 OR p.现有文化程度 =9)";
		}else if("本科".equals(whcd)){
			sql = sql+" and p.现有文化程度 =1";
		}else if("大专".equals(whcd)){
			sql = sql+" and ( p.现有文化程度 =4 OR p.现有文化程度 = 7)";
		}else if("中专".equals(whcd)){
			sql = sql+" and ( p.现有文化程度 =5 OR p.现有文化程度 =6 OR p.现有文化程度 =10)";
		}else if("高中以下".equals(whcd)){
			sql = sql+" and ( p.现有文化程度 =3 OR p.现有文化程度 IS NULL)";
		}
		return jt.queryForList(sql);
	}

	@Override
	public List getZgsfCount() {
		String sql =SQLScript.getZgsfCount;
		List list =jt.queryForList(sql);
		final Map<String, String> map = new HashMap<String, String>();
		map.put("干部", 1+"");
		map.put("工人", 2+"");
		...
		java.util.Collections.sort(list, new Comparator<Map<String,Object>>() {
			...
			public int compare(Map<String, Object> o1, Map<String, Object> o2) {
				...
				return Type.GetInt(map.get(o1.get("zgsf")))-Type.GetInt(map.get(o2.get("zgsf")));
			}
		} );
		return list;
	}
	...
	@Override
	public List getZgsfList(String zgsf, PageInfo pageInfo) {
		String sql =SQLScript.getZgsfList;
		if("干部".equals(zgsf)){
			sql = sql+" and p.身份 =1 order by 出生日期   desc";
		}else if("工人".equals(zgsf)){
			sql = sql+" and p.身份 =2 order by 出生日期   desc";
		}else if("合同制工人".equals(zgsf)){
			sql = sql+" and p.身份 =3 order by 出生日期   desc";
		}else if("聘用干部".equals(zgsf)){
			sql = sql+" and p.身份 =4 order by 出生日期   desc";
		}else if("离休".equals(zgsf)){
			sql = sql+" and p.身份 = 5 order by 出生日期   desc";
		}else if("退休干部".equals(zgsf)){
			sql = sql+" and p.身份 =6 order by 出生日期   desc";
		}else if("退休工人".equals(zgsf)){
			sql = sql+" and (p.身份 = 7 or p.身份  is null) order by 出生日期   desc";
		}
		return jt.queryForList(sql);
	}

	@Override
	public List getZzygCount() {
		String sql =SQLScript.getZzygCount;
		List list =jt.queryForList(sql);
		final Map<String, String> map = new HashMap<String, String>();
		map.put("在职", 1+"");
		map.put("退休", 2+"");
		java.util.Collections.sort(list, new Comparator<Map<String,Object>>() {
			...
			public int compare(Map<String, Object> o1, Map<String, Object> o2) {
				...
				return Type.GetInt(map.get(o1.get("zzyg")))-Type.GetInt(map.get(o2.get("zzyg")));
			}
		} );
		return list;
	}
	...
	@Override
	public List getZzygList(String zzyg, PageInfo pageInfo) {
		String sql =SQLScript.getZzygList;
		if("在职".equals(zzyg)){
			sql = sql+" and (p.身份 =1 or p.身份 =2 or p.身份 =3 or p.身份 =4  ) order by 出生日期   desc";
		}else if("退休".equals(zzyg)){
			sql = sql+" and (p.身份 = 5 or p.身份 =6 or p.身份 =7 or p.身份  is null) order by 出生日期   desc";
		}
		return jt.queryForList(sql);
	}

	@Override
	public List getDeptManagerList() {
		String sql = SQLScript.getDeptManagerList;
		return jt.queryForList(sql);
	}
