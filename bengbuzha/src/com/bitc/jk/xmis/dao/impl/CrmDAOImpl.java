...
public class CrmDAOImpl extends BaseDAOImpl implements CrmDAO {
	...
	public List getMeetingRoomInfo(String room_id) {
		// TODO Auto-generated method stub
		String sql ="select room_id as room_id," +
					" room_No as room_No," +
					" room_name as room_name," +
					" floor as floor," +
					" address as address," +
					" remark as remark," +
					" number_people as number_people " +
					" from T_MeetingRoom_Info where room_id = ?";
		return jt.queryForList(sql, new Object[] { room_id});
	}
	...
	public void dodelMeetingRoomInfoById(String room_id) {
		// TODO Auto-generated method stub
		String sql ="delete from T_MeetingRoom_Info where room_id =? ";
		jt.update(sql, new Object[] { room_id });
	}

	@Override
	public void doVehicleManagement(VehicleManagement vm) {
		// TODO Auto-generated method stub
		if ("".equals(vm.getCar_id())) {
			saveObject(vm);
		}else{
			upldateObject(vm);
		}
	...
	public List getVehicleManagement() {
		// TODO Auto-generated method stub
		String sql = "select car_id as car_id,car_No as car_No,car_brand as car_brand," +
					 " car_type as car_type,CONVERT(VARCHAR,purchase_date , 120) as purchase_date,capacity as capacity," +
					 " remark as remark from T_Car_Info ";
		return jt.queryForList(sql, new Object[] {});
	}
	...
	public List getVehicleManagemenid(String car_id) {
		...
		String sql = "select car_id as car_id,car_No as car_No,car_brand as car_brand," +
		 " car_type as car_type,CONVERT(VARCHAR,purchase_date , 120) as purchase_date,capacity as capacity," +
		 " remark as remark from T_Car_Info where car_id = ? ";
		return jt.queryForList(sql, new Object[] { car_id});
	}

	@Override
	public void dodelVehicleManagemenById(String car_id) {
		// TODO Auto-generated method stub
		String sql ="delete from T_Car_Info where car_id =? ";
		jt.update(sql, new Object[] { car_id });
	}
	
	@Override
	public void doOfficeSupplies(OfficeSuppliesDetails osds) {
		// TODO Auto-generated method stub
		if ("".equals(osds.getSupplies_id())) {
			saveObject(osds);
		}else{
			upldateObject(osds);
		}
	...
	public List getOfficeSupplies(String queryStr, String ordersql) {
		...
		String sql = SQLScript.getOfficeSuppliesSQL;
		if (queryStr != null && !"".equals(queryStr)) {
			queryStr = "'%" + queryStr + "%'";
			String sqlend = "\n where supplies_name like  " + queryStr + "  \n"
					+ "or supplies_type_Name like    " + queryStr
					+ " \n";
			String sqlresult = sql + sqlend + ordersql+"  order by supplies_type_name";
			return jt.queryForList(sqlresult);
		} else {
			return jt.queryForList(sql + ordersql+"  order by supplies_type_name");
		}
	}
	...
	public List getOfficeSuppliesid(String supplies_id) {
		// TODO Auto-generated method stub
		String sql =" select supplies_id as supplies_id,t1.supplies_type_id as supplies_type_id,t2.supplies_type_Name as supplies_type_name," +
				" supplies_name as supplies_name,supplies_brand as supplies_brand," +
				" supplies_model as supplies_model,CONVERT(VARCHAR,purchase_date , 120) as purchase_date," +
				" qty as qty,t1.remark as remark " +
				"  from T_Office_Supplies_Info t1,T_office_supplies_type_info t2 " +
				" where t1.supplies_type_id = t2.supplies_type_id " +
				" and supplies_id =? ";
		return jt.queryForList(sql, new Object[] { supplies_id});
	}

	@Override
	public void dodelOfficeSuppliesById(String supplies_id) {
		// TODO Auto-generated method stub
		String sql =" delete from T_Office_Supplies_Info where supplies_id =?";
		jt.update(sql, new Object[] { supplies_id });
	}

	@Override
	public void doStaffRoles(final String id, final Integer userID, final String empID) {
		// TODO Auto-generated method stub
		String sql = "insert into sec_人员角色 (ID,人员ID,角色ID,最后修改日期) values(?,?,?,GETDATE())";
		jt.update(sql, new PreparedStatementSetter() {
			public void setValues(PreparedStatement ps) throws SQLException {
				ps.setString(1, id);
				ps.setInt(2, userID);
	...
	public List getStaffRoles(Integer userID) {
		// TODO Auto-generated method stub
		String sql = "select ID as id,人员ID as userID,角色ID as jID  from sec_人员角色  where 人员ID = ? ";
	...
	public void dodelSataffRoles(Integer userID) {
		// TODO Auto-generated method stub
		String sql = "delete from sec_人员角色 where 人员ID =? AND 角色ID IN (SELECT s.角色ID FROM  xmis.sec_角色 s WHERE   s.显示标志=1) ";
	...
	public List getDepartment() {
		// TODO Auto-generated method stub
		String sql = "SELECT d.部门ID AS BMID,dept.部门名称 AS BMMC,t.role_flag AS JSID FROM org_部门归属公司表 d " +
				"left join T_Dept_Leader_Mapping t on t.dept_id= d.部门ID " +
				"left join org_部门 dept on (d.部门ID = dept.部门ID and (dept.禁用 =0 or dept.禁用  is null) ) " +
				"WHERE  d.公司ID = 1 or t.id is null order by d.排序 ";
		return jt.queryForList(sql, new Object[] {  });
	} 
	...
	System.out.println("=-=-=============="+userID);
		...
		String sql = "SELECT d.部门ID AS BMID,dept.部门名称 AS BMMC,CASE WHEN t.role_flag  = '0' THEN "
				+ WebKeys.APPLY_DEPTMANAGER_ROLE
				+ " WHEN t.role_flag = '1' THEN "
				+ WebKeys.APPLY_CHARGELEADER_ROLE
				+ " ELSE NULL END AS JSID FROM org_部门归属公司表 d "
				+ "LEFT JOIN T_Dept_Leader_Mapping t ON ( d.部门ID =t.dept_id  AND t.user_id = ? ) "
				+ "JOIN org_部门 dept ON (d.部门ID = dept.部门ID and (dept.禁用 =0 or dept.禁用  is null) ) " 
				+ "WHERE  d.公司ID = 1 order by d.排序  ";
		return jt.queryForList(sql, new Object[] { userID });
	}
	@Override
	public List getOfficeSuppliesCombot() {
		// TODO Auto-generated method stub
		String sql = "select 角色ID as supplies_type_id ,RTRIM(角色名称) as supplies_type_Name from sec_角色 where 角色ID in ("
				+ WebKeys.APPLY_DEPTMANAGER_ROLE + ","
				+ WebKeys.APPLY_CHARGELEADER_ROLE + ")";
		return jt.queryForList(sql, new Object[] {  });
	}
	...
	@Override
	public void doStaffRolestty(final String enjsID, final Integer userID,final String empID) {
		// TODO Auto-generated method stub
		String sql = "insert into T_Dept_Leader_Mapping (id,role_flag,user_id,dept_id) values(newid(),?,?,?)";
		jt.update(sql, new PreparedStatementSetter() {
			public void setValues(PreparedStatement ps) throws SQLException {
				ps.setString(1, enjsID);
				ps.setInt(2, userID);
				ps.setString(3, empID);
	...
	public void dodelSataffRolestty(Integer userID) {
		// TODO Auto-generated method stub
		String sql = "delete from T_Dept_Leader_Mapping where user_id =? ";
		jt.update(sql, new Object[] { userID });
	}

	@Override
	public void doStaffRoles(final Integer userID, final String roeID) {
		// TODO Auto-generated method stub
		String sql = "insert into sec_人员角色 (ID,人员ID,角色ID,最后修改日期) values(newid(),?,?,GETDATE())";
		jt.update(sql, new PreparedStatementSetter() {
			public void setValues(PreparedStatement ps) throws SQLException {
				
				ps.setInt(1, userID);
				ps.setString(2, roeID);
	...
	public void dodelSataffRolesteqw(Integer userID) {
		...
		String sql = "delete from sec_人员角色 where 人员ID = ? and 角色ID in ("
				+ WebKeys.APPLY_DEPTMANAGER_ROLE + ","
				+ WebKeys.APPLY_CHARGELEADER_ROLE + ")";
	...
	public void doStaffRolesttyryyu(Integer userID) {
		// TODO Auto-generated method stub
		System.out.println("=====");
		String sql = "delete from T_Dept_Leader_Mapping user_id =? ";
		jt.update(sql, new Object[] { userID });
	}
}
