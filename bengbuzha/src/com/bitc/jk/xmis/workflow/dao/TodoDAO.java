...
//		return getListStore(query, pageInfo, Todo.class);
	...
	public void updateCurrentPersonReplayWfTrigger(final String id) {
		ht.executeWithNativeSession(new HibernateCallback() {
			public Object doInHibernate(Session session)
					throws HibernateException, SQLException {
//				return session.createSQLQuery(
//						"UPDATE  XMIS.wf_历史\n" +
//						"SET     当前执行人 = xmis.f_getwfactor(?,GETDATE())\n" + 
//						"FROM               xmis.wf_历史\n" + 
//				"WHERE   xmis.wf_历史.流程实例ID =?")
				return session.createSQLQuery(
						"UPDATE  XMIS.wf_历史\n" +
						"SET     备注 = xmis.f_getwfactor(?,GETDATE())\n" + 
						"FROM               xmis.wf_历史\n" + 
						"WHERE   xmis.wf_历史.流程实例ID =?")
						.setString(0, id).setString(1, id)
						.executeUpdate();
