...
protected:
	...
	void rollover();
	QDateTime calculateNextRolloverTime(const QDateTime& t) const;
	QDateTime calculateOverTime(const QDateTime& time) const;
	QString fileName(const QString& filename, const QDateTime& t, DailyRollingFileSchedule schedule) const;

protected:
	QDateTime adjustDateTime(const QDateTime& date) const;
