	...
	: QFrame(parent)
{
	...
	m_pGridLayout=new QGridLayout(groupBoxCpuRecords);
	groupBoxCpuRecords->setLayout(m_pGridLayout);
...
void QFrameProcessor::showPerfProcessor()
{
	...
	if (m_mapFrameCurve.size() == 0)
	{
		...
		for(int i=0; i<m_perfProcessor.core.size(); i++)
		{
			...
			QFrameCurve* pFrameCurve=new QFrameCurve(groupBoxCpuRecords);
...
void QFrameProcessor::currentIndexChanged(int index)
{
	if (index >= 0)
	{
		...
		{
			labelLogo->setPixmap(QPixmap(":/images/intel.png"));
		}
		else if (processor.manufacturer == "AuthenticAMD")
		{
			labelLogo->setPixmap(QPixmap(":/images/amd.png"));
		}
		else
		{
			labelLogo->setPixmap(QPixmap(":/images/processor.png"));
		}


		QString strDesc;
		strDesc=tr("Name:\t\t")+(processor.name.isEmpty()? tr("Unknown"):processor.name)
			+"\n"+tr("Manufacturer:\t\t")+(processor.manufacturer.isEmpty()? tr("Unknown"):processor.manufacturer)
			+"\n"+tr("Address Width:\t")+(processor.addressWidth==-1? tr("Unknown"):QString::number(processor.addressWidth)+" "+tr("bits"))
			+"\n"+tr("Data Width:\t\t")+(processor.dataWidth==-1? tr("Unknown"):QString::number(processor.dataWidth)+" "+tr("bits"))
			+"\n"+tr("Max Clock Speed:\t")+(processor.maxClockSpeed==-1? tr("Unknown"):QString::number(processor.maxClockSpeed)+" "+ tr("MHz"))
			+"\n"+tr("External Clock:\t")+(processor.externalClock==-1? tr("Unknown"):QString::number(processor.externalClock)+" "+ tr("MHz"))
			+"\n"+tr("L2 Cache Size:\t")+(processor.L2CacheSize==-1? tr("Unknown"):sizeToString(processor.L2CacheSize))
			+"\n"+tr("L2 Cache Speed:\t")+(processor.L2CacheSpeed==-1? tr("Unknown"):QString::number(processor.L2CacheSpeed)+" "+ tr("MHz"))
			+"\n"+tr("Description:\t\t")+processor.description;
