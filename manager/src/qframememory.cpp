...
void QFrameMemory::currentIndexChanged(int index)
{
	if (index >= 0)
	{
		...
		const PhysicalMemory& memory=m_vecPhysicalMemory[nIndex];
		labelLogo->setPixmap(QPixmap(":/images/ddr.png"));
		...
		QString strDesc;
		strDesc=tr("Description:\t")+memory.description
			+"\n"+tr("Version:\t\t")+(memory.version.isEmpty()? tr("Unknown"):memory.version)
			+"\n"+tr("Manufacturer:\t")+(memory.manufacturer.isEmpty()? tr("Unknown"):memory.manufacturer)
			+"\n"+tr("Memory Type:\t")+memory.memoryType
			+"\n"+tr("Capacity:\t\t")+strCapacity
			+"\n"+tr("Data Width:\t\t")+strDataWidth
			+"\n"+tr("Speed:\t\t")+((memory.speed == -1 || memory.speed == 0)? tr("Unknown"):(QString::number(memory.speed)+" "+tr("ns")))
			+"\n"+tr("Tag:\t\t")+memory.tag
			+"\n"+tr("Serial Number:\t")+(memory.serialNumber.isEmpty()? tr("Unknown"):memory.serialNumber);
