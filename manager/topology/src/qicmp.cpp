...
m_pThreadRecv(NULL)
{
	...
	if (m_socket == -1)
	{
		LOGPLUS_DEBUG_STR(QLogger::root(), "Init SOCK_RAW failed!");
	...
	LOGPLUS_DEBUG_STR(QLogger::root(), "Start ICMP send and recv threads to find alive IP...");
...
void QIcmp::destroy()
{
	LOGPLUS_DEBUG_STR(QLogger::root(), "Stop ICMP send and recv threads.");
...
void QIcmp::nextTask()
{
	if (m_pThreadSend == NULL || m_pThreadRecv == NULL)
	{
		LOGPLUS_ERROR_STR(QLogger::root(), ("QIcmp: The send thread or the recv thread is NULL!!"));
		return;
	}
	...
	qDebug("QIcmp: Find alive IP in segment %s", QHostAddress(m_task).toString().toLocal8Bit().data());
...
void QThreadSendIcmp::run()
{
	...
	LOGPLUS_DEBUG_STR(QLogger::root(), "QThreadSendIcmp: Thread quit!");
}
...
void QThreadSendIcmp::addTask(quint32 segment)
{
	if (status(Tasking))
	{
		qDebug("QThreadSendIcmp: A task is running! Add task failed!");
		return;
	}

	qDebug("QThreadSendIcmp: Send ICMP in segment %s", QHostAddress(segment).toString().toLocal8Bit().data());
...
void QThreadSendIcmp::sendPacket(struct sockaddr_in& dest)
{
	...
	qDebug("QThreadSendIcmp: Send ICMP to %s", inet_ntoa(dest.sin_addr));
...
void QThreadRecvIcmp::run()
{
	...
	while(status(Running))
	{
		...
		{
			qDebug("QThreadRecvIcmp: recv ICMP packet from %s", inet_ntoa(from.sin_addr));

			emit icmpEcho(ntohl(from.sin_addr.s_addr));
		}
	}

	qDebug("QThreadRecvIcmp: Thread quit!");
}
