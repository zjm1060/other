Ext.define('bitc.wrenv.zdst.bengzhangongcheng', {
    ...
    initComponent: function () {
        var gridview = this.gridview = Ext.create('bitc.report.ReportView', {
            reportToken: 'bengzhangongchengGrid',
            region: 'center',
            listeners: {
    ...
    initGrid: function (panel, grid) {
        ...
        grid.on('itemdblclick', function (g, record) {
            ...
            this.superMap.reLocation(record.get('水利工程项目_ID'),record.get('泵站名称'),record.get(''),record.get('经纬度坐标'));
        }, this);
    },
    ...
    initForm: function (panel) {
        ...
        panel.down('toolbar').insert(0, [{
            ...
            beforeDelete: function () {
                if (panel.getForm().findField("PUMP_CD").value != null && panel.getForm().findField("水利工程项目_ID").value != null) {
                    return true;
                }
                return false;
            },
            getDeleteRows: function () {
                return {
                    ...
                    formFields: [{
                        tableName: 'WR_PUMP_B',
                        rowKey: 'PUMP_CD',
                        rowKeyValue: panel.getForm().findField("PUMP_CD").value
                    }, {
    ...
    reSetForm: function (record) {
        this.form.setValues([{
            ...
            name: 'PUMP_CD',
            value: record ? record.get('泵站工程代码') : null,
            rowKeyValue: record ? record.get('泵站工程代码') : null
        }, {
            name: 'PUMP_NM',
            value: record ? record.get('泵站名称') : null,
            rowKeyValue: record ? record.get('泵站工程代码') : null
        }, {
            name: 'LOC',
            ...
            name: 'PUMP_TP',
            value: record ? record.get('泵站类型ID') : null,
            rowKeyValue: record ? record.get('泵站工程代码') : null
        },{
            name: 'INS_Q',
            value: record ? record.get('装机流量') : null,
            rowKeyValue: record ? record.get('泵站工程代码') : null
        },{
            name: 'DES_HEAD',
            value: record ? record.get('设计扬程') : null,
            rowKeyValue: record ? record.get('泵站工程代码') : null
        },{
            name: 'PUMP_NUM',
            value: record ? record.get('水泵数量') : null,
            rowKeyValue: record ? record.get('泵站工程代码') : null
        },{
            name: 'ALL_EQU_WW',
            value: record ? record.get('设备总取水能力') : null,
            rowKeyValue: record ? record.get('泵站工程代码') : null
        }, {
            name: 'ENG_MAN_CD',
