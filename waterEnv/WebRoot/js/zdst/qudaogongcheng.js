Ext.define('bitc.wrenv.zdst.qudaogongcheng', {
    ...
    initComponent: function () {
        var gridview = this.gridview = Ext.create('bitc.report.ReportView', {
            reportToken: 'qudaogongchengGrid',
            region: 'center',
    ...
    initGrid: function (panel, grid) {
        ...
        grid.on('itemdblclick', function (g, record) {
            ...
            this.superMap.reLocation(record.get('水利工程项目_ID'),record.get('渠道名称'),record.get(''),record.get('经纬度坐标'));
        }, this);
    },
    ...
    initForm: function (panel) {
        ...
        panel.down('toolbar').insert(0, [{
            ...
            beforeDelete: function () {
                if (panel.getForm().findField("CHAN_CD").value != null && panel.getForm().findField("水利工程项目_ID").value != null) {
                    return true;
                }
                return false;
            },
            getDeleteRows: function () {
                return {
                    ...
                    formFields: [{
                        tableName: 'WR_CHAN_B',
                        rowKey: 'CHAN_CD',
                        rowKeyValue: panel.getForm().findField("CHAN_CD").value
                    }, {
    ...
    reSetForm: function (record) {
        this.form.setValues([{
            ...
            name: 'CHAN_CD',
            value: record ? record.get('渠道工程代码') : null,
            rowKeyValue: record ? record.get('渠道工程代码') : null
        }, {
            name: 'CHAN_NM',
            value: record ? record.get('渠道名称') : null,
            rowKeyValue: record ? record.get('渠道工程代码') : null
        },{
            name: 'CHAN_G',
            value: record ? record.get('渠道级别ID') : null,
            rowKeyValue: record ? record.get('渠道工程代码') : null
        }, {
            name: 'CHAN_DES_Q',
            value: record ? record.get('渠道设计流量') : null,
            rowKeyValue: record ? record.get('渠道工程代码') : null
        },{
            name: 'LEAK_R',
            value: record ? record.get('渗漏系数') : null,
            rowKeyValue: record ? record.get('渠道工程代码') : null
        },{
            name: 'CHAN_LEN',
            value: record ? record.get('渠道长度') : null,
            rowKeyValue: record ? record.get('渠道工程代码') : null
        },{
            name: 'LIN_LEN',
            value: record ? record.get('衬砌长度') : null,
            rowKeyValue: record ? record.get('渠道工程代码') : null
        },{
            name: 'ENG_MAN_CD',
