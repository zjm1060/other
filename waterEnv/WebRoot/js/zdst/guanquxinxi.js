Ext.define('bitc.wrenv.zdst.guanquxinxi', {
    ...
    initGrid: function (panel, grid) {
        ...
        grid.on('itemdblclick', function (g, record) {
            ...
            this.superMap.reLocation(record.get('水利工程项目_ID'),record.get('灌区名称'),record.get(''),record.get('经纬度坐标'));
        }, this);
    },
    ...
    initForm: function (panel) {
        ...
        panel.down('toolbar').insert(0, [{
            ...
            beforeDelete: function () {
                if (panel.getForm().findField("IRR_CD").value != null && panel.getForm().findField("水利工程项目_ID").value != null) {
                    return true;
                }
                return false;
            },
            getDeleteRows: function () {
                return {
                    ...
                    formFields: [{
                        tableName: 'WR_IRR_B',
                        rowKey: 'IRR_CD',
                        rowKeyValue: panel.getForm().findField("IRR_CD").value
                    }, {
    ...
    reSetForm: function (record) {
        this.form.setValues([{
            ...
            name: 'IRR_CD',
            value: record ? record.get('灌区代码') : null,
            rowKeyValue: record ? record.get('灌区代码') : null
        }, {
            name: 'IRR_NM',
            value: record ? record.get('灌区名称') : null,
            rowKeyValue: record ? record.get('灌区代码') : null
        },{
            name: 'IRR_TP',
            value: record ? record.get('灌区类型ID') : null,
            rowKeyValue: record ? record.get('灌区代码') : null
        }, {
            name: 'IRR_SCAL',
            value: record ? record.get('灌区规模ID') : null,
            rowKeyValue: record ? record.get('灌区代码') : null
        },{
            name: 'DES_IRR_A',
            value: record ? record.get('设计灌溉面积') : null,
            rowKeyValue: record ? record.get('灌区代码') : null
        },{
            name: 'IRR_A',
            value: record ? record.get('总灌溉面积') : null,
            rowKeyValue: record ? record.get('灌区代码') : null
        },{
            name: 'FIELD_IRR_A',
            value: record ? record.get('其中耕地灌溉面积') : null,
            rowKeyValue: record ? record.get('灌区代码') : null
        },{
            name: 'ENG_MAN_CD',
