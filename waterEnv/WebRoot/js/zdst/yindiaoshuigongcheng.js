Ext.define('bitc.wrenv.zdst.yindiaoshuigongcheng', {
    ...
    initComponent: function () {
        var gridview = this.gridview = Ext.create('bitc.report.ReportView', {
            reportToken: 'yindiaoshuigongchengGrid',
            region: 'center',
            listeners: {
        ...
        var form = this.form = Ext.create('bitc.report.FormView', {
            formToken: 'yindiaoshuigongchengForm',
            title: '基本信息',
            fieldConfig: {
                ENG_MAN_NM: {
    ...
    initGrid: function (panel, grid) {
        ...
        grid.on('itemdblclick', function (g, record) {
            ...
            this.superMap.reLocation(record.get('水利工程项目_ID'),record.get('引调水工程名称'),record.get(''),record.get('经纬度坐标'));
        }, this);
    },
            ...
            handler: function () {
                ...
                /* if (records.length == 0) {
                 ...
                 return;
                    ...
                    Ext.MessageBox.alert("提示", "请先选择相关联的引调水工程!");
                    return;
                } else {
                    var stIds = "";
    ...
    initForm: function (panel) {
        ...
        panel.down('toolbar').insert(0, [{
            ...
            beforeDelete: function () {
                if (panel.getForm().findField("WDE_CD").value != null && panel.getForm().findField("水利工程项目_ID").value != null) {
                    return true;
                }
                return false;
            },
            getDeleteRows: function () {
                return {
                    ...
                    formFields: [{
                        tableName: 'WR_WDE_B',
                        rowKey: 'WDE_CD',
                        rowKeyValue: panel.getForm().findField("WDE_CD").value
                    }, {
    ...
    reSetForm: function (record) {
        this.form.setValues([{
            ...
            name: 'WDE_CD',
            value: record ? record.get('引调水工程代码') : null,
            rowKeyValue: record ? record.get('引调水工程代码') : null
        }, {
            name: 'WDE_NM',
            value: record ? record.get('引调水工程名称') : null,
            rowKeyValue: record ? record.get('引调水工程代码') : null
        }, {
            name: 'SWS_CD',
            value: record ? record.get('地表水水源地代码') : null,
            rowKeyValue: record ? record.get('引调水工程代码') : null
        }, {
            name: 'SWS_NM',
            value: record ? record.get('地表水水源地名称') : null,
            rowKeyValue: record ? record.get('引调水工程代码') : null
        }, {
            name: 'LINE_A',
            value: record ? record.get('输水线路区') : null,
            rowKeyValue: record ? record.get('引调水工程代码') : null
        },{
            name: 'TOT_LINE_LEN',
            value: record ? record.get('输水干线总长') : null,
            rowKeyValue: record ? record.get('引调水工程代码') : null
        },{
            name: 'INTK_A',
            value: record ? record.get('受水区范围') : null,
            rowKeyValue: record ? record.get('引调水工程代码') : null
        },{
            name: 'WDE_TP',
            value: record ? record.get('引调水方式ID') : null,
            rowKeyValue: record ? record.get('引调水工程代码') : null
        },{
            name: 'CONS_COND',
            value: record ? record.get('建设状况ID') : null,
            rowKeyValue: record ? record.get('引调水工程代码') : null
        },{
            name: 'DES_WDE_Q',
            value: record ? record.get('设计引调水流量') : null,
            rowKeyValue: record ? record.get('引调水工程代码') : null
        },{
            name: 'DES_YR_DW',
            value: record ? record.get('设计年引调水量') : null,
            rowKeyValue: record ? record.get('引调水工程代码') : null
        }, {
            name: 'ENG_MAN_CD',
