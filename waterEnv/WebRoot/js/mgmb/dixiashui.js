Ext.define('bitc.wrenv.mgmb.dixiashui',{
    ...
    initComponent : function(){
        ...
        var shuiwenDXPanel = this.shuiwenDXPanel = Ext.create('bitc.report.ReportView', {
            title: '水文测站',
            reportToken: 'sydcz',
            reportCondition: "1=2",
            gridConfig: {
        ...
        this.items = [gridview,{
            ...
            frame: true,
            region: 'east',
            items: [form,superMap,syGrid,shuiwenDXPanel]
        }];
    ...
    initGrid : function(view,grid){
        ...
        grid.on('itemdblclick', function (g, record) {
            this.link(record);
            this.superMap.reLocation(record.get('水源地_ID'),record.get('地下水水源地名称'),record.get(''),record.get('经纬度坐标'));
        }, this);
        if (this.sydId) {
        ...
        grid.down('toolbar').insert(0, {
            ...
            handler: function () {
                ...
                if (records.length == 0) {
                    ...
                    Ext.MessageBox.alert("提示", "请先选择相关联的地下水!");
                    return;
                } else {
                    var stIds = "";
    ...
    link: function (record) {
        this.form.setValues([{
            ...
            name: 'GWS_CD',
            value: record.get('地下水水源地代码'),
            rowKeyValue: record.get('地下水水源地代码')
        }, {
            name: 'GWS_NM',
            value: record.get('地下水水源地名称'),
            rowKeyValue: record.get('地下水水源地代码')
        }, {
            name: 'GWS_A',
            value: record.get('地下水水源地面积'),
            rowKeyValue: record.get('地下水水源地代码')
        }, {
            name: 'RANG_DESC',
            value: record.get('范围描述'),
            rowKeyValue: record.get('地下水水源地代码')
        }, {
            name: 'WQ_COND',
            ...
            name: 'PUT_PROD_TM',
            value: Ext.util.Format.date(Ext.util.Format.substr(record.get('投产时间'),0,10), 'Y-m-d'),
            rowKeyValue: record.get('地下水水源地代码')
        }, {
            name: 'RUN_COND',
            ...
            name: 'DES_INT',
            value: record.get('设计年取水量'),
            rowKeyValue: record.get('地下水水源地代码')
        },{
            name: 'PERM_INT',
            value: record.get('年许可取水量'),
            rowKeyValue: record.get('地下水水源地代码')
        },{
            name: '经纬度坐标',
        ...
        this.shuiwenDXPanel.reload("水源地_ID='" + record.get('水源地_ID') + "'");
        var sydId = this.sydId = record.get('水源地_ID');
        var sydDm = this.sydDm = record.get('地下水水源地代码');
        Ext.Ajax.request({
