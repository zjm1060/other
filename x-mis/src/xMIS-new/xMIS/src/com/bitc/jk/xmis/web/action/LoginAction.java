	...
	public ActionForward updatePwd(ActionMapping mapping, ActionForm form,
		...
		try {
				if (organizationService.doChangePassword(Integer
					...
					String env=(String) context.getAttribute("environment");
					if("production".equals(env)){
						logger.info("production********修改密码，mysql执行密码信息的同步！");
						//密码修改  缺陷管理同步 bug
						
						final int userid = Type.GetInt(request.getParameter("userid"));
						final String pwd = request
						.getParameter("pwd");
						
						new Thread(new Runnable() {
							public void run() {
								try {
									organizationService.doUpdateMantisPwd(userid, pwd);
								} catch (Exception e) {
									logger.error(e);
								}
							}
