 ...
 * 
 */
public class SimpleParam implements StandardQuery {
	 ...
	 * @return 构造的新实例
	 */
	public static SimpleParam buildSimpleParam(String name, Object value,
	 ...
	 * @param operator
	 *            操作符类型， see {@link RelationalOperator}
	 *            取值为JDBCSQL时表示为JDBC使用的条件，取值为SQL时表示为Hibernate的SQL
	 * @param sql
	 * @param value
	 * @return
	 */
	public static SimpleParam buildSimpleParam(String operator, String sql,
			Object value) {
		SimpleParam simpleParam = new SimpleParam();
		simpleParam.setValue(value);
		simpleParam.setOperator(operator);
		simpleParam.setSql(sql);
		return simpleParam;
	}

	public String getJdbcSQL() {
		if (RelationalOperator.JDBCSQL.equals(this.operator)) {
			if (this.sql.indexOf("%?%") > -1) {
				return StringUtils.replace(this.sql, "?", getValueString());
			}
			return StringUtils.replace(this.sql, "?", "'" + getValueString()
					+ "'");
		} else {
			return name + " " + operator + " '" + getValueString() + "' ";
		}
	}
	...
	private String getValueString() {
		...
		} else if (RelationalOperator.LIKE.equals(operator)) {
	...
	public void addToSQLGenerator(SQLGenerator generator) {
		generator.addAppendSQL(getJdbcSQL());
	}
