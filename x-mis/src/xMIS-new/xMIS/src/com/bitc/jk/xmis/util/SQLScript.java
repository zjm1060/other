...
//			+ "        ) AS b\n" + "WHERE   #sumInfo.合同ID = b.支出合同ID"
	...
	select
			...
			+ " select reason.*,RTRIM(a.姓氏)+RTRIM(a.名字) as 经办人,RTRIM(b.姓氏)+RTRIM(b.名字) as 登记人 INTO #delay from [xmis].[xmis].[con_付款推迟原因] as reason \n"
			+ " left outer join xmis.hr_人员 as a on reason.经办人ID = a.人员ID \n"
			+ " left outer join xmis.hr_人员 as b on reason.登记人ID = b.人员ID  \n"
			+ " where ID in ( \n"
			+ " SELECT MAX(ID) \n"
			+ " FROM [xmis].[xmis].[con_付款推迟原因] group by 合同id) \n"
	...
	public static String getDelayReasonListByContractID = "select m.ID,m.合同ID,m.原因,m.经办人ID,m.登记人ID,CONVERT(CHAR(10), m.登记日期, 23) as 登记日期,rtrim(a.姓氏)+rtrim(a.名字) 经办人,rtrim(b.姓氏)+rtrim(b.名字) 登记人 from [xmis].[xmis].[con_付款推迟原因] m"
	...
	public static String getAttachmentListByRefID = select
			...
			+ DBSchema.Employee.FirstName + ")+RTRIM("
			+ DBSchema.Employee.TableName + "." + DBSchema.Employee.LastName
			+ ") as " + " uploader_name " + intoResult + from
			+ DBSchema.Attachment.TableName + ","
			+ DBSchema.AttachmentType.TableName + ","
	...
	public static String getContactPeasonIDByCompany = select
			...
			+ DBSchema.ContactPerson.TableName + where + "TRIM("
			+ DBSchema.ContactPerson.FirstName + ")+TRIM("
	...
	public static String getPaymentListByIDAndFas = "SET NOCOUNT ON "
			...
			+ " FROM [xmis].[xmis].[con_合同] where 合同ID = @contractID"
	...
	public static String getLeaderInfo = " \ndeclare @maxCP int, @maxBP int \n"
			...
			+ " left join xmis.org_部门归属部门表 f  on  b.部门ID =  f.部门ID \n"
			+ " left join xmis.org_部门 g  on  g.部门ID =  f.父部门ID left join xmis.add_地址 ad on a.办公地址ID = ad.地址ID    \n"
			+ " WHERE  a.人员ID >0  and  e.公司ID = 1 and  c.排序<102  and  (  a.禁用 = 0  or  a.禁用 is NULL  )  ) AS  new  \n"
	...
	public static String existValidateReturnAttID = select + " 附件ID " + " from "
	+ DBSchema.Attachment.TableName + " where "
	+ DBSchema.Attachment.AttachmentTypeID + " = ?" + " and "
	+ DBSchema.Attachment.REFID + " = ?" + " and "
	+ DBSchema.Attachment.AttachmentName + " = " + "?";
	...
	public static String getPaymentPromptList = "SET NOCOUNT ON \n  declare @i DECIMAL(18,10) set @i=? SELECT payed.合同ID,sum(收款金额) as 已收金额  "
			...
			+ " group by 合同ID "
			+ " select xmis.xmis.con_合同.合同ID \n"
			+ " ,case when xmis.xmis.con_合同.简称 is null or xmis.xmis.con_合同.简称 = '' then xmis.xmis.con_合同.合同名称 else xmis.xmis.con_合同.简称 end as 合同名称 \n"
			+ " ,xmis.xmis.con_合同.合同负责人 \n"
			+ " ,xmis.xmis.con_合同.乙方部门ID \n"
			+ " ,xmis.fas_glInfo.MB as 财务来款  \n"
			+ " ,xmis.xmis.con_合同.最后修改日期 as time \n"
			+ " into #payedFas \n"
			+ " from xmis.xmis.prj_项目,xmis.fas_glInfo,xmis.xmis.con_合同 \n"
			+ " where xmis.xmis.prj_项目.项目编码 = xmis.fas_glInfo.project_code \n"
			+ " and xmis.xmis.con_合同.项目ID = xmis.xmis.prj_项目.项目ID \n"
			+ " and xmis.fas_glInfo.MB>0 \n"
			+ " and xmis.xmis.con_合同.合同类型ID = 1   \n"
			+ " and xmis.xmis.con_合同.父合同ID is null --合同的财务来款 \n"
			+ " "
	...
	public static String getTaskListByQuery = "--根据taskId 将\"我的任务\"分类\n"
			...
			+ "        into #result \n"
			+ " FROM   xmis.xmis.T_FF_RT_TASKINSTANCE a ,\n"
			+ "        xmis.wf_待办 todo ,\n"
	...
	public static String delAttachById=
		"DELETE  FROM xmis.sys_附件\n" +
		"WHERE   附件ID =?";
...
//	public static String getImportDeviceSheetCount="SELECT COUNT(1)+1 FROM xmis.dev_设备清单导入Sheet WHERE 附件ID=?";
	public static String getTodayList=
		"SELECT  CAST(考勤日期 AS VARCHAR(10))  考勤日期,\n" +
		"        CAST(上班打卡时间 AS VARCHAR(8))  上班打卡时间 ,\n" + 
		"        CAST(下班打卡时间 AS VARCHAR(8))  下班打卡时间\n" + 
		"FROM    xmis.xmis.v_考勤打卡\n" + 
		"WHERE   考勤日期 = CONVERT(VARCHAR(20), GETDATE(), 23)\n" + 
		"        AND 人员ID = ?";

	public static String getYesterdayList=
		"SELECT TOP 1\n" +
		"        CAST(考勤日期 AS VARCHAR(10))  考勤日期 ,\n" + 
		"        CAST(上班打卡时间 AS VARCHAR(8))  上班打卡时间,\n" + 
		"        CAST(下班打卡时间 AS VARCHAR(8))  下班打卡时间\n" + 
		"FROM    xmis.xmis.v_考勤打卡\n" + 
		"WHERE   考勤日期 < CONVERT(VARCHAR(20), GETDATE(), 23)\n" + 
	...
	public static void main(String[] args) {
		System.out.println(getContractStateList);
	}
}
