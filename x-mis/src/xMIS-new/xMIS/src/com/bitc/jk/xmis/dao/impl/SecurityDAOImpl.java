	...
	public List getSecurityInfoList() {
		String sql = "\ndeclare @maxA int, @maxPD int \n"+
		 ...
		 " select @maxPD = MAX(排序)+1 from xmis.sys_职务 \n"+
			...
			" select a.角色名称 , a.角色描述  as 角色说明,d.部门ID,d.部门名称,p.人员ID , rtrim(p.姓氏)+rtrim(p.名字) as 姓名 ,PD.排序 as sortType,a.角色ID as roleID ,c.排序  as deptSort,CASE WHEN s.排序 IS NULL THEN @maxPD ELSE s.排序   END AS positionSort,CASE WHEN a.排序 IS NULL THEN @maxA ELSE a.排序  END AS 角色排序" +
			"  from   "+
			" xmis.sec_角色 as a,  "+
			"  xmis.sec_人员角色 as b,  "+
			"   xmis.hr_人员 as p,  "+
			...
			//所有员工的
			" UNION " +
			" SELECT a.角色名称 ,a.角色描述 AS 角色说明 ,null ,null , NULL ,'所有员工' ,CASE WHEN a.排序 IS NULL THEN @maxA ELSE a.排序  END AS 角色排序, 10000 ,@maxPD AS positionSort,a.角色ID AS roleID ,10000" +
			...
			//部门级的所有员工
			" UNION " +
			" select a.角色名称 , a.角色描述  as 角色说明,d.部门ID,d.部门名称,'' AS 人员ID , '所有员工' AS 姓名 ,PD.排序 as sortType,a.角色ID as roleID ,c.排序  as deptSort,@maxPD AS positionSort,CASE WHEN a.排序 IS NULL THEN @maxA ELSE a.排序  END AS 角色排序" +
			"  from   "+
			" xmis.sec_角色 as a,  "+
			"  xmis.sec_人员角色 as b,  "+
			"   xmis.hr_人员 as p,  "+
