...
public class MissAttendanceServiceImpl implements MissAttendanceService {
	...
	public String doStartMissAttence(MissAttendanceObj missAttendanceObj,
		...
		WorkflowUtil.addProcessInstanceVariable(processInstance, "isSystemTask",
				isSystemTask);
		
		WorkflowUtil.addProcessInstanceVariable(processInstance,
				"applicantID", applicant.getUserID());// 用于被退回的链接
		WorkflowUtil.addProcessInstanceVariable(processInstance,
				"isOfficialTravel", missAttendanceObj.getIsOfficialTravel());// 用于被退回的链接
		WorkflowUtil.addProcessInstanceVariable(processInstance,
				"isAgent", missAttendanceObj.getIsAgent());// 用于被退回的链接
		WorkflowUtil.addProcessInstanceVariable(processInstance,
	...
	public void doCreateTimerTaskSystem(String workItemId, String commandBeanName) {
		IWorkflowSession workflowSession = workflowContext.getWorkflowSession();
	...
	public Boolean isInDuration() {
		...
		String durationStrs = getSomeWfCalendarBeforeDate(this.getDurationNum(), lastDate);
		String[] arrays = durationStrs.split(",");
		String nowDate = DateUtil.formatDate(DateUtil.getCurrentDate(),  "yyyyMMdd");
		boolean result = false;
	 ...
	 */
	public String doApprovalTask(String workItemId, String isAgree, String comment)
			throws WorkflowException {
		...
		WorkflowUtil.completeTask(workflowSession, workItemId,
				Type.getString(emp.getUserID()), "0".equals(isAgree)?("[审批通过] "+comment):("[审批未通过] "+comment));
...
//				Type.getString(emp.getUserID()), "0".equals(isAgree)?("[审批通过] "+comment):("[审批未通过] "+comment));
	...
	public String doApprovalTaskList(String workItemIds, String isAgree,
		...
		//自动签收
		workflowService.doClaimTaskList(workItemIds, Type.getString(WebUtil.getCurrentEmployee().getUserID()));
	...
	public void doWarnInfo() {
		...
		try {
			if (nowdate.equals(w1) || nowdate.equals(w2)) {
				boolean isHasExist = organizationDAO.isHasExistAttendanceWarnInfo(msg, nowdate);
				if (!isHasExist) {
					...
					List<String> receiverList_p =  missAttendanceDAO.getAllMyCompEmailList() ;
					String receverAddress = receiverList_p.get(0);

					EmailMessageInfo messageObj = new EmailMessageInfo();
					messageObj.setTitle(msg);
					messageObj.setContent("xMis中有一条新的消息： " + msg + " http://xmis.bitc.net.cn/");
...
//					messageObj.setReceiverList(receiverList_p);
					try {
						...
						logger.info("MissAttendanceServiceImpl//doWarnInfo----//"+e);
					}
				}
			}
		} catch (Exception e) {
	...
	public String doGoOnSubmitMissAttence(String missAttendanceId,
		...
		IProcessInstance processInstance=WorkflowUtil.getProcessInstance(workflowSession, workItemId);
		WorkflowUtil.addProcessInstanceVariable(processInstance,
				"isAgree", true);// 再次提交被默认同意
	...
	public void doEndMissAttence(String workItemId,String missAttendanceId) throws WorkflowException, EngineException {
...
//		missAttendanceDAO.doDeleteMissAttendanceObj(missAttendanceObj);
	...
	public String doSaveMissAttence(String missAttendanceId,MissAttendanceObj missAttendanceObj,
	...
	public void doUpdateMissAttendanceRecordState(String infoId,int year, int month,int state) {
		boolean isExist = missAttendanceDAO.isExistMissAttendanceRecord(infoId,year, month);
		if(isExist){
			missAttendanceDAO.doUpdateMissAttendanceRecordState(infoId,year, month,state );
		}else{
			missAttendanceDAO.doCreateMissAttendanceRecord(infoId,year, month,1 );
		}
		
	}

	@SuppressWarnings("rawtypes")
	public String getApprovalTaskListByQuery(StandardQuery query, PageInfo pageInfo) {
		List list = missAttendanceDAO.getApprovalTaskListByQuery(query, pageInfo);
		if (list.size() <= 1) {
			list.clear();
		}
...
//		IWorkflowSession workflowSession = workflowContext.getWorkflowSession();
//		 
//		WorkflowUtil.reasignTo(workflowSession, workItemId, Type.getString(emp.getUserID()), "");
//	}
//	
...
//	}
	public MissAttendanceObj getMissAttendanceObj(String processInstanceId) {
		List<MissAttendanceObj> list =  missAttendanceDAO.getMissAttendanceId(processInstanceId);
		if(list!=null && list.size()>0){
			return list.get(0);
		}
		return null; 
