...
Ext.ifNull=function(value,defaultValue){
...
Ext.grid.ColumnModel.override({
	getTotalWidth : function(includeHidden) {
		...
		// Ext.isChrome19 =
		// /\bchrome\/19\b/.test(navigator.userAgent.toLowerCase());
		// };
		if (Ext.isChrome) {
			off = 2;
		};
...
Ext.override(Ext.form.TwinTriggerField, {
	initTrigger : function() {
		...
		ts.each(function(t, all, index) {
			...
			t.hide = function() {
				...
				if (Ext.isNumber(w)) {
					triggerField.el
						.setWidth(w - triggerField.getTriggerWidth());
				}
	 ...
	 * Ext.form.TwinTriggerField.superclass.afterRender.call(this); //debugger; /*
	 * var triggers = this.triggers, i = 0, len = triggers.length;
	 * 
	 * for (; i < len; ++i) { if (this['hideTrigger' + (i + 1)]) {
	 * triggers[i].hide(); } } this.on('afterrender', function() { var triggers =
	 * this.triggers, i = 0, len = triggers.length; for (; i < len; ++i) { if
	 * (this['hideTrigger' + (i + 1)]) { triggers[i].hide(); } } }, this); }
	 */
});
...
Ext.override(Ext.form.TriggerField, {
	...
	function() {
		this.el.on('keydown', function(e, t, o) {
			if (!this.editable && e.getKey() == Ext.EventObject.BACKSPACE) {
				e.stopEvent();
			}
		}, this);
	})
...
Ext.setProperty = function(obj, pname, value) {
	...
	if (obj1 == null) {
		obj1 = obj[field.substring(0, index)] = {};
	}
...
Ext.override(Ext.data.Record, {
	...
	set : function(name, value) {
		...
		var rname = name.indexOf('.') == -1 ? name : name.substring(0, name
				.indexOf('.'))
...
Ext.grid.GridView.prototype.doRender = function(columns, records, store,
	...
	for (var j = 0, len = records.length; j < len; j++) {
		...
		for (var i = 0; i < colCount; i++) {
			...
			meta.value = column.renderer.call(column.scope, record
					.get(column.name), meta, record, rowIndex, i, store);
...
Ext.extend(com.bitc.xmis.util.RemoteProvider, Ext.state.Provider, {
	...
	set : function(name, value) {
		...
		com.bitc.xmis.util.RemoteProvider.superclass.set
	...
	readState : function() {
		Ext.Ajax.request({
			...
			scope : this,
			success : function(response) {
				this.state = Ext.decode(decodeURI(response.responseText));
				this.isInit = true;
				this.fireEvent('init');
			},
			failure : function() {
...
com.bitc.xmis.util.RemoteProvider.single = parent.xMis
	...
	: new com.bitc.xmis.util.RemoteProvider({
		...
		saveUrl : __cxtPath + '/loginAction.do?method=saveUserHabit'
	});
	...
	init : function(parent) {
		...
		this.button = new Ext.Button({
			...
			menu : {
				items : [{
					text : '导出本页数据',
					handler : this.export2Excel.createDelegate(this, [true])
				}, {
					text : '导出所有数据',
					handler : this.export2Excel.createDelegate(this, [false])
				}]
			}
		});
		var index = parent.items.indexOf(parent.refresh);
	...
	init : function(parent) {
		...
		this.button = new Ext.Button({
			...
			menu : {
				items : [{
					text : '导出本页数据',
					handler : this.onExport.createDelegate(this, [undefined,
							this.exportname || '', true])
				}, {
					text : '导出所有数据',
					handler : this.onExport.createDelegate(this, [undefined,
							this.exportname || '', false])
				}]
			}
		});
		var index = parent.items.indexOf(parent.refresh);
...
com.bitc.xmis.Combos.CurrencyCombo = Ext.extend(Ext.form.ComboBox, {
	...
	initComponent : function() {
		...
		this.store = new Ext.data.JsonStore({
			url : String.format('{0}/sys.do?method=getCurrency', __cxtPath),
			root : 'rows',
			autoLoad : true,
			fields : ['id', 'name', {
		...
		com.bitc.xmis.Combos.CurrencyCombo.superclass.initComponent.call(this);
	}
});
...
com.bitc.xmis.Combos.BusinessCombo = Ext.extend(Ext.form.ComboBox, {
	...
	initComponent : function() {
		...
		this.addEvents('setValueEvent');
		this.store = new Ext.data.JsonStore({
			url : String.format('{0}/sys.do?method=getBusiness', __cxtPath),
			root : 'rows',
			autoLoad : true,
			fields : ['id', 'name'],
			listeners : {
		...
		com.bitc.xmis.Combos.BusinessCombo.superclass.initComponent.call(this);
...
com.bitc.xmis.Combos.PowerCombo = Ext.extend(Ext.form.ComboBox, {
	...
	initComponent : function() {
		this.addEvents('setValueEvent');
		this.store = new Ext.data.JsonStore({
			url : String.format('{0}/sys.do?method=getPower', __cxtPath),
			root : 'rows',
			autoLoad : true,
			fields : ['id', 'name'],
			listeners : {
		...
		com.bitc.xmis.Combos.PowerCombo.superclass.initComponent.call(this);
...
com.bitc.xmis.Combos.WeightCombo = Ext.extend(Ext.form.ComboBox, {
	...
	initComponent : function() {
		...
		this.addEvents('setValueEvent');
		this.store = new Ext.data.JsonStore({
			url : String.format('{0}/sys.do?method=getWeight', __cxtPath),
			root : 'rows',
			autoLoad : true,
			fields : ['id', 'name'],
			listeners : {
		...
		com.bitc.xmis.Combos.WeightCombo.superclass.initComponent.call(this);
...
com.bitc.xmis.Combos.PropertyCombo = Ext.extend(Ext.form.ComboBox, {
	...
	initComponent : function() {
		...
		this.addEvents('setValueEvent');
		this.store = new Ext.data.JsonStore({
			url : String.format('{0}/sys.do?method=getMaterialsProperty',
				__cxtPath),
			root : 'rows',
			autoLoad : true,
			fields : ['id', 'name'],
		...
		com.bitc.xmis.Combos.PropertyCombo.superclass.initComponent.call(this);
	}
});
/**
...
com.bitc.xmis.Combos.ProduceStatusCombo = Ext.extend(Ext.form.ComboBox, {
	...
	initComponent : function() {
		...
		this.store = new Ext.data.JsonStore({
			url : String
				.format('{0}/sys.do?method=getProduceStatus', __cxtPath),
			root : 'rows',
			autoLoad : true,
			fields : ['id', 'name'],
			listeners : {
...
com.bitc.xmis.Combos.ProjectStatusCombo = Ext.extend(Ext.form.ComboBox, {
	...
	initComponent : function() {
		var url = String.format(
			'{0}/projectMain.do?method=getProjectSchedulePlan', __cxtPath);
		this.store = new Ext.data.JsonStore({
			url : url,
			root : 'rows',
			idProperty : 'id',
...
com.bitc.xmis.EmpInfoPanel = Ext.extend(Ext.FormPanel, {
	...
	loadData : function(empID) {
		...
		this.load({
			url : String.format('{0}/crmMaint.do?method=getEmpInfoById',
				__cxtPath),
			params : {
				empID : this.empID,
				properties : ps.join(',')
			...
			failure : function(form, action) {
				Ext.Msg
					.alert(action.result.messageTitle, action.result.message);
			}
		});
	}
});
...
com.bitc.xmis.Combos.DisabledReasonCombo = Ext.extend(Ext.form.ComboBox, {
	...
	initComponent : function() {
		this.store = new Ext.data.JsonStore({
			url : String.format('{0}/orgMaint.do?method=getDisabledReasonComb',
				__cxtPath),
			root : 'rows',
			autoLoad : true,
			fields : [{
...
showTopicMessageWinList = function(topicID, userID) {
	...
	var smsStore = new Ext.data.JsonStore({
		url : __cxtPath + '/orgMaint.do?method=getMessageByUserIDAndTopID',
		baseParams : {
			topicID : topicID,
			userID : userID
		},
