...
Ext.ux.form.ChooseOrgTreeField = Ext.extend(Ext.form.TwinTriggerField, {
	...
	onTrigger2Click : function() {
		...
		var treeLoader = this.treeLoader = new Ext.tree.TreeLoader({
		...
		var me = this;
		var wintbar = [new Ext.ux.form.SearchField({
			width : 140,
            value:winConfig.defaultSearchValue,
			onTrigger1Click : function() {
				this.el.dom.value = '';
				this.triggers[0].hide();
					...
					Ext.Ajax.request({
						...
						success : function(response, action) {
							...
							if ("" != nodePath && nodePath.indexOf("/") > -1) {
								nodePath=nodePath.substring(5);
								var userID = nodePath.substring(nodePath
										.lastIndexOf("/")
									+ 1);
								me.treeLoader.load(me.treepanel.getRootNode(), function() {
									me.treepanel.expandPath(nodePath, "",
										function() {
											//me.treepanel
											//	.getNodeById(userID)
											//	.select();
											var node = me.treepanel
												.getNodeById(userID);
											node.getUI().toggleCheck(true);//节点选中
										})
								});
								...
								me.treeLoader.load(me.treepanel.getRootNode(),
									function(n) {
