Ext.onReady(function() {
	...
	var chooseContractType = IncomeContractId;
	var strTips = invoiceEditFlag == true
			? '※点击\'合同名称\'等进入合同信息,点击\'已开发票金额/已报销金额\'进入票据收发列表'
			: '※点击\'合同名称\'等进入合同信息,点击\'已收/付合同款\'进入收款信息列表';
	...
	var paymentLECombo = new Ext.form.ComboBox({
				...
				editable : false,
				emptyText : '请选择合同类型',
				value : IncomeContractId,
				listeners : {
					'afterrender' : function(cmp) {
						pEStore.setBaseParam("contractTypeID", cmp.getValue());
					},
					"select" : function(ombo, record, index) {
						chooseContractType = record.get('num');
						pEStore.setBaseParam("contractTypeID", record
										.get('num'));
					}
				},
				triggerAction : 'all'
	...
	var pEStore = new Ext.data.Store({
				proxy : new Ext.data.HttpProxy({
							url : __cxtPath
									+ '/contractMain.do?method=getContractListByType'
						}),
				remoteSort : true,
				sortInfo : {
					field : Abbreviation,
	...
	pEStore.on('load', function() {
				if (paymentLECombo.getValue() == IncomeContractId) {
					...
					paymentListEditGrid.getColumnModel().setHidden(15, false);
					paymentListEditGrid.getColumnModel().setColumnHeader(
							16,
							"<font color="
					...
					paymentListEditGrid.getColumnModel().setHidden(15, true);
					paymentListEditGrid.getColumnModel().setColumnHeader(
							16,
							"<font color="
									+ Color_Font_Contract_Total_UnCharged_Amt
									+ ">未付合同款</font>");
					paymentListEditGrid.getColumnModel().setHidden(24, true);
				}
			}, this);
	 ...
	 * contractTypeInEdit =contractType; contractIdInEdit = id;
	 * paymentDetailEditStore.removeAll(true); paymentDetailEditStore.load({
	 * params:{ contractID: id }swv }); };
	 */
	...
	var paymentListEditGrid = new Ext.grid.GridPanel({
		...
		tbar : ["合同类型:", paymentLECombo, '-', {
					ref : '../conditionBtn',
					iconCls : 'condition',
					text : '条件设定',
					handler : function(b, e) {
						
						pEStore.proxy.setUrl( __cxtPath
									+ '/contractMain.do?method=getContractListByType&contractTypeID='+paymentLECombo.value);
									
						searchPanel.store = pEStore;
//						searchPanel.store.proxy.setUrl( __cxtPath
//									+ '/contractMain.do?method=getContractListByType&contractTypeID='+paymentLECombo.value);
//						pEStore.setBaseParam("contractTypeID", paymentLECombo.value);
...

				{
						...
						if (value == '' || value == null) {
							...
							if (invoiceEditFlag == true) {
								return formatCurrency(
										record.data[CurrenyGraph], value,
										'blue');
							} else {
								// return
								// formatCurrency(record.data[CurrenyGraph],
								// value, 'blue');
								
								return formatCurrency(
										record.data[CurrenyGraph],
										value + '^onClick=showPaymentEditGrid('
												+ record.data[ContractID]
												+ ',\'' + strName + '\',\''
												+ strType + '\',\''
												+ srtCurrencyG + '\',\''
					...
					renderer : function(value, cellMeta, record, rowIndex,
						...
						if (invoiceEditFlag == true) {
							...
							return formatCurrency(record.data[CurrenyGraph],
									value + '^onClick=showInvoiceEditGrid('
											+ record.data[ContractID] + ',\''
											+ strName + '\',\'' + strType
											+ '\',\'' + srtCurrencyG + '\',\''
											+ strContractAmt + '\',1,\''
							...
							return formatCurrency(record.data[CurrenyGraph],
									value, '#FF00FF');
							// return formatCurrency(record.data[CurrenyGraph],
							// value+'^onClick=showPaymentEditGrid('+record.data[ContractID]+',\''+strName+'\',\''+strType
							// + '\',\''+srtCurrencyG+'\',\''+ strContractAmt+
							// '\')', 'blue');
						}
						// return formatCurrency(record.data[CurrenyGraph],
						...
						// '\','+record.data[ContractID]+')' +
						// ' id=\'invoiceListLink_'+record.data[ContractID] +
						// '\'', Color_Font_Contract_Total_Invoiced_Amt);
						// return formatCurrency(record.data[CurrenyGraph],
						// value, '#FF00FF');
					},
...

				{
					...
					renderer : function(value, cellMeta, record, rowIndex,
						...
						return formatCurrency(record.data[CurrenyGraph], value,
								Color_Font_Contract_Total_UnAccount_Amt);
						// return record.data[invoiceTotalAmt] == "0" ?
						// formatCurrency(record.data[CurrenyGraph],
						// record.data[invoiceTotalAmt],
						// Color_Font_Contract_Total_UnAccount_Amt);
						// // formatCurrency(record.data[CurrenyGraph], value,
						// Color_Font_Contract_Total_UnAccount_Amt);
					},
					align : 'right'
				}, {
