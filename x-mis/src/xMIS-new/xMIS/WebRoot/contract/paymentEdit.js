				...
				tbar : [new Ext.ux.form.SearchField({
					...
					onTrigger2Click : function() {
						...
						if (inputName != null && '' != inputName.trim()) {
							PaymentAEmpPeasonStore.findBy(function(record){
								if(record.get('user_name') == inputName){
									PaymentAEmpPeasonGrid.getSelectionModel().clearSelections();
									PaymentAEmpPeasonGrid.getSelectionModel().selectRecords([record]);
								}
							});
