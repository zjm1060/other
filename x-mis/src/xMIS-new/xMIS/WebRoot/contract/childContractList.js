Ext.onReady(function() {
	...
	var childRecord = Ext.data.Record.create([{
				...
				name : childIncomAmtTotalByGl
			}, {
				name : childPurAmtTotalByGl
			}
	...
	var childStore = new Ext.data.Store({
		proxy : new Ext.data.HttpProxy({
					url : __cxtPath
							+ '/contractMain.do?method=getChildContractListByID'
				}),
		remoteSort : false,
		sortInfo : {
			field : childAbbreviation,
			direction : 'ASC'
		},
	...
	childStore.on('load', function(store, records, options) {
		for (var i = 0; i < records.length; i++) {
			...
			totalAmt += formatCurrency("null", record.get(childExchangeRate),
					"")
					* formatCurrency("null", record.get(childContractAmt), "");
			paymentAmt += formatCurrency("null", record.get(childExchangeRate),
					"")
					* formatCurrency("null", record.get(childTotalAmtByID), "");
			unPaymentAmt += formatCurrency("null", record
							.get(childExchangeRate), "")
					* formatCurrency("null",
							record.get(childUnChargedAmtTotal), "");
			invoicedAmt += formatCurrency("null",
					record.get(childExchangeRate), "")
					* formatCurrency("null", record.get(childInvoiceTotalAmt),
							"");
			invoicedUnAccount += formatCurrency("null", record
							.get(childExchangeRate), "")
					* formatCurrency("null", record
									.get(childInvoiceUnAccountAmt), "");
		}
		// totalCount.setText("总共" + String.format('<font
		// color="{0}">{1}</font>', "blue", store.getCount().toString())+"
		// 份追加合同", false);
		totalAmtMsg
				.setText(
						"已付约："
								...
								+ "&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;未付约："
								+ formatCurrency("", unPaymentAmt.toString(),
										'red')
								+ "&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;已开票约："
								+ formatCurrency("", invoicedAmt.toString(),
										Color_Font_Contract_Total_Invoiced_Amt)
								+ "&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;未到账约："
								+ formatCurrency("", invoicedUnAccount
												.toString(),
										Color_Font_Contract_Total_UnAccount_Amt),
						false);

		childGrid.setTitle("追加合同列表&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;共"
				+ String.format('<font color="{0}">{1}</font>', "blue", store
								.getCount().toString())
				+ " 份追加合同，合同金额合计约："
				+ formatCurrency("", totalAmt.toString(),
						Color_Font_Contract_Total_Amt));
	}, this, {
		delay : 100
	})

	var childColM = new Ext.grid.ColumnModel([new Ext.grid.RowNumberer(),

	{
		header : "合同简称",
		width : 150,
		dataIndex : childAbbreviation,
		sortable : true,
		renderer : function(value, cellMeta, record, rowIndex, columnIndex,
				store) {

			cellMeta.attr = biuldTip("", "合同名称："
							+ record.data[childContractName]);
...

	{
		header : "已收合同金额",
		width : 100,
		dataIndex : childTotalAmtByID,
		sortable : true,
		renderer : function(value, cellMeta, record, rowIndex, columnIndex,
				store) {
			return formatCurrency(record.data[childCurrenyGraph], value,
					Color_Font_Contract_Total_Charged_Amt);
		},
		align : 'right'
	},
	...
	{
		header : "已开发票金额",
		dataIndex : childInvoiceTotalAmt,
		width : 150,
		sortable : true,
		renderer : function(value, cellMeta, record, rowIndex, columnIndex,
				store) {
		...
		/*
		 * renderer: function(value, cellMeta, record, rowIndex, columnIndex,
		 * store){ return formatCurrency(record.data[childCurrenyGraph],
		 * value+'^onClick=showInvoiceList(\''+
	...
	{
		header : "已开票未到账金额",
		dataIndex : childInvoiceUnAccountAmt,
		sortable : true,
		width : 150,
		renderer : function(value, cellMeta, record, rowIndex, columnIndex,
				store) {
	...
	{
		header : "未收合同金额",
		dataIndex : childUnChargedAmtTotal,
		width : 150,
		sortable : true,
		renderer : function(value, cellMeta, record, rowIndex, columnIndex,
				store) {
	...
	{
		header : "财务来款合计",
		dataIndex : childIncomAmtTotalByGl,
		width : 150,
		sortable : true,
		renderer : function(value, cellMeta, record, rowIndex, columnIndex,
				store) {
	...
	{
		header : "采购支出合计",
		dataIndex : childPurAmtTotalByGl,
		width : 150,
		sortable : true,
		renderer : function(value, cellMeta, record, rowIndex, columnIndex,
				store) {
	...
	var childGrid = new Ext.grid.GridPanel({
			...
			new Ext.Panel({
				border : false,
				height : Ext.get('childContractList').getHeight(),
				layout : 'border',
				renderTo : "childContractList",
				items : childGrid
			});
