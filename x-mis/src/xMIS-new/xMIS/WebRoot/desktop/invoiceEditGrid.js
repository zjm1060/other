Ext.onReady(function() {
	...
	var invoiceEditRecord = Ext.data.Record.create([{
			...
			},// 需要改
			{
				...
				name : InvoiceDateDisplayed,
				type : 'date'
			}, {
				name : UpdateDateDisplayed,
				type : 'date'
			}, {
				name : UpdateID
			}, {
	...
	var invoiceEditStore = new Ext.data.GroupingStore({
				...
				reader : new Ext.data.JsonReader({
					...
					fields : invoiceEditRecord
						// invoice paymentDetailEditRecord
					})

			});
	/*
	 * paymentDetailEditStore.on('load',function(store,records,options){
	 * //alert(records.length+'==in load'); for(var i=0;i<records.length;i++){
	 * var record = records[i]; alert(record.get(paymentTransactorID)); }
	 * 
	 * },this,{delay:0})
	 */
	...
	var invoiceAEmpPeasonColM = new Ext.grid.ColumnModel([invoiceAEmpPeasonSM,
			{
				header : "部门名称",
				dataIndex : "deptName",
				sortable : true,
	...
	var invoiceAEmpPeasonStore = new Ext.data.GroupingStore({//
		...
		reader : new Ext.data.JsonReader({
					...
					fields : [{
								...
								name : EmployeeIDInInvo
							},// EmployeeIDInPaymentCharged 改
							{
								name : 'user_name'
							}]
				}),
	...
	this.showInvoiceEditGridInJS = function(id, contractName, contractType,
			curr, type, cmpId) {
		if (type == undefined) {
			type = 1;
		}
		...
		if (type == 2) {
			invoiceEditPanel.setTitle("收据收发列表" + '-' + contractName);
		} else {
			invoiceEditPanel.setTitle("发票收发列表" + '-' + contractName);
		}
	...
	var invoiceAEmpPeasonGrid = new Ext.grid.GridPanel({
				...
				enableColumnResize : false, // 设置为false可以禁止这个表格的列改变宽度 (默认值为
				// true).
				...
				tbar : [{
					...
					text : '确定',
					handler : function() {
						var record = invoiceAEmpPeasonGrid.getSelectionModel()
								.getSelected();
						...
						searchInvoiceAEmpField
								.setValue(record.get('user_name'));
						invoiceEditAEmpIDField.setValue(record
								.get(EmployeeIDInInvo));
					...
					disabled : false,
					handler : function() {
						if (invoiceAEmpPeasonWin)
							invoiceAEmpPeasonWin.hide();

						// there is an err of dom.style when window.close() on
						// (window + grid), so shuold use window.hide() to close
					}
				}
	...
	var invoiceDetailEditGrid = new Ext.grid.GridPanel({
		...
		loadMask : true, // 读取数据时的遮罩和提示功能，默认为false
		stateful : true,
		stateId : 'contract-invoiceDetail-edit-grid',
		...
		tbar : [{
			...
			handler : function() {
				...
				invoiceListEditForm.getForm().submit({
					...
					success : function(form, action) {
						 ...
						 * for(var i = 0, r; r = s[i]; i++){ stStore.remove(r); };
						 */
						invoiceDetailEditGrid.getStore().reload({
							callback : function(record, options, success) {
								if (success == true) {
			...
			handler : function() {
				...
				var winUpl;
				var strUrl = __cxtPath + '/fileUpload.do?type='
						+ (editType == 2 ? attchmentType2 : attchmentType);
				if (!winUpl) {
		...
		/*
		 * ,'-',{ ref: '../refreshBtn', iconCls: 'x-tbar-loading', text: '刷新',
		 * disabled: false, handler: function(){ paymentConditionGrid.hide(); } }
		 */
		],
		viewConfig : {
			forceFit : true
		...
		colModel : new Ext.grid.ColumnModel({
					columns : [new Ext.grid.RowNumberer(), {
								...
								header : "开票日期",
								dataIndex : InvoiceDateDisplayed,
								sortable : true,
								// renderer:customer_formatDate,
								align : 'center',
								xtype : 'datecolumn',
	...
	var downloadStoreInInvoEd = new Ext.data.Store({
		proxy : new Ext.data.HttpProxy({
			url : __cxtPath
					+ '/contractMain.do?method=getAttachedListStroeByApplicationType'// 修改成新滴
		}),
		remoteSort : true,
		sortInfo : {
			field : attachmentInvoUploadDate,
			direction : 'ASC'
		},
		reader : new Ext.data.JsonReader({
					root : 'rows',
	...
	/*
	 * downloadStoreInPayEd.load({ params:{ applicationType: attachType, refID:
	 * contractIdInEdit } });
	 */
