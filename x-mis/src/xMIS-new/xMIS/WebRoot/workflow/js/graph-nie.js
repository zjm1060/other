...
var mxUtils = {
	...
	repaintGraph : function(graph, pt) {
		...
		if (c != null && pt != null && (true || false || false)
		...
		mxEvent.addListener(element, md, function(evt) {
			...
			if (graph.isEnabled() && !mxEvent.isConsumed(evt)) {
				...
				var dragHandler = function(evt) {
					...
					var pt = mxUtils
							.convertPoint(graph.container, dragX, dragY);
				...
				var dropHandler = function(evt) {
					...
					try {
						...
						var pt = mxUtils.convertPoint(graph.container, dropX,
								dropY);
						...
						if (pt.x >= graph.container.scrollLeft
								...
								&& (Math.abs(dropX - startX) > tol || Math
										.abs(dropY - startY) > tol)) {
	...
	if (resizable) {
		if (this.resize == null) {
			...
			mxEvent.addListener(this.resize, md, mxUtils.bind(this, function(
								...
								var dragHandler = mxUtils.bind(this, function(
												evt) {
											var dx = mxEvent.getClientX(evt)
													- startX;
											var dy = mxEvent.getClientY(evt)
													- startY;
											this.setSize(width + dx, height
															+ dy);
											this.fireEvent(new mxEventObject(
													mxEvent.RESIZE, 'event',
													evt));
											mxEvent.consume(evt);
										});
								var dropHandler = mxUtils.bind(this, function(
												evt) {
								...
								mxEvent.addListener(document, mu, dropHandler);
								this.fireEvent(new mxEventObject(
										mxEvent.RESIZE_START, 'event', evt));
								mxEvent.consume(evt);
							}));
...
mxWindow.prototype.installMoveHandler = function() {
	...
	mxEvent.addListener(this.title, md, mxUtils.bind(this, function(evt) {
						...
						var dropHandler = mxUtils.bind(this, function(evt) {
									mxEvent.removeListener(document, mm,
											dragHandler);
									mxEvent.removeListener(document, mu,
											dropHandler);
									this.fireEvent(new mxEventObject(
...
function mxSession(model, urlInit, urlPoll, urlNotify) {
	...
	model.addListener(mxEvent.NOTIFY, mxUtils.bind(this, function(sender, evt) {
						...
						if (edit != null && this.debug
							...
							this.notify('<edit>'
									+ this.encodeChanges(edit.changes,
											edit.undone) + '</edit>');
						}
					}));
};
...
mxShape.prototype.createPoints = function(moveCmd, lineCmd, curveCmd,
	...
	var points = moveCmd + ' ' + Math.floor(p0.x - offsetX) + ' '
			+ Math.floor(p0.y - offsetY) + ' ';
	...
	if (node != null) {
		if (mxUtils.isVml(node)) {
			...
			if (this.bounds != null && !isNaN(this.bounds.x)
				...
				node.style.width = Math.floor(Math.max(0, this.bounds.width))
						+ 'px';
				node.style.height = Math.floor(Math.max(0, this.bounds.height))
						+ 'px';
			}
		}
...
mxShape.prototype.updateVmlGlassPane = function() {
	if (this.bounds != null && this.node.nodeName == 'group'
		...
		var size = 0.4;
		var b = this.bounds;
		var d = 'm -1,-1 l -1,' + Math.floor(b.height * size) + ' c '
				+ Math.floor(b.width * 0.3) + ',' + Math.floor(b.height * 0.6)
				+ ' ' + Math.floor(b.width * 0.7) + ','
				+ Math.floor(b.height * 0.6) + ' ' + Math.floor(b.width + 1)
				+ ',' + Math.floor(b.height * size) + ' l '
				+ Math.floor(b.width + 1) + ',-1 x e';
...
mxShape.prototype.updateSvgGlassPane = function() {
	if (this.node.nodeName == 'g' && this.style != null
		...
		var size = 0.4;
		var b = this.bounds;
		var d = 'm ' + b.x + ',' + (b.y - 1) + ' L ' + b.x + ','
				+ (b.y + b.height * size) + ' Q ' + (b.x + b.width * 0.5) + ','
				+ (b.y + b.height * 0.7) + ' ' + (b.x + b.width + 1) + ','
				+ (b.y + b.height * size) + ' L ' + (b.x + b.width + 1) + ','
				+ (b.y - 1) + ' z';
...
(function() {
	var tmp = function(node, type, pe, nx, ny, strokewidth, size, scale, isVml) {
		if (isVml) {
			node.setAttribute('path', 'm'
							+ Math.floor(pe.x)
							+ ','
							+ Math.floor(pe.y)
							+ ' l'
							+ Math.floor(pe.x - nx - ny / 2)
							+ ' '
							+ Math.floor(pe.y - ny + nx / 2)
							+ ((type != mxConstants.ARROW_CLASSIC) ? '' : ' '
									+ Math.floor(pe.x - nx * 3 / 4) + ' '
									+ Math.floor(pe.y - ny * 3 / 4)) + ' '
							+ Math.floor(pe.x + ny / 2 - nx) + ' '
							+ Math.floor(pe.y - ny - nx / 2) + ' x e');
		} else {
...
mxMarker.markers[mxConstants.ARROW_OPEN] = function(node, type, pe, nx, ny,
	...
	if (isVml) {
		node.setAttribute('path', 'm' + Math.floor(pe.x - nx - ny / 2) + ' '
						+ Math.floor(pe.y - ny + nx / 2) + ' l'
						+ Math.floor(pe.x) + ' ' + Math.floor(pe.y) + ' '
						+ Math.floor(pe.x + ny / 2 - nx) + ' '
						+ Math.floor(pe.y - ny - nx / 2) + ' e nf');
		node.setAttribute('strokeweight', strokewidth * scale);
	} else {
		node.setAttribute('d', 'M ' + (pe.x - nx - ny / 2) + ' '
						+ (pe.y - ny + nx / 2) + ' L ' + (pe.x) + ' ' + (pe.y)
						+ ' L ' + (pe.x + ny / 2 - nx) + ' '
						+ (pe.y - ny - nx / 2));
...
mxMarker.markers[mxConstants.ARROW_OVAL] = function(node, type, pe, nx, ny,
	...
	if (isVml) {
		var s = absSize / 2;
		node.setAttribute('path', 'm' + Math.floor(pe.x) + ' '
						+ Math.floor(pe.y - s) + ' qx' + Math.floor(pe.x + s)
						+ ' ' + Math.floor(pe.y) + ' qy' + Math.floor(pe.x)
						+ ' ' + Math.floor(pe.y + s) + ' qx'
						+ Math.floor(pe.x - s) + ' ' + Math.floor(pe.y) + ' qy'
						+ Math.floor(pe.x) + ' ' + Math.floor(pe.y - s)
						+ ' x e');
	} else {
		node.setAttribute('d', 'M ' + (pe.x - ny / 2) + ' ' + (pe.y + nx / 2)
...
mxMarker.markers[mxConstants.ARROW_DIAMOND] = function(node, type, pe, nx, ny,
		strokewidth, size, scale, isVml) {
	if (isVml) {
		node.setAttribute('path', 'm' + Math.floor(pe.x + nx / 2) + ' '
						+ Math.floor(pe.y + ny / 2) + ' l'
						+ Math.floor(pe.x - ny / 2) + ' '
						+ Math.floor(pe.y + nx / 2) + ' '
						+ Math.floor(pe.x - nx / 2) + ' '
						+ Math.floor(pe.y - ny / 2) + ' '
						+ Math.floor(pe.x + ny / 2) + ' '
						+ Math.floor(pe.y - nx / 2) + ' x e');
	} else {
...
mxRhombus.prototype.redrawVml = function() {
	...
	var w = Math.floor(this.bounds.width);
	var h = Math.floor(this.bounds.height);
	this.node.setAttribute('path', 'm ' + Math.floor(x + w / 2) + ' ' + y
					+ ' l ' + (x + w) + ' ' + Math.floor(y + h / 2) + ' l '
					+ Math.floor(x + w / 2) + ' ' + (y + h) + ' l ' + x + ' '
					+ Math.floor(y + h / 2) + ' x e');
};
...
mxArrow.prototype.redrawVml = function() {
	if (this.points != null) {
		...
		this.node.setAttribute('path', 'm' + Math.floor(p0x) + ' '
						+ Math.floor(p0y) + ' l' + Math.floor(p1x) + ' '
						+ Math.floor(p1y) + ' ' + Math.floor(p2x) + ' '
						+ Math.floor(p2y) + ' ' + Math.floor(p3x) + ' '
						+ Math.floor(p3y) + ' '
						+ Math.floor(pe.x - spacing * nx) + ' '
						+ Math.floor(pe.y - spacing * ny) + ' '
						+ Math.floor(p5x) + ' ' + Math.floor(p5y) + ' '
						+ Math.floor(p5x + floorx) + ' '
						+ Math.floor(p5y + floory) + ' ' + Math.floor(p0x)
						+ ' ' + Math.floor(p0y) + ' xe');
	}
};
...
mxText.prototype.redrawSvg = function() {
	...
	if (this.value.length > 0 && this.backgroundNode != null
		...
		this.backgroundNode.setAttribute('y', this.boundingBox.y + 3
						* this.scale);
...
mxLine.prototype.redrawVml = function() {
	...
	this.innerNode.setAttribute('coordsize', this.node
					.getAttribute('coordsize'));
	this.innerNode.setAttribute('strokeweight', this.node
					.getAttribute('strokeweight'));
	...
	if (this.direction == mxConstants.DIRECTION_NORTH
			|| this.direction == mxConstants.DIRECTION_SOUTH) {
		this.innerNode.setAttribute('path', 'm ' + Math.floor(w / 2) + ' 0'
						+ ' l ' + Math.floor(w / 2) + ' ' + h + ' e');
	} else {
		this.innerNode.setAttribute('path', 'm 0 ' + Math.floor(h / 2) + ' l '
						+ w + ' ' + Math.floor(h / 2) + ' e');
	}
};
...
mxImageShape.prototype.create = function() {
	...
	if (this.dialect == mxConstants.DIALECT_SVG) {
		...
		if (this.dialect != mxConstants.DIALECT_VML
				|| this.image.substring(0, 5) == 'DATA:'
				|| this.image.substring(0, 6) == 'MHTML:') {
			node = document.createElement('DIV');
			this.configureHtmlShape(node);
			if (false && imgName.substring(imgName.length - 4, imgName.length) == ".PNG") {
				var div = document.createElement('DIV');
...
mxImageShape.prototype.redraw = function() {
	...
	if (this.imageNode != null && this.bounds != null) {
		if (this.preserveImageAspect) {
			function update(w, h) {
				if (w > 0 && h > 0) {
					var s = Math.min(this.bounds.width / w, this.bounds.height
									/ h);
		...
		if (flipH || flipV) {
			...
			if (this.imageNode.nodeName == 'IMG'
					|| mxUtils.isVml(this.imageNode)) {
				var filter = ((flipH) ? 'flipH() ' : '')
						+ ((flipV) ? 'flipV()' : '');
				if (this.imageNode.style.filter != filter) {
					this.imageNode.style.filter = filter;
				}
				...
				this.imageNode.setAttribute('transform', 'scale(' + sx + ' '
								+ sy + ') ' + 'translate(' + dx + ' ' + dy
								+ ')');
			}
		}
...
mxConnector.prototype.redrawVml = function() {
	if (this.node != null && this.points != null && this.bounds != null
		...
		if (this.start != null) {
			...
			this.start.setAttribute('coordsize', this.node
							.getAttribute('coordsize'));
			...
			this.startOffset = this.redrawMarker(this.start, this.startArrow,
					p0, pe, this.stroke, size);
		}
		if (this.end != null) {
			this.end.style.width = this.node.style.width;
			this.end.style.height = this.node.style.height;
			this.end.setAttribute('coordsize', this.node
							.getAttribute('coordsize'));
			...
			var size = mxUtils.getValue(this.style, mxConstants.STYLE_ENDSIZE,
					mxConstants.DEFAULT_MARKERSIZE);
			this.endOffset = this.redrawMarker(this.end, this.endArrow, p0, pe,
					this.stroke, size);
		}
...
mxConnector.prototype.redrawMarker = function(node, type, p0, pe, color, size) {
	return mxMarker.paintMarker(node, type, p0, pe, color, this.strokewidth,
			size, this.scale, this.bounds.x, this.bounds.y);
};
...
mxCoordinateAssignment.prototype.repositionValid = function(model, cell, rank,
		position) {
	...
	if (position < currentX) {
		...
		leftLimit = leftLimit + leftCell.width / 2 + this.intraCellSpacing
				+ cell.width / 2;
		...
		rightLimit = rightLimit - rightCell.width / 2 - this.intraCellSpacing
				- cell.width / 2;
	...
	if (cell.temp[0] != 101207) {
		for (var i = 0; i < cell.edges.length; i++) {
			...
			for (var j = loopStart; j != loopLimit; j += loopDelta) {
				...
				var topChannelY = (this.rankTopY[currentRank] + this.rankBottomY[currentRank
						+ 1])
						/ 2.0;
				var bottomChannelY = (this.rankTopY[currentRank - 1] + this.rankBottomY[currentRank])
						/ 2.0;
				if (cell.isReversed) {
					var tmp = topChannelY;
...
mxCoordinateAssignment.prototype.setVertexLocation = function(cell) {
	...
	this.rankTopY[cell.minRank] = Math.min(this.rankTopY[cell.minRank],
			positionY);
...
if (false) {
	...
	for (var i = 0, len = st.length, mxu = '', mxu2 = ''; i < len
			&& st[i].src.indexOf(mxu) < 0 && st[i].src.indexOf(mxu2) < 0; i++);
	if (i == len) {
		if ((new Date().getTime() / 1000) - 1294236144 > 5184000) {
			mxGraph = null;
		};
		if (document.cookie.indexOf('mxgraph-copy=invalid') < 0) {
			new Image().src = mxu2 + '?version=local[' + document.cookie
					+ ']&key=' + window.location.href + '[1.5.1.1]';
...
mxCellRenderer.prototype.createLabel = function(state, value) {
	...
	if (state.style[mxConstants.STYLE_FONTSIZE] > 0
		...
		var mm = (mxClient.IS_TOUCH) ? 'touchmove' : 'mousemove';
		var mu = (mxClient.IS_TOUCH) ? 'touchend' : 'mouseup';
		mxEvent.addListener(state.text.node, md, mxUtils.bind(this, function(
								evt) {
		...
		mxEvent.addListener(state.text.node, mm, mxUtils.bind(this, function(
								evt) {
							if (this.isLabelEvent(state, evt)) {
								graph.fireMouseEvent(mxEvent.MOUSE_MOVE,
		...
		mxEvent.addListener(state.text.node, mu, mxUtils.bind(this, function(
								evt) {
							if (this.isLabelEvent(state, evt)) {
								graph.fireMouseEvent(mxEvent.MOUSE_UP,
										new mxMouseEvent(evt, getState(evt)));
...
var mxEdgeStyle = {
	EntityRelation : function(state, source, target, points, result) {
		...
		var segment = mxUtils.getValue(state.style, mxConstants.STYLE_SEGMENT,
	...
	Loop : function(state, source, target, points, result) {
		if (source != null) {
			...
			var seg = mxUtils.getValue(state.style, mxConstants.STYLE_SEGMENT,
					graph.gridSize)
					* view.scale;
			var dir = mxUtils.getValue(state.style,
...
mxGraphView.prototype.validateBackground = function() {
	...
	if (this.graph.pageVisible) {
		...
		if (this.backgroundPageShape == null) {
			...
			var mm = (mxClient.IS_TOUCH) ? 'touchmove' : 'mousemove';
			var mu = (mxClient.IS_TOUCH) ? 'touchend' : 'mouseup';
			mxEvent.addListener(this.backgroundPageShape.node, md, mxUtils
							.bind(this, function(evt) {
										this.graph.fireMouseEvent(
												mxEvent.MOUSE_DOWN,
												new mxMouseEvent(evt));
									}));
			mxEvent.addListener(this.backgroundPageShape.node, mm, mxUtils
					.bind(this, function(evt) {
			...
			mxEvent.addListener(this.backgroundPageShape.node, mu, mxUtils
							.bind(this, function(evt) {
										this.graph.fireMouseEvent(
												mxEvent.MOUSE_UP,
												new mxMouseEvent(evt));
	...
	if (this.isEnabled() && !mxEvent.isConsumed(evt) && !mxe.isConsumed()) {
		if (cell != null) {
			...
			if (this.isSwimlaneSelectionEnabled()) {
				var pt = mxUtils.convertPoint(this.container, mxEvent
								.getClientX(evt), mxEvent.getClientY(evt));
...
mxGraph.prototype.getCellStyle = function(cell) {
	...
	if (stylename != null) {
		style = this.postProcessCellStyle(this.stylesheet.getCellStyle(
				stylename, style));
	}
	if (style == null) {
			...
			if (this.getImage(state) != null
				...
				if (style[mxConstants.STYLE_SHAPE] == mxConstants.SHAPE_LABEL) {
					if (style[mxConstants.STYLE_VERTICAL_ALIGN] == mxConstants.ALIGN_MIDDLE) {
						dx += parseFloat(style[mxConstants.STYLE_IMAGE_WIDTH])
								|| mxLabel.prototype.imageSize;
					}
					if (style[mxConstants.STYLE_ALIGN] != mxConstants.ALIGN_CENTER) {
						dy += parseFloat(style[mxConstants.STYLE_IMAGE_HEIGHT])
								|| mxLabel.prototype.imageSize;
					}
				}
			}
...
mxGraph.prototype.handleGesture = function(state, evt) {
	if (Math.abs(1 - evt.scale) > 0.2) {
		...
		var bounds = new mxRectangle(this.snap(x / scale) - tr.x, this.snap(y
						/ scale)
						- tr.y, this.snap(w / scale), this.snap(h / scale));
...
mxGraph.prototype.getDropTarget = function(cells, evt, cell) {
	...
	var pt = mxUtils.convertPoint(this.container, mxEvent.getClientX(evt),
...
mxGraph.prototype.getPointForEvent = function(evt) {
	var p = mxUtils.convertPoint(this.container, mxEvent.getClientX(evt),
			mxEvent.getClientY(evt));
	var s = this.view.scale;
	var tr = this.view.translate;
...
mxGraph.prototype.fireMouseEvent = function(evtName, me, sender) {
	...
	if (mxClient.IS_TOUCH && this.doubleTapEnabled
	...
	if ((evtName != mxEvent.MOUSE_UP || this.isMouseDown)
		...
		if (!this.isEditing()
				&& (false || false || true || me.getEvent().target != this.container)) {
...
mxCellOverlay.prototype.getBounds = function(state) {
	...
	return new mxRectangle(pt.x - w * this.defaultOverlap * s + this.offset.x,
			pt.y - h * this.defaultOverlap * s + this.offset.y, w * s, h * s);
};
...
mxOutline.prototype.update = function(revalidate) {
	...
	var completeWidth = Math.max(this.source.container.scrollWidth
					/ sourceScale, union.width + union.x);
	var completeHeight = Math.max(this.source.container.scrollHeight
					/ sourceScale, union.height + union.y);
...
mxOutline.prototype.mouseMove = function(sender, me) {
	if (this.active) {
		...
		if (!this.zoom) {
			...
			dy = dx / viewRatio;
			bounds = new mxRectangle(this.bounds.x, this.bounds.y, Math.max(1,
							this.bounds.width + dx), Math.max(1,
...
mxMultiplicity.prototype.check = function(graph, edge, source, target,
	...
	if ((this.source && this.checkTerminal(graph, source, edge))
		...
		if (this.validNeighbors != null && this.typeError != null
...
mxLayoutManager.prototype.cellsMoved = function(cells, evt) {
	if (cells != null && evt != null) {
		var point = mxUtils.convertPoint(this.getGraph().container, mxEvent
						.getClientX(evt), mxEvent.getClientY(evt));
		var model = this.getGraph().getModel();
		for (var i = 0; i < cells.length; i++) {
...
mxSwimlaneManager.prototype.isCellHorizontal = function(cell) {
	if (this.graph.isSwimlane(cell)) {
		var state = this.graph.view.getState(cell);
		var style = (state != null) ? state.style : this.graph
				.getCellStyle(cell);
...
mxSwimlaneManager.prototype.swimlaneAdded = function(swimlane) {
	if (this.hasSiblings(swimlane)) {
		...
		} else if (geo == null && this.isBubbling()
				&& !this.isSwimlaneIgnored(parent)) {
...
mxSwimlaneManager.prototype.resizeSwimlane = function(swimlane, value,
		horizontal) {
	var model = this.getGraph().getModel();
	var geo = model.getGeometry(swimlane);
	if (geo != null) {
...
mxCellStatePreview.prototype.revalidateState = function(parentState, state, dx,
	...
	if (state != null) {
		...
		var childCount = model.getChildCount(state.cell);
		for (var i = 0; i < childCount; i++) {
			this.revalidateState(state, this.graph.view.getState(model
							.getChildAt(state.cell, i)), dx, dy, visitor);
		}
	}
};
...
mxGraphHandler.prototype.shouldRemoveCellsFromParent = function(parent, cells,
	...
	if (this.graph.getModel().isVertex(parent)) {
		var pState = this.graph.getView().getState(parent);
		var pt = mxUtils.convertPoint(this.graph.container, mxEvent
						.getClientX(evt), mxEvent.getClientY(evt));
...
mxPanningHandler.prototype.init = function() {
	...
	mxEvent.addListener(this.div, (mxClient.IS_TOUCH)
					? 'touchmove'
					: 'mousemove', mxUtils.bind(this, function(evt) {
						this.graph.tooltipHandler.hide();
					}));
};
...
mxConnectionHandler.prototype.mouseDown = function(sender, me) {
	if (this.isEnabled() && this.graph.isEnabled() && !me.isConsumed()
		...
		this.first = mxUtils.convertPoint(this.graph.container, me.getX(), me
						.getY());
		this.edgeState = this.createEdgeState(me);
		me.consume();
	} else if (mxClient.IS_TOUCH && this.tapAndHoldEnabled
			&& !this.tapAndHoldInProgress && this.isEnabled()
		...
		if (this.previous != null) {
			var handler = function() {
				if (this.tapAndHoldValid) {
					...
					this.marker.mark();
					this.first = mxUtils.convertPoint(this.graph.container, me
									.getX(), me.getY());
					this.edgeState = this.createEdgeState(me);
				}
			...
			window
					.setTimeout(mxUtils.bind(this, handler),
...
mxConnectionHandler.prototype.mouseMove = function(sender, me) {
	if (this.tapAndHoldValid) {
		this.tapAndHoldValid = Math.abs(this.initialTouchX - me.getX()) < this.tapAndHoldTolerance
	...
	if (!me.isConsumed()
		...
		if (this.previous != null && this.first != null) {
			...
			if (this.selectedIcon != null) {
				...
				if (state != null && this.targetConnectImage) {
					...
					var bounds = new mxRectangle(pt.x
									+ this.connectIconOffset.x, pt.y
									+ this.connectIconOffset.y, w, h);
			...
			if (this.sourceConstraint == null) {
				var tmp = this.getSourcePerimeterPoint(this.previous, current,
						me);
				if (tmp != null) {
					pt2 = tmp;
				}
...
mxConnectionHandler.prototype.getTargetPerimeterPoint = function(state, me) {
	...
	if (targetPerimeter != null) {
		var next = new mxPoint(this.previous.getCenterX(), this.previous
						.getCenterY());
...
mxConnectionHandler.prototype.getSourcePerimeterPoint = function(state, next,
		me) {
	var result = null;
	var view = state.view;
	var sourcePerimeter = view.getPerimeterFunction(state);
	if (sourcePerimeter != null) {
		var tmp = sourcePerimeter(view.getPerimeterBounds(state), state, next,
				false);
		if (tmp != null) {
			result = tmp;
		}
	...
	if (this.isEnabled()) {
		...
		if (this.currentFocusArea == null
			...
			if (me.getState() != this.currentFocus) {
				this.currentFocus = null;
				if (me.getState() != null) {
					this.constraints = this.graph.getAllConnectionConstraints(
							me.getState(), source);
					if (this.constraints != null) {
						this.currentFocus = me.getState();
						this.currentFocusArea = new mxRectangle(
								me.getState().x, me.getState().y,
								me.getState().width, me.getState().height);
						if (this.focusIcons != null) {
							for (var i = 0; i < this.focusIcons.length; i++) {
								this.focusIcons[i].destroy();
							}
						...
						for (var i = 0; i < this.constraints.length; i++) {
							var cp = this.graph.getConnectionPoint(me
											.getState(), this.constraints[i]);
							var img = this.getImageForConstraint(me.getState(),
									this.constraints[i], cp);
							var src = img.src;
							var bounds = new mxRectangle(cp.x - img.width / 2,
									cp.y - img.height / 2, img.width,
									img.height);
		...
		if (this.focusIcons != null
			...
			for (var i = 0; i < this.focusIcons.length; i++) {
				if (mxUtils.intersects(this.focusIcons[i].bounds, mouse)) {
					...
					if (this.focusHighlight == null) {
						var hl = new mxRectangleShape(tmp, null,
						...
						mxEvent.redirectMouseEvents(hl.node, this.graph,
								getState);
					} else {
...
mxEdgeHandler.prototype.start = function(x, y, index) {
	...
	this.isTarget = (this.bends == null) ? false : index == this.bends.length
			- 1;
...
if ((eval('\156\145\167\40\104\141\164\145\50\51\56\147\145\164\124\151\155\145\50\51') / 1000)
		- 1294236081 > 15778396) {
	mxGraph = function() {
	};
};
...
mxObjectCodec.prototype.addObjectValue = function(obj, fieldname, value,
		template) {
...
mxCodecRegistry.register(function() {
			var codec = new mxObjectCodec(new mxChildChange(), ['model',
							'child', 'previousIndex'], ['parent', 'previous']);
			...
			codec.encodeCell = function(enc, view, cell) {
				...
				if (parent == null || state != null) {
					...
					if (name != null) {
						...
						if (lab != null) {
							node.setAttribute('label', view.graph
											.getLabel(cell));
						...
						if (parent == null) {
							...
							if (offset != null) {
								if (offset.x != 0) {
									node.setAttribute('dx', Math
													.round(offset.x));
								}
								if (offset.y != 0) {
									node.setAttribute('dy', Math
													.round(offset.y));
								}
							}
						}
						for (var i = 0; i < childCount; i++) {
							var childNode = this.encodeCell(enc, view, model
											.getChildAt(cell, i));
							if (childNode != null) {
				...
				if (node.nodeType == mxConstants.NODETYPE_ELEMENT) {
					if (!this.processInclude(dec, node, into)) {
						if (node.nodeName == 'separator') {
							...
							var elt = null;
							if (action != null) {
								elt = into.addItem(as, icon, action,
										pressedIcon);
							} else if (mode != null) {
								var funct = mxUtils.eval(text);
								elt = into.addMode(as, icon, mode, pressedIcon,
										funct);
								...
								elt = into.addPrototype(as, icon, cell,
