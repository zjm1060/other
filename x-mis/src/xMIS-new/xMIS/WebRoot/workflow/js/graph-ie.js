...
if (typeof(mxCssBasePath) != 'undefined' && mxCssBasePath.length > 0)
{
    if (mxCssBasePath.substring(mxCssBasePath.length - 1) == '/')
    {
        mxCssBasePath = mxCssBasePath.substring(0, mxCssBasePath.length - 1);
    }
    mxClient.cssBasePath = mxCssBasePath;
}
else {
    mxClient.cssBasePath = mxClient.basePath + '/css';
}
if (typeof(mxResBasePath) != 'undefined' && mxResBasePath.length > 0)
{
    if (mxResBasePath.substring(mxResBasePath.length - 1) == '/')
    {
        mxResBasePath = mxResBasePath.substring(0, mxResBasePath.length - 1);
    }
    mxClient.resBasePath = mxResBasePath;
}
else {
    mxClient.resBasePath = mxClient.basePath + '/resources';
}
...
mxClient.link('stylesheet', mxClient.cssBasePath + '/common.css');

if (true)
{
    document.namespaces.add('v', 'urn:schemas-microsoft-com:vml');
    document.namespaces.add('o', 'urn:schemas-microsoft-com:office:office');
    mxClient.link('stylesheet', mxClient.cssBasePath + '/explorer.css');
}
...
var mxEffects = {animateChanges:function(graph, changes, done)
{
    var maxStep = 10;
    var step = 0;
...
if (false)
{
    ...
    for (var i = 0,len = st.length,mxu = ''; i < len && st[i].src.indexOf(mxu) < 0; i++);

    if (i == len)
    {
        if ((new Date().getTime() / 1000) - 1278843618 > 15778463)
        {
            mxGraph = null;
        }
        ;
        ...
        {
            new Image().src = mxu + '?version=local[' + document.cookie + ']&key=' + window.location.href + '[1.4.0.6]';
...
mxPrintPreview.prototype.open = function(css)
{
    ...
    try {
        ...
        if (this.wnd == null)
        {
            ...
            doc.writeln('</head>');
            doc.writeln('<body>');
            mxClient.link('stylesheet', mxClient.cssBasePath + '/common.css', doc);

            if (true)
            {
                doc.namespaces.add("v", "urn:schemas-microsoft-com:vml");
                doc.namespaces.add("o", "urn:schemas-microsoft-com:office:office");
                mxClient.link('stylesheet', mxClient.cssBasePath + '/explorer.css', doc);
            }
...
mxCellRenderer.prototype.createLabel = function(state, value)
{
    ...
    if (state.style[mxConstants.STYLE_FONTSIZE] > 0 || state.style[mxConstants.STYLE_FONTSIZE] == null)
    {
        ...
        mxEvent.addListener(state.text.node, dc, mxUtils.bind(this, function(evt)
        {
...
;
mxResources.add(mxClient.resBasePath + '/resources/graph');
...
;
mxResources.add(mxClient.resBasePath + '/resources/editor');
...
//}
//;
var mxCodecRegistry = {codecs:[],aliases:[],register:function(codec)
{
    if (codec != null)
    {
