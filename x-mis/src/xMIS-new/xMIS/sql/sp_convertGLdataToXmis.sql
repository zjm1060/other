...
AS 
    BEGIN
        ...
        --DELETE xmis.fas_income WHERE year = @year
        ...
        FROM    xmis.fas_GL_accvouch ( INDEX=i_GLaccvouch_yiicc ) ,
                xmis.fas_codeType
        WHERE   --xmis.fas_GL_accvouch.year = @year
                --AND 
                xmis.fas_GL_accvouch.iperiod > 0
        ...
        INSERT  INTO xmis.fas_qch
                ...
                SELECT  citem_id ,
                        CASE WHEN cbegind_c = '借' THEN -1 * mb
                             ELSE 0
                        END AS income_mb
                FROM    xmis.fas_GL_accass ,
                        xmis.fas_codeType
                WHERE   xmis.fas_GL_accass.year = 2008
                        AND iperiod = 1
                        AND citem_class = 00
                        AND xmis.fas_codeType.type = 0
                        AND xmis.fas_codeType.year = 2011 --系统公式适用年限从2011年开始，期初值是从2008年开始
                        AND xmis.fas_GL_accass.ccode = xmis.fas_codeType.ccode
                UNION ALL
                SELECT  citem_id ,
                        CASE WHEN cbegind_c = '贷' THEN mb
                             ELSE 0
                        END
                FROM    xmis.fas_GL_accass ,
                        xmis.fas_codeType
        ...
        UNION ALL
        SELECT  project_code ,
                SUM(adjusted_mb) AS adjusted_mb
        FROM    xmis.fas_项目金额调整表 adjusted ,
                xmis.fas_fitemss00
        ...
        */
        ...
        --删除在调整表里存在的项目的财务数据明细
        DELETE  #glTotal
        WHERE   project_code IN ( SELECT  DISTINCT
                                            project_code
                                  FROM      xmis.fas_项目金额调整表 adjusted ,
                                            xmis.fas_fitemss00
                                  WHERE     xmis.fas_fitemss00.year = 2010
                                            AND adjusted.citem_id = citemcode )
                                            
		--将调整表的项目的财务最后结余数据导入，则该项目没有财务明细数据
		INSERT INTO #glTotal
        SELECT  project_code ,
                SUM(adjusted_mb) AS adjusted_mb
        FROM    xmis.fas_项目金额调整表 adjusted ,
