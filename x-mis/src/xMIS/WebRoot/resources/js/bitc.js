...
function limitCalculate(sourceFieldName, spanFieldName, limitFieldName, unitFieldName, hiddenFileName){
	...
	result = (sourceObj.value.trim() == "" || limitObj.value.trim() == "") ? "" : addDate(type, limitObj.value, sourceObj.value);
 ...
 function addDate(type,NumDay,vdate){
        ...
        date.setFullYear(parseInt(vdate.substring(0,4),10),parseInt(vdate.substring(5,7),10)-1,parseInt(vdate.substring(8,10),10));
  ...
  function convertDateObjToFormatStr(dateValue){
    ...
    return dateValue==""?"":dateValue.getYear() +'-' + ((dateValue.getMonth()+1)<10?'0'+(dateValue.getMonth()+1):(dateValue.getMonth()+1)) + '-' +(dateValue.getDate()<10?'0'+dateValue.getDate():dateValue.getDate());
...
//  	alert(new Date(Date.parse(endDate)).getTime()-new Date(Date.parse(begDate)).getTime());
  	
  	if(new Date(Date.parse(endDate)).getTime()< new Date(Date.parse(begDate)).getTime())
  		return false;
  	else
  		return true;
...
　　}    	
	...
	function biuldTip(title,val)	{
		...
		return (val.indexOf("^") > 0)?extTip+val.substring(0, val.indexOf("^"))+"'":extTip+val+"'";
	}
...
//		            v =  "\u00A5 " + v
				...
				if(color=="")
		        	...
		        	return urlFlag?"<a href=" + url + " " + onclickStr + ">" + fontColor+ v + "</font></a>":fontColor+v+"</font>";
...
//		alert(val);
		...
		val = convertNameWithoutLinker(val)+'^onClick=showContractWin('+contractID+')';
	...
	function setWarningBackgroudColor(warningDate, warningAmt, warningDays, currenyGraph){
		...
		var tipMessage = "预计收款日期："+customer_formatDate(warningDate)	+"<br/>预计收款金额："	+ formatCurrency(currenyGraph, warningAmt, "");

		//var extTip = colorStr==""?"":biuldTip(tipTitle, tipMessage);
		
		return colorStr==""?biuldTip(tipTitle, tipMessage):"style=background-color:" + colorStr +" "+ biuldTip(tipTitle, tipMessage);
