...
function Validate_Input()
{
	...
	for (i=0;i<l_intLen;i++)
	{
		...
		if (l_obj.tagName.toLowerCase() == 'input'){
			if (l_obj.type.toLowerCase()=='checkbox' ||l_obj.type.toLowerCase()=='radio'){
				if ( l_obj.cannull !=null && l_obj.cannull == "0" ){
					var oTmp = document.all[l_obj.name];
					var iAryLen = oTmp.length;
					if ( iAryLen == null )
					{
						...
						var bTmp = false;
						for ( iTmp=0;iTmp<iAryLen;iTmp++ )
						{
							bTmp = oTmp[iTmp].checked;
							if ( bTmp == true )
								break;
						}
						if ( bTmp != true )
...
function CheckAction(TipsPagePath)
{
	var l_return;
	l_return = Validate_Input();
	if (l_return != ""){
		AlertWindow(TipsPagePath,l_return);
		return false;
	}
	EncodeInput();
	return true;
...
function CheckCanSave(TipsPagePath)
{
	var l_return;
	l_return = Validate_CanSave();
	if (l_return != ""){
		AlertWindow(TipsPagePath,l_return);
		return false;
	}
