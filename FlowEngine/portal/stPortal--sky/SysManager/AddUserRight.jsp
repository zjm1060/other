...
<%@ page language="java" import="portalproj.*,java.util.*,java.sql.*,suntek.portal.publicLib.RecordSetObject,jbportal.PortalUtil" %>
...
<%
    ...
    m_rsAll=oManager.getUserDeptGroupInfo(portalproj.sysManager.DEPARTMENT);
    try
      {
    ...
    finfo.setSELECTNAME("our_select");
    finfo.setMODUTYPE("type");
    m_rsAll=oManager.getUserDeptGroupInfo(portalproj.sysManager.GROUP);
    try
      {
    ...
    finfo.setSELECTNAME("our_select");
    finfo.setMODUTYPE("type");
    m_rsAll=oManager.getUserDeptGroupInfo(portalproj.sysManager.USER);
    try
      {
        m_sMenuString += PortalUtil.FillClass(m_rsAll,"1",finfo);
      }
      catch(Exception e)
      {
         throw e;
...
function tNodeOnClick()
{
	...
	SetDBFocus(strNodeID);
        if (strNodeID!="" && strNodeID!="2^undefined^用户组列表"&&strNodeID!="0^undefined^部门列表"&&strNodeID!="1^undefined^用户列表"&&strNodeID!="-1^undefined^列表"){
...
*/		l_return +="&nbsp;</td>\n";
	...
	l_return += " onclick=\"javascript:oTree.Find('" + this.nodeName + "').OnClick();this.blur();\"";
