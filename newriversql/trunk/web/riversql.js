...
var databasesReader = new Ext.data.JsonReader({
    ...
    id: 'id'
}, ['id', 'iconurl', 'name', 'hasCatalogs', 'catalogs', 'catalog', 'autocommit', 'schemas']);
                    ...
                    handler: function () {
                        ...
                        Ext.Ajax.request({
                            ...
                            params: {
                                ...
                                data: Ext.util.JSON.encode(myData),
                                info: Ext.util.JSON.encode(info2)
                            }
                        });
                    }
                });
...
function createTaskShow() {
    var tabFolder = Ext.getCmp('tabpanelpages');
    var taskshowpage = new Ext.Panel({
        layout: 'border',
        autoScroll: true,
        closable: true,
        title: "<img src='icons/cd_edit.png' style='vertical-align:bottom;height:16px;width:16px' />&nbsp;ShowTask"
    });
    ...
    taskshowpage.add(grid);
    tabFolder.add(taskshowpage);
    tabFolder.setActiveTab(taskshowpage);
}
var taskInfoWin = new Ext.Window({
    title: 'taskInfo',
    id: 'taskInfoWin',
    height: 150,
    width: 300,
        ...
        buttons: [{
            ...
            handler: function () {
               ...
               var taskName = Ext.getCmp('taskName').getValue();
                var taskTrigger = Ext.getCmp('taskTrigger').getValue();
                Ext.Ajax.request({
                    url: 'do?action=task&method=saveTaskInfo',
                    success: function(){
                        taskInfoWin.hide();
                        storeTask.reload();
                    },
...
var storeTask = new Ext.data.Store({
    proxy: new Ext.data.HttpProxy({
        url: 'do?action=task&method=getTasks',
        method: 'POST'
    }),
    reader: new Ext.data.JsonReader({
        root: 'result.tasks'
    }, ['id', 'name', 'cronTrigger', 'state']),
    autoLoad: true
});
var cmTask = new Ext.grid.ColumnModel([{
    header: 'id',
    align: 'center',
    dataIndex: 'id',
    hidden: true,
    ...
    renderer: function (value) {
        if (value == 1) {
            return '<font color="green">Running</color>'
        } else {
            return '<font color="red">Stop</color>'
        }
    }
}, {
    header: '',
    ...
    width:70,
    renderer: function (value) {
        return '<a href="javascript:;" onclick="showTask(\'' + value + '\')">Detail</a>';
    }
}, {
    header: '',
    align: 'center',
    ...
    width:70,
    renderer: function (value) {
        return '<a href="javascript:;" onclick="modifyTask(\'' + value + '\')">Modify</a>';
    }
}, {
    header: '',
    align: 'center',
    ...
    width:70,
    renderer: function (value) {
        return '<a href="javascript:;" onclick="deleteTask(\'' + value + '\')">Delete</a>';
    }
},{
    header:'',
    align:'center',
    ...
    width:70,
    renderer: function (value) {
            return '<a href="javascript:;" onclick="startTask(\'' + value + '\')">Start</a>';
    }
},{
    header:'',
    align:'center',
    ...
    width:70,
    renderer: function (value) {
        return '<a href="javascript:;" onclick="stopTask(\'' + value + '\')">Stop</a>';
    }
}]);
function startTask(value){
    Ext.Ajax.request({
        url: 'do?action=task&method=startTask',
        success: function(){
...
function stopTask(value){
    Ext.Ajax.request({
        url: 'do?action=task&method=stopTask',
        success: function(){
            Ext.getCmp('taskGrid').getStore().reload();
        },
        params: {
...
function modifyTask(value) {
    taskInfoWin.show();
    Ext.getCmp('taskGrid').getStore().findBy(function(record){
        if(record.get('id') == value){
            Ext.getCmp('taskId').setValue(record.get('id'));
            Ext.getCmp('taskName').setValue(record.get('name'));
            Ext.getCmp('taskTrigger').setValue(record.get('cronTrigger'));
        }
    });
}
function showTask(taskId) {
    var commandStore = new Ext.data.Store({
        proxy: new Ext.data.HttpProxy({
            url: 'do?action=task&method=getTaskDetail&taskId=' + taskId,
            method: 'POST'
        }),
        reader: new Ext.data.JsonReader({
            root: 'result.taskDetail'
        }, ['id', 'className', 'config', 'sortNo']),
        autoLoad: true
    });
    new Ext.Window({
        title: 'Command List',
        ...
        items: [{
            ...
            columns: [{
                ...
                dataIndex: 'id',
                renderer: function (value) {
                    return '<a href="javascript:;" onclick="showCommand(\'' + value + '\')">config</a>';
                }
            }, {
                header: '',
                align: 'center',
                dataIndex: 'id',
                renderer: function (value) {
                    return '<a href="javascript:;" onclick="modifyCommand(\'' + taskId + '\',\'' + value + '\')">modify</a>';
                }
            }, {
                header: '',
                align: 'center',
                dataIndex: 'id',
                renderer: function (value) {
                    return '<a href="javascript:;" onclick="deleteCommand(\'' + value + '\')">delete</a>';
                }
            }],
            store: commandStore
        }],
        fbar: [{
            ...
            handler: function () {
                commandInfoWin.show();
                Ext.getCmp('commandForm').getForm().reset();
                Ext.getCmp('command_taskId').setValue(taskId);
...
function deleteTask(value){
    Ext.Ajax.request({
        url: 'do?action=task&method=deleteTask',
        success: function(){
            taskInfoWin.hide();
            storeTask.reload();
        },
...
function deleteCommand(value){
    Ext.Ajax.request({
        url: 'do?action=task&method=deleteCommand',
        success: function(){
            Ext.getCmp('commandGrid').getStore().reload();
        },
        params: {
            commandId:value
        }
...
function showCommand(commandId) {
    ...
    Ext.getCmp('config_textarea').setValue('');
    Ext.getCmp('config_commandId').setValue(commandId);
    Ext.Ajax.request({
        url: 'do?action=task&method=getCommandConfig',
        success: function(response,opts){
            commandConfigWin.show();
            var msg = Ext.decode(response.responseText);
            Ext.getCmp('config_textarea').setValue(msg.result.commandConfig);
        },
        params: {
            commandId:commandId
        }
...
function modifyCommand(taskId,commandId) {
    commandInfoWin.show();
    Ext.getCmp('commandGrid').getStore().findBy(function(record){
        if(record.get('id') == commandId){
            Ext.getCmp('command_taskId').setValue(taskId);
            Ext.getCmp('command_commandId').setValue(commandId);
            Ext.getCmp('commandClassName').setValue(record.get('className'));
            Ext.getCmp('SortNo').setValue(record.get('sortNo'));
        }
    });
}
var commandInfoWin =  new Ext.Window({
        title:'Commad Detail',
        id:'commandInfoWin',
        height: 150,
        width: 300,
        ...
        items: [{
            ...
            buttons: [{
                text: 'Save',
                handler: function () {
                    var taskId = Ext.getCmp('command_taskId').getValue();
                    var commandId = Ext.getCmp('command_commandId').getValue();
                    var commandClassName = Ext.getCmp('commandClassName').getValue();
                    var SortNo = Ext.getCmp('SortNo').getValue();
                    Ext.Ajax.request({
                        url: 'do?action=task&method=saveCommand',
                        success: function(){
                            commandInfoWin.hide();
                            Ext.getCmp('commandGrid').getStore().reload();
                        },
                ...
                text: 'Close',
                handler: function () {
                    Ext.getCmp('commandInfoWin').hide();
                }
            }]
        }]

    });
var commandConfigWin =  new Ext.Window({
    ...
    items: [{
        ...
        region:'center',
        title: 'config',
        emptyText: 'please enter configuration ...'

    }],
    fbar: [{
        text: 'Save',
        handler: function () {
            var config_textarea = Ext.getCmp('config_textarea').getValue();
            Ext.Ajax.request({
                url: 'do?action=task&method=saveCommandConfig',
                success: function(){
                    commandConfigWin.hide();
                    Ext.getCmp('commandGrid').getStore().reload();
                },
                params: {
                    commandId:Ext.getCmp('config_commandId').getValue(),
                    commandCofig:Ext.getCmp('config_textarea').getValue()
                }
            });
        }
    }, {
...
function ShowImportLog() {
    ...
    var taskshowpage = new Ext.Panel({
        ...
        autoScroll: true,
        closable: true,
        title: "<img src='icons/cd_edit.png' style='vertical-align:bottom;height:16px;width:16px' />&nbsp;ShowImportLog"
    });
    var grid = new Ext.grid.GridPanel({
        border: false,
        // width : 980,
        ...
        store: new Ext.data.Store({
            proxy: new Ext.data.HttpProxy({
                url: 'do?action=task&method=getImportLog',
                method: 'POST'
            }),
            reader: new Ext.data.JsonReader({
                root: 'result.importLog'
            }, ['id', 'fileName', 'tableNames', 'state','logDate']),
            autoLoad: true
        }),
        cm: cmImportLog
    });
...
var cmImportLog = new Ext.grid.ColumnModel([{
    header: 'id',
    align: 'center',
    dataIndex: 'id',
    hidden: true,
    ...
    renderer: function (value) {
        if (value == 1) {
            return '<font color="green">Success</color>'
        } else {
            return '<font color="red">Defail</color>'
        }
    }
}, {
    header: 'logDate',
...
function createCustomizeExport() {
    ...
    var importpage = new Ext.Panel({
        autoScroll: true,
        closable: true,
        title: "<img src='icons/cd_edit.png' style='vertical-align:bottom;height:16px;width:16px' />&nbsp;Export"
    });
    ...
    connectionCombo.on('select', function () {
        ...
        if (catalogs == '') {
            type = 'schema';
            catalogs = selectedConnection.get('schemas');
        }
        var rta = [];
        for (var i = 0; i < catalogs.length; i++) {
            var rta2 = [];
            rta2.push(catalogs[i], type);
            rta.push(rta2);
        }
    ...
    databaseCombo.on('select', function (combo, record) {
        var tableComboReader = new Ext.data.JsonReader({
            root: 'result.tables'
        }, ['tablename', 'type']);
        var tableComboProxy = new Ext.data.HttpProxy({
            url: 'request?class=DatabaseInfo&method=getTables&id=' + connectionCombo.getValue() + '&' + (record.get('type')) + 'Name=' + databaseCombo.getValue(),
            method: "POST"
        });
        ...
        function loadTablesSuccessful() {
            var rta = [];
            for (var i = 0; i < tableComboDataStore.getCount(); i++) {
                var rta2 = [];
                rta2.push(tableComboDataStore.getAt(i).get('tablename'), tableComboDataStore.getAt(i).get('type'));
                rta.push(rta2);
            }
    ...
    tableCombo.on('select', function (combo, record) {
        var columnSelectorReader = new Ext.data.JsonReader({
            root: 'result.columns'
        }, ['cname']);

        var columnSelectorProxy = new Ext.data.HttpProxy({
            url: 'request?class=DatabaseInfo&method=getColumns&id=' + connectionCombo.getValue() + '&' + record.get('type') + 'Name=' + databaseCombo.getValue() + '&tableName=' + tableCombo.getValue(),
            method: "POST"
        });
        ...
        function loadColumnsSuccessful() {
            ...
            pkColumnStore.loadData(rta, false);
            Ext.getCmp("columnSelector").fromStore.loadData(rta, false);
            Ext.getCmp("columnBlobSelector").fromStore.loadData(rta, false);
            Ext.getCmp("columnSelector").toStore.removeAll();
            Ext.getCmp("columnBlobSelector").toStore.removeAll();
        }

        columnSelectorDataStore.load();

    });
    var pkColumnStore = new Ext.data.ArrayStore({
        data: [[]],
        fields: ['column']
    });
    var pkColumnCombo = new Ext.form.ComboBox({
        store: pkColumnStore,
        fieldLabel: 'PK',
        displayField: 'column',
        valueField: 'column',
    ...
    var columnBlobDef = new Ext.data.Record.create([{
        name: 'cname',
        type: 'string'
    }]);

    var columnBlobStore = new Ext.data.ArrayStore({
        data: [[]],
        fields: columnBlobDef,
        sortInfo: {
            field: 'cname',
            direction: 'ASC'
        }
    ...
    var columnBlobSelector = new Ext.ux.ItemSelector({
        name: 'blobcolumns',
        id: 'columnBlobSelector',
        fieldLabel: 'BlobColumns',
        dataFields: ['cname'],
        toData: [[]],
        msWidth: 150,
        msHeight: 200,
        ...
        toLegend: 'BlobSelected',
        fromLegend: 'BlobAvailable',
        fromStore: columnBlobStore
    });
    ...
    var exportFormPanel = new Ext.FormPanel({
        ...
        items: [/*{
         ...
         }, */connectionCombo, databaseCombo, tableCombo, pkColumnCombo, columnSelector, columnBlobSelector,
            {
                xtype: 'textarea',
                fieldLabel: 'Where Condition',
                anchor: '90%',
                name: 'where'
            }],
        buttons: [{
                ...
                if (exportFormPanel.getForm().isValid()) {
                    exportFormPanel.getForm().submit({
                        ...
                        success: function (basicForm, action) {
                            window.open('do?action=tempFileDownload&fileId=' + action.result.result.fileId + '&fileName=' + action.result.result.fileName);
                        }
                    });
                }
            }
...
function createCustomizeImport() {
    ...
    var importpage = new Ext.Panel({
        autoScroll: true,
        closable: true,
        title: "<img src='icons/cd_edit.png' style='vertical-align:bottom;height:16px;width:16px' />&nbsp;Import"
    });

    var uploadstep1 = new Ext.ux.form.FileUploadField({
        allowBlank: false,
        id: 'uploadstep1',
        emptyText: 'Select a Exchange File',
        width: 300,
        fieldLabel: 'Exchange File',
        name: 'csvfile',
        buttonText: '...',
        buttonCfg: {
            iconCls: 'upload-icon'
            ...
            handler: function () {
                if (fileUploadPanel1.getForm().isValid()) {
                    fileUploadPanel1.getForm().submit({
                        url: 'do?action=importstep1',
                        waitMsg: 'Uploading your file...',
                        success: function (form, action) {
                            var index = 0;
                            var tableName;
                            var tableRow;
                            filename = action.result.result.filename;
                            fileUploadPanel2.getForm().findField('fileName').setValue(action.result.result.fileName);
                            for(var i=0;i<action.result.result.tableCount;i++) {
                                     tableName = new Ext.form.TextField({
                                        fieldLabel: 'Table Name',
                                    }).setValue(action.result.result.tables[i]['name']);
                                     tableRow = new Ext.form.TextField({
                                        fieldLabel: 'Table Rows',
                                    }).setValue(action.result.result.tables[i]['count']);
                                fileUploadPanel2.items.add(index, new Ext.Panel({
                                    border:false,
                                    columnWidth : .5,
                                    layout : 'form',
                                ...
                                fileUploadPanel2.items.add(index+1, new Ext.Panel({
                                    border:false,
                                    columnWidth : .5,
                                    layout : 'form',
                                    bodyStyle:'margin:10px 10px 0 10px',
    ...
    var fileUploadPanel2 = new Ext.FormPanel({
        ...
        items: [
            ...
            name: 'fileName'
                ...
                columnWidth : 0.55,
                layout : 'form',
                id:'tableColumnsPanel',
                items:[ {
                    xtype: 'radiogroup',
                    fieldLabel: 'Record exists',
                    items: [{
                        boxLabel: 'Replace',
                        name: 'strategy',
                        inputValue: 'replace'
            ...
            handler: function () {
                if (fileUploadPanel2.getForm().isValid()) {
                    fileUploadPanel2.getForm().submit({
                        url: 'do?action=importstep2',
                        waitMsg: 'Importing...',
                        success: function (basicForm, action) {
                            Ext.Msg.alert('Info', 'import success!');
                        }
                    });
                }
            }
