	...
	{	
		...
		if( HostNewStat==CUR_WS_IN_HOST)
		{
			...
			{	
				...
				{
					DisconnectHDB(&mysql_ctx_rw);
				}
				MySleep(2000);
				HdbConn=FALSE;
				if( ConnectHDB(&mysql_ctx_rw,strUsername,strPassword,strDbname,strHostToken)!=TRUE )
				{ 
			...
			oldhour!=jh_time_cur.hour || shm_hdb_addr->readpt_real == 1)
		{
			...
			GenInfoTable();
			if(SelectIdFromHdb(&mysql_ctx_rw)== FALSE)
			{
			...
			if(SelectStatPtFromHdb(&mysql_ctx_rw)== FALSE)
			{
...
{	
	...
	{		
		...
		{	
			...
			{	
				if(InsertHdbByYcStatus( &mysql_ctx_rw, yc[i].id, yc[i].TimeTag, yc[i].status ) !=TRUE)
				{
					HdbConn=FALSE; 	
					return;
				}
		...
		if(n >= 0 )
		{
			if( UpdateHdbByYc_A(&mysql_ctx_rw, yc_all, n) != TRUE)
			{
				HdbConn_ctx=FALSE; 	
				return;	
			}
...
{ 	
	...
	{		
		...
		if(n >= 0 )
		{
			if( UpdateHdbByYx_A(&mysql_ctx_rw, yx_all_update, n) != TRUE)
			{
				HdbConn_ctx=FALSE; 	
				return;	
			}
...
{	
	...
	{		
		...
		{	
			...
			{
				n++;
				if(UpdateHdbByChara( &mysql_ctx_rw, chara[i].id, chara[i].TimeTag,
	...
	{		
		...
		{	
			if( yxstat[i].ischg==1 ) 
			{
				...
				n++;
				if(UpdateHdbByStat( &mysql_ctx_rw, &(yxstat[i])) !=TRUE)
				{
					yxstat[i].ischg = 0;
					HdbConn=FALSE; 	
					return;
...
{ 	
	...
	{	
		if(InsertHdbByJh( &mysql_ctx_rw, i, jh[i].num) != TRUE)
		{
			HdbConn=FALSE;
			return;
		}
		...
		{
			if(UpdateHdbByJh( &mysql_ctx_rw, i, j, jh[i].value[0][j], jh[i].value[1][j],
				jh[i].value[2][j]) != TRUE)
			{
				HdbConn=FALSE;
				return;
	...
	{		
		...
		{	
			...
			{	
				...
				if (shm_ots_addr->hdb_write_flag == 1)
				{
					if(InsertHdbAlarm( &mysql_ctx_rw, al[i].TimeTag, id, al[i].status, 
					...
					CvtAlToRec(	&seg,al[i]);			
					if(InsertHdbRecord( &mysql_ctx_rw, seg.user_id,seg.host_id,seg.groups_id,seg.datetime,seg.ms, 
	...
	if(SmsChg==1)
	{
		...
		{	
			...
			{	
				...
				if (shm_ots_addr->hdb_write_flag == 1)
				{
					if(InsertHdbSmsAlarm( &mysql_ctx_rw, sms[i].TimeTag, sms[i].message,
						sms[i].type_id) != TRUE)
					{
						HdbConn=FALSE;
						return;
	...
	{		
		...
		for(i=0;i<YxNum;i++)
		{
			...
			{	
				...
				memcpy(&status,&(yx[i].status),2);
				if( InsertHdbAlarm( &mysql_ctx_rw, yx[i].TimeTag,id, 
...
{ 	
	...
	{	
		...
		/*	if(InsertHdbAlarm( &mysql_ctx_rw, yx[i].ReadTime, id, 
	...
	if(n > 0 )
	{
		if( InsertHdbAlarm_A(&mysql_ctx_rw, yx_fix, n) != TRUE)
		{
			HdbConn_ctx=FALSE; 	
			return;	
		}
...
void InsertYxInHDB()
{
	...
	if( YxChg==1 )
	{
		...
		{	
			...
			{	
				...
				*/
				...
				/*if(InsertHdbByYx( &mysql_ctx_rw, yx[i].ReadTime, yx[i].id, 
					state, yx[i].ms, -1, DPS_IND_ALM) != TRUE)
				{
					HdbConn=FALSE;
					return;
				}
		...
		if(n > 0 )
		{
			if( InsertHdbByYx_A(&mysql_ctx_rw, yx_all_insert, n) != TRUE)
			{
				HdbConn_ctx=FALSE; 	
				return;	
			}
	...
	for(k=0;k<2;k++)
	{
...

		{
			...
			case	DPS_SM_ALM:
				...
				memcpy(sms[j].message,rec.sms_rec.message,SMS_MESSAGE_SIZE);				
				SmsChg 	= 1;
				sms[j].ChgFlag=1;
