...
int GetPtConf()
{
	...
	for(stnid=1;stnid<=g_stn_num;stnid++)
	{
		for(devid=1;devid<=g_dev_num[stnid];devid++)
		{
			...
			n++;
			ptconf[stnid][devid].stnid=stnid;
			ptconf[stnid][devid].devid=devid;
			ptconf[stnid][devid].point_num = 20;
			for(k=0;k<ptconf[stnid][devid].point_num;k++)
			{
				ptconf[stnid][devid].is_curve_point[k] = TRUE;
			}	
		}
	}
	hdb_dev_num = n;
...
void ReadDefFile()
{
        ReadAppDefFile();
        strcpy(SIDSTR,g_sid);
		strcpy(strHostToken,g_hostname_mysql);
}
...
int StrToTime(char *timestr,struct tm *p_datetime,unsigned char interval,unsigned char flag)
{
	...
	//printf("\n timestr==%s \n",timestr);
	sscanf(timestr,"%d-%2d-%2d %2d:%2d:%2d",&year,&mon,&day,
	 				&hour,&min,&sec);
	if(flag==1)
	{
	switch (interval)
...
int GetHdbIDByName(unsigned int *p_id,char * logicalname)
{
	...
	*p_id = hdb_pt_id[nDev][nPt].id;
	...
	if(*p_id == -1)
	{
		printf("logicalname=%s,%d,%d,%d,%d",logicalname,point.stn_id,point.dev_id,point.data_type,point.pt_id);
		return FALSE;
	}
	else
	{
