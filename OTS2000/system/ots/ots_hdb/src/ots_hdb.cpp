...
DATA_USER_TYPE	data_user_type[MAX_ROW_NUM];
DATA_DOC_TYPE	 data_doc_type[MAX_ROW_NUM];
DATA_DOC	data_doc[MAX_ROW_NUM];
DATA_STUDY_PLAN 	data_study_plan[MAX_ROW_NUM];
DATA_STUDY_RECORD	data_study_record[MAX_ROW_NUM];
DATA_CONTEST		data_contest[MAX_ROW_NUM];
DATA_SEND_CONTEST	data_send_contest[MAX_ROW_NUM];
DB_FMT			db_data;
...
int	SelectRecs(DB_FMT *p_data,QSqlDatabase db)
{
	...
	while(db_opr.next())
	{
		...
		if( p_data->table_id == TABLE_RECORD || p_data->table_id == TABLE_LOGIN_INFO || p_data->table_id == TABLE_SEND_TASK|| p_data->table_id == TABLE_SEND_CONTEST  )
		{
...
void InitTableName()
{
	...
	strcpy(table_name[TABLE_USER_TYPE],	"user_type");
	strcpy(table_name[TABLE_DOC],	"doc");
	strcpy(table_name[TABLE_DOC_TYPE],	"doc_type");
	strcpy(table_name[TABLE_STUDY_PLAN],	"study_plan");
	strcpy(table_name[TABLE_STUDY_RECORD],	"study_record");
	strcpy(table_name[TABLE_CONTEST], "contest");
	strcpy(table_name[TABLE_SEND_CONTEST], "send_contest");
}	
...
void InitTableData(char table_id)
{
	...
	case	TABLE_GROUPS_RECORD:
		...
		break;
	case	TABLE_USER_TYPE:
		memset(data_user_type,0, sizeof(DATA_USER_TYPE) * MAX_ROW_NUM );
		break;
	case	TABLE_DOC:
		memset(data_doc,0, sizeof(DATA_DOC) * MAX_ROW_NUM );
		break;
	case	TABLE_DOC_TYPE:
		memset(data_doc_type,0, sizeof(DATA_DOC_TYPE) * MAX_ROW_NUM );
		break;
	case	TABLE_STUDY_PLAN:
		memset(data_study_plan,0, sizeof(DATA_STUDY_PLAN) * MAX_ROW_NUM );
		break;
	case	TABLE_STUDY_RECORD:
		memset(data_study_record,0, sizeof(DATA_STUDY_RECORD) * MAX_ROW_NUM );
		break;
	case	TABLE_CONTEST:
		memset(data_contest,0, sizeof(DATA_CONTEST) * MAX_ROW_NUM );
		break;
	case	TABLE_SEND_CONTEST:
		memset(data_send_task,0, sizeof(DATA_SEND_CONTEST) * MAX_ROW_NUM );
		break;
	}
}	
...
int		ExecSql(char *sql_str,QSqlDatabase db )
{
	int ret;
	QSqlQuery db_opr("",db);
	ret = db_opr.exec(QString::fromLocal8Bit(sql_str));
...
int		ExecSqlQStr(const QString &sql_str,QSqlDatabase db)
{
	int ret;
	QSqlQuery db_opr("",db);
...
{
	QSqlError err;
	QApplication::addLibraryPath("/home/ems/h9000/bin/plugins");
	//QApplication::addLibraryPath("./plugins"); /////DZY 140619
	...
	///////test DZY!!!
	/*
	strcpy(hdb_type,"MYSQL");
	server_port = 3306;
	if (connectionName == "defaultConnection")
		db = QSqlDatabase::addDatabase("QMYSQL"); 
	else
		db = QSqlDatabase::addDatabase("QMYSQL",connectionName); 
	strcpy(server_ip,"172.18,179.253");
	strcpy(server_sid,"OTS");
	strcpy(username,"OTS");
	strcpy(password,"OTS");
	*/
	///////test DZY!!!
...
int	CvtFieldData(DB_FMT *p_data, int mode)
{
	...
	case	TABLE_GROUPS_RECORD:
		...
		break;
	case	TABLE_USER_TYPE:
		cvt_user_type(p_data,mode);
		cvt_user_type_cond(p_data,mode);
		break;
	case	TABLE_DOC_TYPE:
		cvt_doc_type(p_data,mode);
		cvt_doc_type_cond(p_data,mode);
		break;
	case	TABLE_DOC:
		cvt_doc(p_data,mode);
		cvt_doc_cond(p_data,mode);
		break;
	case	TABLE_STUDY_PLAN:
		cvt_study_plan(p_data,mode);
		cvt_study_plan_cond(p_data,mode);
		break;
	case	TABLE_STUDY_RECORD:
		cvt_study_record(p_data,mode);
		cvt_study_record_cond(p_data,mode);
		break;
	case	TABLE_CONTEST:
		cvt_contest(p_data,mode);
		cvt_contest_cond(p_data,mode);
		break;
	case	TABLE_SEND_CONTEST:
		cvt_send_contest(p_data,mode);
		cvt_send_contest_cond(p_data,mode);
		break;
	}
	return TRUE;
}
...
int	GetInexistenceIdInHdb(unsigned char table_id)
{
	...
	case	TABLE_USER_TYPE:
		strcpy(FieldStr,"user_type_id");
		break;

	case	TABLE_DOC_TYPE:
		strcpy(FieldStr,"doctype_id");
		break;

	case	TABLE_DOC:
		strcpy(FieldStr,"doc_id");
		break;

	case	TABLE_STUDY_PLAN:
		strcpy(FieldStr,"plan_id");
		break;
	...
	case	TABLE_CONTEST:
		strcpy(FieldStr,"contest_id");
		break;
		
	case	TABLE_SEND_CONTEST:
		strcpy(FieldStr,"send_contest_id");
		break;
...
int HdbLogWrite(char *sql,char *sql_error,QSqlDatabase db)
{
	FILE *fp;
	char log_name[256];
	QString appName;
	...
	fprintf(fp,"%s\t%s\t%s\t%s\n",time.toLocal8Bit().data(),db.databaseName().toLocal8Bit().data(),sql,sql_error);
	fclose(fp);
	//DZY ADDED 140706
	int errcount=0;
	while(ConnectHDB(db)==(int)FALSE)
	{
		errcount++;
		MySleep(1000);
		if (errcount>10)
		{
			...
			if (db.databaseName() == db_conn.databaseName())
			{
				exit(0);
			}
		}
