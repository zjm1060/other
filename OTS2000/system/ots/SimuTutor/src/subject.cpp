...
void  CSubjectEdit::setLanguage(int language)
{	
...
void CSubjectEdit::save()
{
	if (flag_add)
	{
		...
		if (editSubjectName->text().length()>sizeof(data_subject->name))
		{
			QMessageBox().critical(this,QString::fromLocal8Bit("错误!"),QString::fromLocal8Bit("题名长度超过40字!"),QMessageBox::Ok);	
			return;
		}
		if (editStepTip->text().length()>sizeof(data_subject->subject_tip))
		{
			QMessageBox().critical(this,QString::fromLocal8Bit("错误!"),QString::fromLocal8Bit("步骤提示信息长度超过50字!"),QMessageBox::Ok);	
			return;
		}
		if (editCondition->toPlainText().length()>sizeof(data_subject->judge_logic))
		{
			QMessageBox().critical(this,QString::fromLocal8Bit("错误!"),QString::fromLocal8Bit("题目约束条件长度超过250字!"),QMessageBox::Ok);	
			return;
		}
		if (editPreCondition->toPlainText().length()>sizeof(data_subject->pre_condition))
		{
			QMessageBox().critical(this,QString::fromLocal8Bit("错误!"),QString::fromLocal8Bit("题目前提约束条件长度超过250字!"),QMessageBox::Ok);	
			return;
		}
...
void CSubjectCondition::loadChinese()
{
	QTranslator translator;
	translator.load("SimuTutor_zh_CN");
	qApp->installTranslator(&translator);
...
void CSubjectCondition::loadEnglish()
{
	QTranslator translator;
	translator.load("SimuTutor_en");
	qApp->installTranslator(&translator);
...
void CSubjectCondition::showEvent(QShowEvent *event)
{
	event->accept();
}
void  CSubjectCondition::setLanguage(int language)
{	
	QTranslator translator;
	if(flag_simutrainee)
	{
