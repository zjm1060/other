	...
	:QDialog(parent,Qt::WindowMinMaxButtonsHint)
{
	...
	actionCopyTask = new QAction(QString::fromLocal8Bit("拷贝任务"), this);
	...
	connect(actionCopyTask,SIGNAL(triggered()),this,SLOT(openTaskCopy()));
...
void CTaskTable::contextMenuEvent(QContextMenuEvent * event)
{
	...
	if(pTableWidgetItem != NULL)
	{
		...
		popMenu->addAction(actionCopyTask);
	}
	popMenu->exec(QCursor::pos()); // 菜单出现的位置为当前鼠标的位置
}//设置右键弹出菜单
void CTaskTable::showEvent(QShowEvent *event)
{
// 	if (flag_langueage == 0)
// 	{
// 		loadChinese();
...
void  CTaskTable::setLanguage(int language)
{	
	QTranslator translator;
	if(flag_simutrainee)
	{
...
void CTaskTable::openTaskModify()
{
	...
	else
	{
		...
		//DZY 2014
		QTableWidgetItem* usertype = listTask->item(curRow,11);
	
		if(usertype->text()==QString::fromLocal8Bit("全体适用"))
			taskmodify->editUsertype->setCurrentIndex(0);
		else	if(usertype->text()==QString::fromLocal8Bit("新员工"))
			taskmodify->editUsertype->setCurrentIndex(1);
		else	if(usertype->text()==QString::fromLocal8Bit("初级工"))
			taskmodify->editUsertype->setCurrentIndex(2);
		else	if(usertype->text()==QString::fromLocal8Bit("中级工"))
			taskmodify->editUsertype->setCurrentIndex(3);
		else	if(usertype->text()==QString::fromLocal8Bit("高级工"))
			taskmodify->editUsertype->setCurrentIndex(4);
		else	if(usertype->text()==QString::fromLocal8Bit("技师"))
			taskmodify->editUsertype->setCurrentIndex(5);
		else	if(usertype->text()==QString::fromLocal8Bit("高级技师"))
			taskmodify->editUsertype->setCurrentIndex(6);
		QTableWidgetItem* iftest = listTask->item(curRow,12);
		if(iftest->text()==QString::fromLocal8Bit("可学可考"))
			taskmodify->checkBoxstudy->setChecked(true);
		else
			taskmodify->checkBoxstudy->setChecked(false);

		connect(taskmodify,SIGNAL(modifyRecord()),this,SLOT(modifyTask()));
		taskmodify->show();	
	}
}
void CTaskTable::openTaskCopy()
{
	int task_id;
	if (select_status==false)
	{
	...
	else
	{
		...
		data.cond_seg.seg_task.task_id = task_id;
		...
		data.flag.flag_task.task_userange=TRUE;
		data.flag.flag_task.user_type_id=TRUE;
		data.flag.flag_task.user_range=TRUE;
		int cur_id = GetInexistenceIdInHdb(TABLE_TASK);
		 data.seg.seg_task.task_id =cur_id;
		GetCurTimeStr(data.seg.seg_task.datetime);
		...
		DATA_SUBJECT data_s[200];////定义最大步骤数200
		if (data.rownum <200)
		{
			for(int j = 0;j<data.rownum;j++)
			{
				data_s[j] = data_subject[j];
			}
			for(int i = 0;i<data.rownum;i++)
			{
				DB_FMT data2;
				InitDbData(&data2,TABLE_SUBJECT);
				data2.flag.flag_subject.subject_id = TRUE;
				data2.flag.flag_subject.name = TRUE;
				data2.flag.flag_subject.task_id = TRUE;
				data2.flag.flag_subject.step = TRUE;
				data2.flag.flag_subject.type = TRUE;
				data2.flag.flag_subject.datetime = TRUE;
				data2.flag.flag_subject.judge_logic = TRUE;
				data2.flag.flag_subject.interlock_name = TRUE;
				data2.flag.flag_subject.pre_condition = TRUE;
				data2.flag.flag_subject.subject_tip = TRUE;
				data2.flag.flag_subject.time_limit = TRUE;
				data2.flag.flag_subject.score = TRUE;
				data2.flag.flag_subject.is_keystep = TRUE;
				data2.seg.seg_subject = data_s[i];
				data2.seg.seg_subject.task_id = cur_id;
				data2.seg.seg_subject.subject_id = GetInexistenceIdInHdb(TABLE_SUBJECT);
				GetCurTimeStr(data2.seg.seg_subject.datetime);
				if(InsertOneRec(&data2) != TRUE)
				{
					//QMessageBox().critical(this,QString::fromLocal8Bit("错误!"),QString::fromLocal8Bit("查询数据库失败!"),QMessageBox::Ok);		
...
int CTaskTable::displayTask()
{
	...
	{	
		for(int i = 0 ; i <listTask->rowCount() ; i ++)
		{
			listTask->removeRow(i);
		}
	}
	...
	{	
		...
		//		DZY
		QTableWidgetItem *itemusertype;
		if (data_task[i].user_type_id==0)
		{
			itemusertype = new QTableWidgetItem(QString::fromLocal8Bit("新员工"));
			itemusertype->setData(1,data_task[i].user_type_id);
		//	listTask->setItem(i,11,new QTableWidgetItem(QString::fromLocal8Bit("新员工")));
		}
		else if (data_task[i].user_type_id==1)
		{
			itemusertype = new QTableWidgetItem(QString::fromLocal8Bit("初级工"));
			itemusertype->setData(1,data_task[i].user_type_id);
		//	listTask->setItem(i,11,new QTableWidgetItem(QString::fromLocal8Bit("初级工")));
		}
		else if (data_task[i].user_type_id==2)
		{
			itemusertype = new QTableWidgetItem(QString::fromLocal8Bit("中级工"));
			itemusertype->setData(1,data_task[i].user_type_id);
		//	listTask->setItem(i,11,new QTableWidgetItem(QString::fromLocal8Bit("中级工")));
		}
		else if (data_task[i].user_type_id==3)
		{
			itemusertype = new QTableWidgetItem(QString::fromLocal8Bit("高级工"));
			itemusertype->setData(1,data_task[i].user_type_id);
		//	listTask->setItem(i,11,new QTableWidgetItem(QString::fromLocal8Bit("高级工")));
		}
		else if (data_task[i].user_type_id==4)
		{
			itemusertype = new QTableWidgetItem(QString::fromLocal8Bit("技师"));
			itemusertype->setData(1,data_task[i].user_type_id);
		//	listTask->setItem(i,11,new QTableWidgetItem(QString::fromLocal8Bit("技师")));
		}
		else if (data_task[i].user_type_id==5)
		{
			itemusertype = new QTableWidgetItem(QString::fromLocal8Bit("高级技师"));
			itemusertype->setData(1,data_task[i].user_type_id);
			//listTask->setItem(i,11,new QTableWidgetItem(QString::fromLocal8Bit("高级技师")));
		}
		else
		{
			//	listTask->setItem(i,11,new QTableWidgetItem(QString::fromLocal8Bit("全体适用")));
			itemusertype = new QTableWidgetItem(QString::fromLocal8Bit("全体适用"));
			itemusertype->setData(1,-1);
		}	
		listTask->setItem(i,11,itemusertype);
		
		if(data_task[i].task_userange == 1)
			listTask->setItem(i,12,new QTableWidgetItem(QString::fromLocal8Bit("可学可考")));
		else
			listTask->setItem(i,12,new QTableWidgetItem(QString::fromLocal8Bit("考试专用")));
	}
	select_status = false;
	select_stndev = false;
	select_status = false;
...
//DZY 2014
	...
	taskadd->seg.task_id = GetMaxIdInHdb(TABLE_TASK)+1;
...
void CTaskAddTable::setControlAtt()
{
	...
	editUsertype->setCurrentIndex(-1);
	checkBoxstudy->setChecked(false);
...
void  CTaskAddTable::setLanguage(int language)
{	
	QTranslator translator;
	if(flag_simutrainee)
	{
...
bool  CTaskAddTable::checkInput()
{
	
	QString str;
	str = editName->text();
	if (str.length()>sizeof(data_task->name))
	{
		QMessageBox().critical(this,QString::fromLocal8Bit("错误!"),QString::fromLocal8Bit("任务名字长度超过25!"),QMessageBox::Ok);
		return false;
	}
	if (remarks.length()>sizeof(data_task->remark))
	{
		QMessageBox().critical(this,QString::fromLocal8Bit("错误!"),QString::fromLocal8Bit("提示信息长度超过100!"),QMessageBox::Ok);
		return false;
	}
	else
		return  true;
...
{	
	...
	if (editUsertype->currentIndex()<=0)
	{
		seg.user_type_id = -1;
		//strcpy(seg.scene_name,CONST_NEG_STR);
	}
	...
	{
		seg.user_type_id = editUsertype->currentIndex()-1;
		//strcpy(seg.scene_name,editScene->currentText().toLocal8Bit().data());
	}
...
void CTaskAddTable::displaySubject()//DZY 2014 添加步骤号
{
	totle_score=0.00;
	DB_FMT data;
	InitDbData(&data,TABLE_SUBJECT);
	...
	for(int i=0;i<data.rownum;i++)
	{
		...
		tableWidget->setItem(i,0,new QTableWidgetItem(QString::number(data_subject[i].step)));//DZY 2014
		tableWidget->setItem(i,1,item);
		tableWidget->setItem(i,2,new QTableWidgetItem(QString::number(data_subject[i].score)));
		tableWidget->setItem(i,3,new QTableWidgetItem(QString::number(data_subject[i].time_limit)));
		tableWidget->setItem(i,4,new QTableWidgetItem(QString::fromLocal8Bit(data_subject[i].subject_tip)));
...
void CTaskAddTable::openModifySubject()
{
	if (selectionModel->hasSelection())
	{
		...
		QTableWidgetItem* subjectNameItem = tableWidget->item(tableWidget->currentRow(),1);
		...
		QTableWidgetItem* scoreItem = tableWidget->item(tableWidget->currentRow(),2);
		subjecteditmodify->editScore->setValue(scoreItem->text().toInt());
		QTableWidgetItem* timeItem = tableWidget->item(tableWidget->currentRow(),3);
		subjecteditmodify->editTime->setValue(timeItem->text().toInt());
		QTableWidgetItem* stepTipItem = tableWidget->item(tableWidget->currentRow(),4);
...
void CTaskAddTable::delSubject()
{
	if (selectionModel->hasSelection())
	{
		...
		for (int i=indexes.size()-1;i>=0;i--)
		{
			...
			QTableWidgetItem* subjectNameItem = tableWidget->item(row,1);//dzy
			DB_FMT data;
			InitDbData(&data,TABLE_SUBJECT);
...
void CTaskAddTable::moveUp()
{
	if (selectionModel->hasSelection())
	{
		...
		if (row == 0)
			return;
		QTableWidgetItem* subjectNameItem = tableWidget->item(row,1);
