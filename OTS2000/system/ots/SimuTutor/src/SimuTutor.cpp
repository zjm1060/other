...
void CTeacherManage::closeEvent(QCloseEvent *event)//关闭窗口最小化
{
	...
	chk->setText(tr("不再提醒"));
	lbl->setText(tr("您点击了关闭按钮，您是想："));
	chkMin->setText(tr("最小化到系统托盘区，不退出程序"));
	chkClose->setText(tr("退出程序"));
	butEnter->setText(tr("确定"));
	butExit->setText(tr("取消"));
...
void CTeacherManage::isQuit()
{
	...
	msgBox.setInformativeText(tr("确定要退出OTS2000教员站？"));
...
{
	actionLoginStatus->setText(tr("当前登录用户：")+QString::fromLocal8Bit("%1").arg(login_name));
...
// 			return;
		...
		actionLoginStatus->setText(tr("用户尚未登录"));
		loginUser->init();
...
void CTeacherManage::initStnView()
{
	...
	buttonEnter->setText(tr("确定"));
	buttonExit->setText(tr("取消"));
...
void CTeacherManage::show_tryconnectfail()
{
	...
	m_threadmsgbox->setWindowTitle (tr("提示"));
	m_threadmsgbox->setInformativeText(tr("多次尝试连接失败，程序将退出，请检查本机及服务器之间网络链接。"));	
...
void CTeacherManage::show_tryconnec(int i,bool connectedOK)
{
	...
	if (!connectedOK)
	{
		m_threadmsgbox->setInformativeText(tr("连接服务器失败，正在尝试第")+QString::number(i)+tr("次连接"));
	}
	else
	{
		m_threadmsgbox->setInformativeText(tr("重新连接历史服务器成功"));
		m_NetstateThreadTutor->cout_connected = 0;
	}
...
void CTeacherManage::createBroadcast()
{
	...
	buttonBroadcast->setText(QString::fromLocal8Bit("发 送"));
	buttonquitBroadcast->setText(QString::fromLocal8Bit("退 出"));
	...
	m_dlgbroadcast->setWindowTitle(QString::fromLocal8Bit("发送广播"));
...
void CTeacherManage::sendBroadcast()
{
	...
	QByteArray datagram =(QString::fromLocal8Bit("来自教员:") + str).toLocal8Bit () ;
