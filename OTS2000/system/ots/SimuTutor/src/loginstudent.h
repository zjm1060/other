...
class CTrainManagerTable : public QDialog,public Ui_CTrainManagerTable
{
	...
	private:
		int user_id,host_id;
		int simu_mode,simu_speed,simu_status,interlock_mode;
	...
	private Q_SLOTS:
		...
		void setSimuPause();
		void setSimuStop();
		void setinterlockmode(int mode);
	public slots:
...
class CLoginStudentTable : public QDialog,public Ui_CLoginStudentTable
{
	...
	public:
		...
		QTimer *refresh;
		QString students_state,studentsmode,studentsready;
	...
	public slots:
		...
		void logoutForce();
		void updateModel();
		void settrainmode(int i);
		void logoutAllusers();
		void setAllusersWait();
		void setAllusersStart();
		void setLanguage( int );
protected:
	void showEvent(QShowEvent *);
};
