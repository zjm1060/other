...
void CSceneTable::showEvent(QShowEvent *event)
{
	event->accept();
}
void  CSceneTable::setLanguage(int language)
{	
	QTranslator translator;
	if(flag_simutrainee)
	{
...
void CSceneTable::delFromMergerList()
{
	...
	for (int i=0;i<listSceneMerger->rowCount();i++)
	{
		...
		QVariant v(item_name->data(3).toInt());
		comboSceneName->addItem(item_name->text(),v);
	}
}
...
{
	if (listSceneMerger->rowCount() != 2)
		return;
	QMessageBox msgBox(this);
	msgBox.setIcon(QMessageBox::Information);
	msgBox.setWindowTitle (QString::fromLocal8Bit("提示"));
	msgBox.setInformativeText(QString::fromLocal8Bit("是否确认合并工况，合并工况时间稍久，且合并后不可恢复"));
	...
	if (listSceneMerger->rowCount() == 2)
	{
		...
		//dzy 用选用合并后的名字的工况作为主工况
		int scene_id = comboSceneName->itemData(comboSceneName->currentIndex()).toInt();
		int scene_id2 = listSceneMerger->item(0,0)->data(3).toInt();
		if (scene_id2==scene_id)
		{
			scene_id2 = listSceneMerger->item(1,0)->data(3).toInt();
		}
		DB_FMT data1,data2;
		InitDbData(&data1,TABLE_SCENE_DATA);
		data1.cond_flag.flag_scene_data.scene_id = TRUE;
		data1.cond_seg.seg_scene_data.scene_id = scene_id;
		if( SelectRecs(&data1) != TRUE )
		{
			QMessageBox().information(this,QString::fromLocal8Bit("提示!"),QString::fromLocal8Bit("合并工况失败!"),QMessageBox::Ok);	
			displayScene();
			return;
		} 
		QList<DATA_SCENE_DATA> data_scene_data2;
		InitDbData(&data2,TABLE_SCENE_DATA);
		data2.cond_flag.flag_scene_data.scene_id = TRUE;
		data2.cond_seg.seg_scene_data.scene_id = scene_id2;
		...
		{
			for(int i = 0 ; i < data2.rownum ; i ++)
				data_scene_data2.append(data_scene_data[i]) ;
		}
		...
		progressDlg->setLabelText(tr("正在更新数据,请稍后......      "));  
		progressDlg->setCancelButtonText(tr("取消"));  
		progressDlg->setRange(0, data2.rownum-1);
		for(int i = 0 ; i < data2.rownum ; i++)
		{
			...
			data1.cond_flag.flag_scene_data.scene_id = TRUE;
			data1.cond_seg.seg_scene_data.scene_id = scene_id;
			data1.cond_flag.flag_scene_data.logicalname = TRUE;
			strcpy(data1.cond_seg.seg_scene_data.logicalname,data_scene_data2.at(i).logicalname);

			SelectRecs(&data1) ;
			data1.flag.flag_scene_data.logicalname = TRUE;
			strcpy(data1.seg.seg_scene_data.logicalname , data_scene_data2.at(i).logicalname);
			data1.flag.flag_scene_data.data = TRUE;
			data1.seg.seg_scene_data.data= data_scene_data2.at(i).data;
			data1.flag.flag_scene_data.status = TRUE;
			data1.seg.seg_scene_data.status =data_scene_data2.at(i).status;
			if (/* SelectRecs(&data1) == TRUE && */data1.rownum>0 )
			{
				MySleep(1);
				if( UpdateOneRec(&data1) != TRUE)
				{
					HdbLogWrite(data1.sql_str,data1.sql_error);
					continue;
				}
			}
			else if(/* SelectRecs(&data1) == TRUE && */data1.rownum==0 )
			{
				MySleep(1);
				data1.flag.flag_scene_data.scene_id = TRUE;
				data1.seg.seg_scene_data.scene_id = scene_id;
				if( InsertOneRec(&data1) != TRUE)
				{
					HdbLogWrite(data1.sql_str,data1.sql_error);
					continue;
				}
	...
	for(i=0;i<n_scene;i++)
	{
		...
		for( j=scenedb[i].head; j<scenedb[i].head+scenedb[i].num; j++ )
		{
			...
			else
			{
				...
				//yyp 2009.2.11
				MySleep(2);//dzy modified 20 to 2
				switch	( data_type )
				{
				case	ANA_TYPE:
				case	PARAM_TYPE:
...
void CSceneAddTable::showEvent(QShowEvent *event)
{
	event->accept();
}
void  CSceneAddTable::setLanguage(int language)
{	
	QTranslator translator;
	if(flag_simutrainee)
	{
