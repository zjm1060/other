...
void CFaultDataTable::showEvent(QShowEvent *event)
{
	event->accept();
}
void  CFaultDataTable::setLanguage(int language)
{	
	QTranslator translator;
	if(flag_simutrainee)
	{
...
int CFaultDataTable::displayFaultData()
{
	...
	listFaultData->setColumnWidth(0,250);
	listFaultData->setColumnWidth(1,100);
	listFaultData->setColumnWidth(2,60);
	listFaultData->setColumnWidth(3,60);
	listFaultData->setColumnWidth(4,60);
	listFaultData->setColumnWidth(5,60);
	listFaultData->setColumnWidth(6,60);
	listFaultData->setColumnWidth(7,60);
	listFaultData->clearSelection();
	select_status = false;
	//listFaultData->resizeColumnsToContents();
	if (listFaultData->rowCount() > 0)
	{
		listFaultData->resizeColumnsToContents();
	}
	return data.rownum;
...
void CFaultDataAddTable::showEvent(QShowEvent *event)
{
	event->accept();
}
void  CFaultDataAddTable::setLanguage(int language)
{	
	QTranslator translator;
	if(flag_simutrainee)
	{
