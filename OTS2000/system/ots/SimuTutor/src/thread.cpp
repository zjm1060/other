...
/***************************************************************************************************************************************/
void NetstateThreadTutor::run()
{
	int server_port;
	if (strcmp(g_ots_hdb_type,"ORACLE")==0)
	{		
		server_port = 1521;
		nettest =  QSqlDatabase::addDatabase("QOCI","NETTESTTUTOR"); 
	}
	else if (strcmp(g_ots_hdb_type,"MYSQL")==0)
	{
		server_port = 3306;
		nettest = QSqlDatabase::addDatabase("QMYSQL","NETTESTTUTOR"); 
	}
...
bool NetstateThreadTutor::tryconeectdb()
{	
	m_connected = nettest.open();
	return m_connected;
}
void NetstateThreadTutor::tryagain()
{
	cout_connected++;
	if(cout_connected<=10)
	{
