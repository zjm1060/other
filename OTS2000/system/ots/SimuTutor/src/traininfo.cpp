...
void CTrainInfoTable::showEvent(QShowEvent *event)
{
	if (flag_langueage == 0)
	{
		loadChinese();
...
void  CTrainInfoTable::setLanguage(int language)
{	
	QTranslator translator;
	if(flag_simutrainee)
	{
...
void CTrainInfoAddTable::loadChinese()
{
	QTranslator translator;
	translator.load("SimuTutor_zh_CN");
	qApp->installTranslator(&translator);
...
void CTrainInfoAddTable::loadEnglish()
{
	QTranslator translator;
	translator.load("SimuTutor_en");
	qApp->installTranslator(&translator);
...
void CTrainInfoAddTable::showEvent(QShowEvent *event)
{
	if (flag_langueage == 0)
	{
		loadChinese();
...
void  CTrainInfoAddTable::setLanguage(int language)
{	
	QTranslator translator;
	if (language == 0)
	{
