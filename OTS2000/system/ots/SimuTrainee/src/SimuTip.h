...
public:
	SimuTipThread(int task_id,int send_task_id,int step_num,QObject *parent=NULL):m_task_id(task_id),m_send_task_id(send_task_id)
							{/*calc = new CTaskCalculation(m_task_id,m_send_task_id);*/db_simutip = NULL;m_step_num=step_num;stop_flag=false;};//dzy 0706 removed calc = new CTaskCalculation(m_task_id,m_send_task_id);
	~SimuTipThread(){stop_flag = true;CloseHDB(*db_simutip);delete calc;};
	void run();
	void stop(){stop_flag = true;};
	bool stop_flag;
	CTaskCalculation *calc;
Q_SIGNALS:
	void sigStepFinished(int step,int mode = 0);
	void sigStepOvertime(int step,int mode = 1);//dzy new added
private:
	QMutex mutex;
	int m_task_id;
	int m_send_task_id;
...
private slots:
	void lock();
	void unLock();
	void setCurStepShow(int, int mode = 0);//dzy new added int mode = 0
protected:
