...
:QWidget(parent)
{
	...
	dlg_created_flag[REVERTDLG] = true;
...
//	connect(listRevert,SIGNAL(entered(const QModelIndex &)),this,SLOT(PopupRevertDetail(const QModelIndex &))); //DZY
	connect(listRevert,SIGNAL(itemClicked(const QModelIndex &)),this,SLOT(PopupRevertDetail(const QModelIndex &)));
	 
}
CRevertPlayer::~CRevertPlayer()
{
	dlg_created_flag[REVERTDLG] = false;
}
...
void CRevertPlayer::loadChinese()
{
...
void CRevertPlayer::loadEnglish()
{
	QTranslator translator;
...
void  CRevertPlayer::setLanguage(int language)
{	
	QTranslator translator;
	if (language == 0)
	{
...
void CRevertPlayer::init()
{
 ...
 //	sliderProgress->setEnabled(false); //DZY
...
void CRevertPlayer::PopupRevertDetail(const QModelIndex &index)
{
	...
	if(listRevert->currentRow()< 0)//DZY
	{
		return;
	}
