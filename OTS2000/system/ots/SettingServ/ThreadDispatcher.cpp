...
extern HENV		henv; 
extern char		szDSN[40];		//ODBC数据源
extern char		szUID[20];		//数据库用户
extern char		szAuthStr[20];	//数据库用户口令

///////////////////////////////////////////////////////////////////
// class CWizThreadDispatcher
// Default Constructor
CWizThreadDispatcher::CWizThreadDispatcher(
	CWizMultiThreadedWorker &rWorker, int MaxThreads)
...
void CWizThreadDispatcher::WaitShutDown()
{
	...
	for (INDEX i = 0; i < hMax; i++)
	{
		while (1)
		{
			const DWORD dwWaitRes = 
				::MsgWaitForMultipleObjects(1, &(m_ahWorkersHandles[i]),
...
UINT CWizThreadDispatcher::Run()
{
	...
	for (INDEX i = 0; i < m_nMaxThreads; i++)
	{
		...
		CWinThread* pThr = 
			AfxBeginThread(CWizThreadDispatcher_WorkerThread_Fun, pTr);
		if (pThr == NULL)
			AfxThrowMemoryException();
	...
	while (1)
	{
		...
		const DWORD res = 
			...
			StartedTreatmentEventsCount, 
			m_ahStartedTreatmentEvents, 
			FALSE, 
			INFINITE);
...
{
	CWizThreadDispatcher::WorkerThread* pWorker = 
		(CWizThreadDispatcher::WorkerThread*)pParam;
