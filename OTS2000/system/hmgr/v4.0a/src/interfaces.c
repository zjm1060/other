...
#define HAVE_IFACE_LIFCONF
#endif

#define AUTOCONF_TEST
...
struct iface_struct {
	...
	struct in_addr netmask;
        struct in_addr bdcast;
#ifdef HAVE_IFACE_LIFCONF
        uint64_t	iflags;
#else
        unsigned int iflags;
#endif
...
static int _get_interfaces(struct iface_struct *ifaces, int max_interfaces)
{
	...
	for (i=n-1;i>=0 && total < max_interfaces;i--) {
		...
		iflags= lifr[i].lifr_flags;
		
		if (ioctl(fd, SIOCGLIFNETMASK, &lifr[i]) != 0) {
			continue;
		}

		nmask = ((struct sockaddr_in *)&lifr[i].lifr_addr)->sin_addr;

		if (ioctl(fd, SIOCGLIFBRDADDR, &lifr[i]) != 0) {
			continue;
		}

		bdcast = ((struct sockaddr_in *)&lifr[i].lifr_addr)->sin_addr;
                
		strncpy(ifaces[total].name, iname, sizeof(ifaces[total].name)-1);
		ifaces[total].name[sizeof(ifaces[total].name)-1] = 0;
		ifaces[total].ip = ipaddr;
		ifaces[total].netmask = nmask;
		ifaces[total].bdcast = bdcast;
		ifaces[total].iflags = iflags;
		total++;
	}
...
#elif defined(HAVE_IFACE_IFCONF)
...
static int _get_interfaces(struct iface_struct *ifaces, int max_interfaces)
{
	...
	for (i=n-1;i>=0 && total < max_interfaces;i--) {
		...
		bdcast = ((struct sockaddr_in *)&ifr[i].ifr_addr)->sin_addr;
 ...
 int main()
{
	...
	for (i=0;i<total;i++) {
		...
		printf("NETMASK=%s ", inet_ntoa(ifaces[i].netmask));
                brdaddr.s_addr= ifaces[i].ip.s_addr | (~(ifaces[i].netmask.s_addr));
                printf("Calc brdaddr=%s ", inet_ntoa(brdaddr));
                printf("FLAGS=%x ", ifaces[i].iflags);
                printf("BRDADDR=%s \n", inet_ntoa(ifaces[i].bdcast));
	}
	return 0;
}
#endif
