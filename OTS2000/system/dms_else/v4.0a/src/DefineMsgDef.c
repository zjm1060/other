...
int GenerateAlmMsgDef(void)
{
	...
	{ 
		...
		if ( !strcmp(strbuf[0], "GROUP") & !strcmp(strbuf[1], "NAME") )
		{
			...
			groupNo= atoi(strbuf[2]);
			if(groupNo<0 || groupNo>=MAX_MSG_GROUP_NUM)
			{
		...
		{
			entryNo= atoi(strbuf[1]);
			if(entryNo<0 || entryNo>=MAX_IND_STATE_NUM)
			{
...
/*			printf("\nmsg_def[%d].group[%d].def[%d]= %s", procType, groupNo, entryNo, strbuf[2]);*/
		}
	}
...
int GenerateCmdMsgDef(void)
{
	...
	{ 
		...
		{
			entryNo= atoi(strbuf[1]);
			if(entryNo<0 || entryNo>=MAX_CMD_OPER_TYPE_NUM)
			{
			...
			/*printf("\ntype= %d, cmd_msg[%d].group[%d].def[%d]= %s", cmdType, cmdPos, groupNo, entryNo, strbuf[2]);*/
		}
	}
