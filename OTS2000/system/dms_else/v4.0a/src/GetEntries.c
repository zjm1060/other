...
/*#define	STATISTIC*/
	...
	{
	FILE	*fp;
	if(fp=fopen("/home/ems/h9000/gen/statistic.txt", "a"))
	{
		fprintf(fp, "\n\n%d: ", entry_num);
		for(id=0; id<12; id++)
...
int RTDB_Set_StrAttrFromString(void)
{
	...
	else if( !strcmp( strbuf[0], "NODENAME" ) )
	{
		if( strlen( strbuf[1] ) >= ENTRY_NODENAME_SIZE )
		{
			strbuf[1][ENTRY_NODENAME_SIZE-1] =0;  
			sprintf(err_buf, "%s line %d: %s: entry node name is too long", rec_filename, lineNo, buffer);
			PrintError(err_buf);
		}
		strcpy( fixed_ptr->nodeName, strbuf[1] );
		return 0;
	}
	return -1;
}
...
int RTDB_Set_Fixed_AttributesFromString(void)
{
	...
	else if( !strcmp( strbuf[0], "CMD_MSG" ) )
	{
		...
		{
			sprintf(err_buf, "%s line %d: %s: over range(0-7)", rec_filename, lineNo, buffer);
			PrintError(err_buf);
		}
		else
