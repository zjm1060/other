...
BOOL CTransferfileDlg::OnInitDialog()
{
	...
	m_listDst.SetColumnWidth(0, 120);
	m_listDst.SetColumnWidth(1, 50);
	m_listDst.SetColumnWidth(2, 50);
	UpdateData(false);
...
bool CTransferfileDlg::UpdateIPList() 
{
	...
	{	
		...
		if(m_config.m_dstComputer[i].Status==0) 
			...
			strDstStatus="启用";
		nIndex = m_listDst.InsertItem(i,m_config.m_dstComputer[i].DstIP);
...
//	char buffer[10];
	...
	for(i=0;i<nDstCount;i++)							//循环处理每个目标计算机的文件传输
	{
		...
		csMessage = "\r\n开始进行  " + m_config.m_dstComputer[i].DstIP +"  的处理...";
		Sleep(30);
		MyMsgBox(csMessage);
		Sleep(1000);
...
{
	// TODO: Add your control notification handler code here
	csMessage="本地磁盘设置为F:\\ ,  本地路径为F:\\home\\ems\\";
	MyMsgBox(csMessage);
...
*/
	csMessage="本地磁盘设置为E:\\ ,  本地路径为E:\\home\\ems\\";
	MyMsgBox(csMessage);
	UpdateData(true);
	m_config.m_srcDriver=strDriverName[m_radioDriver];
//	m_config.m_srcVersion=strVerName[m_radioVer];
...
*/
	csMessage="本地磁盘设置为D:\\ ,  本地路径为D:\\home\\ems\\";
	MyMsgBox(csMessage);
	UpdateData(true);
	m_config.m_srcDriver=strDriverName[m_radioDriver];
//	m_config.m_srcVersion=strVerName[m_radioVer];	
...
bool CTransferfileDlg::ShowFileDlg(int DlgID)
{
	...
	if(DlgID==0)
	{
		dlgFile.m_ofn.lpstrFilter= "项目配置文件(*.def)\0*.def";
	}
	if(DlgID==1)
	{
		dlgFile.m_ofn.lpstrFilter= "数据文本文件(*.*)\0*.*";
	}
	if(DlgID==2)
	{
		dlgFile.m_ofn.lpstrFilter= "数据生成文件(*.*)\0*.*";
	}
	if(DlgID==3)
	{
		dlgFile.m_ofn.lpstrFilter= "可执行文件(*.*)\0*.*";
	}
	if(DlgID==4)
	{
...
{
	// TODO: Add your control notification handler code here
	csMessage="本地磁盘设置为c:\\ ,  本地路径为c:\\home\\ems\\";
	MyMsgBox(csMessage);
	UpdateData(true);
	m_config.m_srcDriver=strDriverName[m_radioDriver];
//	m_config.m_srcVersion=strVerName[m_radioVer];
...
void CTransferfileDlg::OnEnableallmenu() 
{
	// TODO: Add your command handler code here
	int i;
	int nDst=0;										//选中的目标计算机编号
...
void CTransferfileDlg::OnDisenableallmenu() 
{
	// TODO: Add your command handler code here
	int i;
	int nDst=0;										//选中的目标计算机编号
