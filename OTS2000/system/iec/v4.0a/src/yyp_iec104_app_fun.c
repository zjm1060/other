...
int	IEC870_5_Send(int sock)
{
	...
	unsigned short cmd_type,op_type; 
	...
	while( cmd_end != shm_comm_cmd->stn_data[stnid].head )
	{
		...
		memcpy(&cmd_stat,&tmpstr[6],2);
		cmd_data = cmd_rec.data;//?

		op_type = cmd_stat.opr;
		memcpy(&tmp,&cmd_stat,2);
		memcpy(&tmp,&tmpstr[6],2);		
...
#ifdef DEBUG
		//printf("\nNew Cmd:cmd_type=%d stn_id=%d dev_id=%d data_type=%d point_id=%d cmd_stat=%d cmd_data=%f host_id=%d,op_type=%d,status=%d\n",	
		//cmd_type, rcv_ptr.stn_id, rcv_ptr.dev_id, rcv_ptr.data_type, rcv_ptr.pt_id, cmd_stat.state, (cmd_data/1000), host_id, op_type,12 );
		printf("\n host_id=%d,op_type=%d,status=%d\n",	host_id, op_type,tmp);
