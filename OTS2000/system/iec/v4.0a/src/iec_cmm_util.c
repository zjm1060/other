...
void InitOutputFilter(char *comm_task, int stn_no)
{
	...
	if(stn_no==-1)
		sprintf(filename, "%s%s_COM_LOG%d.txt", IEC_LOGPATH, comm_task, threadId);
	else
		sprintf(filename, "%s%s_COM_LOG%d_%d.txt", IEC_LOGPATH, comm_task, stn_no+1, threadId);
	...
	if(i>=MAX_THREAD_NUM)
		return;
	dsp_info_file.file[i].fp= fopen(filename, "w");
...
int UpdateCtrlPrivChan(char *host, unsigned short ctrlPrivChannel)
{
	...
	sprintf(name_buf, "CTRL_PRIV_CHAN");
	if(-1==GetEntryIdByName(name_buf, &point))
	{
