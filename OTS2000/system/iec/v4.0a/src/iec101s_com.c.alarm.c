...
MIRBUF		mirbuf;
/*
ICGROUPBUF	icgroupbuf;
CIGROUPBUF	cigroupbuf;
*/
int	fcb,acd,dfc;
...
void Endproc(int sig_no)
{
	...
	{
		sprintf(infobuf,"  ---- iec101s_com: exit signal %d received!  ----\n",sig_no);
		for(i=0;i<MAX_PROC_NUM;i++)
		{
			if(sProcRec[i].Pid != -1)
			{
				sprintf(infobuf," kill ---- %s iec101s_com: ! ( pid=%05d ) ----\n",sProcRec[i].NameID,sProcRec[i].Pid);
	...
	{
		sprintf(infobuf,"  ---- iec101s_com: exit signal %d received! Exit ----\n",sig_no);
		printf("%s",infobuf);
...
void Waitchild(int nouse)
{
	...
	sprintf(infobuf,"  ---- iec101s_com Master in Waitchild() !!!\n");
	alarm(1);
	signal(SIGALRM,Timer);
	wpid = wait(&status);
	alarm(0);
	if(wpid != -1)
	{
		for(i=0;i<MAX_PROC_NUM;i++)
		{
			...
			{
				sprintf(infobuf,"  ---- %s iec104s_com: stop! ( pid=%05d ) ----\n",sProcRec[i].NameID,sProcRec[i].Pid);
				sProcRec[i].Pid = -1;
				break;
			}
		}
...
/*	打开显示信息记录共享内存      */
	if ((shm_id_buf=shmget(ftok(IEC104_FILE_KEY,(SHM_BUF_KEY_CHAR+stn_id)),sizeof(SHM_BUF),0777|IPC_CREAT|IPC_EXCL))==-1)
	{
		if ( errno==EEXIST)
		{
			shm_id_buf= shmget ( ftok (IEC104_FILE_KEY,(SHM_BUF_KEY_CHAR+stn_id)),sizeof (SHM_BUF),0);
#ifdef DEBUG
			printf ( "iec104s :shmkey %d shmid %d is existed already\n",(SHM_BUF_KEY_CHAR+stn_id),shm_id_buf);
#endif
		}
		else
...
/*	获取记录区共享内存地址  */
	...
	if ((long *)shm_buf_addr==(long *)-1)
		sprintf(infobuf,"\niec104s::Error shmat shm_buf_addr: errno=%d\n",errno);
	else
		sprintf(infobuf,"\niec104s:: shmat shm_buf_addr: OK!\n");

	shm_buf_addr->header_ptr = 0;

	sprintf(infobuf,"H9000通信系统: %s iec101协议(iec101s_com) Start!",sProcRec[stn_id].NameID);
	printf("%s",infobuf);
	WriteDspInfo(infobuf);
	...
	if (hThread_SOE != 0)
	{
		sprintf(infobuf,"iec101s_com:Error in thr_create ThreadID_SOE:%d error: %d!\n ", ThreadID_SOE, errno);
		printf("%s",infobuf);
		WriteDspInfo(infobuf);
		exit(-1);
	}
	...
	{
		sprintf(infobuf,"iec101s_com:thr_create ThreadID_SOE: %d OK!\n",hThread_SOE);
		printf("%s",infobuf);
		WriteDspInfo(infobuf);
	}
	...
	if (hThread_com != 0)
	{
		sprintf(infobuf,"iec101s_com:Error in thr_create ThreadID_com:%d error: %d!\n ", ThreadID_com, errno);
		printf("%s",infobuf);
		WriteDspInfo(infobuf);
		exit(-1);
	}
	...
	{
		sprintf(infobuf,"iec104s_com:thr_create ThreadID_com: %d OK!\n",ThreadID_com);
		printf("%s",infobuf);
		WriteDspInfo(infobuf);
	}
	...
	while( !thr_join(0, &hThread_ret, NULL) )
	{
		...
		{
			sprintf(infobuf,"iec101s_com: restart thread ThreadID_SOE %d\n", hThread_ret);
			printf("%s",infobuf);
			WriteDspInfo(infobuf);
			hThread_SOE = thr_create(NULL, 0, IEC101_SOE, NULL, 0, &ThreadID_SOE);
			if (hThread_SOE != 0)
		...
		{
			sprintf(infobuf,"iec101s_com: restart thread ThreadID_com %d\n", hThread_ret);
			printf("%s",infobuf);
			WriteDspInfo(infobuf);
			hThread_com = thr_create(NULL, 0, Com_Link, (void *)stn_id, 0, &ThreadID_com);
			if (hThread_com != 0)
			...
			{
				sprintf(infobuf,"iec101s_com:thr_create ThreadID_com: %d OK!\n",hThread_com);
				printf("%s",infobuf);
				WriteDspInfo(infobuf);
				continue;
			}
...
*/
	...
	if(hPort_com < 0)
	{
		sprintf(infobuf,"\niec101s_com: device %s init failed!!! Exit now!!!\n\n",strDev);
		printf("%s",infobuf);
		WriteDspInfo(infobuf);
		thr_exit(NULL);
	}
	sprintf(infobuf,"\niec101s_com: %s BaudRate= %d init compelete fd = %d!!!\n\n",strDev,nBaudRate,hPort_com);
#ifdef DEBUG
	printf("%s",infobuf);
#endif
	WriteDspInfo(infobuf);
...
*/
		for(;;)
		{
			...
			else if (errno == EINTR && AlarmFlag) 
			{  
				timeoutnum++;

				sprintf(infobuf,"iec101s_com: %s receive() TIMEOUT No: %d! ret=%d\n",sProcRec[pnum].NameID,timeoutnum,ret);
#ifdef DEBUG
				printf("%s",infobuf);
#endif
				WriteDspInfo(infobuf);
				...
				if(timeoutnum%20==0)
				{
					sprintf(infobuf,"ReceiveData: timeout! No: %d",timeoutnum);
					printf("%s",infobuf);
					WriteDspInfo(infobuf);
		...
		if (dwByteRecvd  <= 0)
		{
			if(AlarmFlag == 0)
			{
				printf("!!!iec101s_com Station %s read error: %s\n",sProcRec[pnum].NameID,strerror(errno));
				printf("%s",infobuf);
				WriteDspInfo(infobuf);
			}
			continue;
...
/*
			...
			if (dwByteRecvd  <= 0)
			{
				sprintf(infobuf,"!!!Controlled Station %s read error: %s\n",sProcRec[pnum].NameID,strerror(errno));
				printf("%s",infobuf);
				WriteDspInfo(infobuf);
				continue;
			}
*/		
			timeoutnum =0;
			sprintf(infobuf,"\nControlled Station %s received data byte nNumber %u :\n",sProcRec[pnum].NameID,dwByteRecvd);
#ifdef DEBUG
			printf("%s",infobuf);
#endif
			WriteDspInfo(infobuf);
...
*/
	...
	else if(dbentry.var.ind.status.rt_state != status)
	{
		...
		lanbcdata.status=status+0x80;	/*communication channel status: 0:normal; 1:Fail */
		lanbcdata.ms=0;
