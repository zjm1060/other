...
MIT_ASSOC_INFO *pstMitAssoc;
  ...
  if (ask ("\n Start all Associations (y:TRUE, n:FALSE) (default = TRUE) : ",1))
    ...
    pstMitAssoc = getAssociationName(buffer);
      /* first find the Association name so we have the configured info	*/
    if (pstMitAssoc)
      {
      startAssociation (pstMitAssoc);
      }
...
MIT_ASSOC_INFO *pstMitAssoc;
  ...
  if (ask ("\n Stop all Associations (y:TRUE, n:FALSE) (default = TRUE) : ",1))
    {
    pstMitAssoc = pstAssocInfoSet;
    while (pstMitAssoc)
      {
      printf( "\n Stopping Association: '%s'", pstMitAssoc->associationName);
