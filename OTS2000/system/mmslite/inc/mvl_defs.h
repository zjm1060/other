...
typedef struct mvl_net_info
  ...
  ST_BOOLEAN mi_in_use;		/* used only for ICCP_LITE_SUPP		*/
  struct _mi_conn *mi_conn;	/* used only for ICCP_LITE_SUPP		*/
...
typedef struct mvl_var_assoc
  ...
  ST_RTINT prim_num;		/* Index to first prim in RUNTIME_TYPE	*/
...
typedef struct mvl_req_pend
  ...
  struct _mi_req_ctrl *mi_req;	/* used for ICCP_LITE_SUPP		*/
  } MVL_REQ_PEND;
