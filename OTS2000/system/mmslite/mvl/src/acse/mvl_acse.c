...
ST_INT mvla_initiate_req_ex (ST_CHAR *locAr,
			     ST_CHAR *remAr,
			     INIT_INFO *req_info, 
			     INIT_INFO *resp_info,
...
#if defined (SISCO_STACK_CFG)	/* Uses just one list of AR Names.	*/
...
ST_UCHAR      *user_info_buf;
  ...
  //modified by wzg
  printf ("ACSE: Associate Indication received from %s\n", inet_ntoa(*((struct in_addr*)&info->calling_paddr.netAddr.ip)));
   MVL_LOG_ACSE1 ("ACSE: Associate Indication received from %s\n", inet_ntoa(*((struct in_addr*)&info->calling_paddr.netAddr.ip)));
