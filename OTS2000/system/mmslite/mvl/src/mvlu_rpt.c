  ...
  /* RBE is enabled, see if a RBE Period is selected ... */
    if (basrcb->RBEPd != 0)
      ...
      if (timeNow > rptClient->next_rbe_rpt_time)
        {
      /* OK, send the report! */
        MVLU_LOG_FLOW1 ("UCA Report RBE Period timeout for client " S_FMT_PTR,
							rptClient);
    ...
    /* we must exceed one or both constraints before sending		*/
      ...
      if (sendRbe == SD_TRUE)
        {
        MVLU_LOG_FLOW1 ("UCA Report RBE for client " S_FMT_PTR, rptClient);
        rc = mvlu_send_report (rptClient, MVLU_RPT_TYPE_RBE);
        return;
        }
      }
...
MMS_BTIME6 TimeOfEntry;		/* Time this report generated.		*/
  ...
  MVLU_LOG_FLOW1 ("Sending UCA Report, MVL_NET_INFO " S_FMT_PTR, rptClient->netInfo);
...
/* We will create a NVL to send, using the dsNvl as a model ...		*/
  ...
  if (optFld & MVLU_RPTTIM_MASK)
    ...
    MVLU_LOG_CFLOW2 ("    RptTim : %ld ms, %ld days",
              (ST_LONG)TimeOfEntry.ms, (ST_LONG)TimeOfEntry.day);
...
ST_RET ret;
  ...
  if (rptClient->basrcb.IntgPd != 0
    ...
    {
    MVLU_LOG_FLOW1 ("UCA Report Integrity Period timeout for client " S_FMT_PTR,
							rptClient);
