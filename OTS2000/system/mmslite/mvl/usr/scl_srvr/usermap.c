...
ST_UINT ld_count)		/* num array elements		*/
{
	...
	*/
		...
		/* If NULL, this is empty line. If first char is '#', this is comment line.*/
		if (ldevice == NULL || ldevice[0] == '#' || ldevice[0] == '!')
		continue;		/* Ignore empty lines & comment lines	*/
		if (ldevice [0] == '\0')
		{
		...
		//added by wzg 借用usr_data_info传递ld_name
		if (strlen(ldevice) < sizeof(map_entry->usr_data_info))
			strcpy(map_entry->usr_data_info, ldevice);
...
ST_CHAR *seps)		/* list of separator chars		*/
{
	...
	desc = get_next_string (&curptr, seps);
	...
	if (leaf && token3 && desc)
	{
		//added by wzg
		ST_CHAR	ld_name[256];
		strcpy(ld_name, map_entry->usr_data_info); //借用usr_data_info传递ld_name,先转存至本地
		...
		//added by wzg
		if(!strcmp(map_entry->usr_data_info, "NULL") && desc[0]) //只有需要使用的点才搜索 
		{
			ST_CHAR	h9000_name[256];

			if(SD_SUCCESS==FindH9000PointFromManDataMapList(ld_name, leaf, h9000_name))
			{
				SXLOG_ERR4 ("Finded h9000_name '%s'%s for %s %s in 'man_datamap.cfg'",
					h9000_name, desc, ld_name, leaf);

				if (strlen(h9000_name) < sizeof(map_entry->usr_data_info))
					strcpy (map_entry->usr_data_info, h9000_name);
				else 
				{
					SXLOG_ERR3 ("h9000_name '%s' too long for %s %s in 'man_datamap.cfg'",
						h9000_name, ld_name, leaf);
				}
			}
		}
	}
	...
	{
		SXLOG_ERR2 ("Invalid input at line %d in '%s'. Must contain 4 parameters.", line_num, datamap_cfg_filename);
...
FILE *fp_out_map)
{
	...
	ST_VOID *deadband_info=NULL;	/* deadband info for this leaf (NULL if not found)*/
	ST_CHAR h9000_name[256]="NULL";
	ST_CHAR desc[MAX_IDENT_LEN*2+1]="";
	//added by wzg
	extern int isGenH9000MapFlag;
...
*/ 
	...
	//added by wzg
	if(isGenH9000MapFlag)
	{
		if(SD_FAILURE==GetDoiLeafDesc(dom_name, leaf_name, desc))
			desc[0]= 0;
		else
		{
			if(SD_FAILURE==FindH9000PointByDoiDesc(dom_name, leaf_name, desc, h9000_name))
			{
				SXLOG_ERR3 ("ERROR: Fail to find h9000 point in db: %s %s \"%s\"",dom_name, leaf_name, desc);
				WriteManMapOutFile(dom_name, leaf_name, desc);
				strcpy(h9000_name, "NULL");
			}
		}
	}
	...
	{    
		...
		{		/* Mapped. Copy entry to output file.	*/
			if (fp_out_map)
				fprintf (fp_out_map, "%s\t%s\t\"%s\"\t\"%s\"\ttype=%s\n", 
				map_ld->ldevice,
				map_entry->leaf,
				h9000_name, //wzg map_entry->usr_data_info,
				desc,tdl_buf);
		}
		else
...
ST_UINT ld_count)			/* num array elements	*/
{
	...
	if (map_ld_array)
	chk_free (map_ld_array);	//DEBUG: done with this now, right??
}
/************************************************************************/
/*			datamap_cfg_read				*/
...
ST_CHAR *out_filename)		/* output cfg file name	*/
{
	...
	//added by wzg
	FILE	*fp_unmap_out;
	if(fp_unmap_out= fopen("man_datamapout.cfg", "w"))
		fclose(fp_unmap_out);
