...
ST_RET deadband_float_calculation (ST_FLOAT *pMag, DEADBAND_FLOAT *deadband)
{
	...
	//modified by wzg 因参数leaf没有被读过，其值没有被初始化，一直为0，无法进行死区计算，临时采用一下方式
	instMag_f = *deadband->instMag_f;  /* get current "instMag" value	*/
	mag = *pMag;	/* get current "mag" value	*/

	if(fabs(instMag_f-mag) >= 0.01 )
		*pMag = instMag_f;	/* modify caller's value	*/
