...
ST_INT array_size;
  ...
  array_size = dom->num_var_assoc - 2;
  if (array_size <= 0)
    {
    MVL_LOG_ERR0 ("Cannot compute 'Health$stVal' because not enough LN found.");
    mvlu_rd_prim_done (mvluRdVaCtrl, retcode);
    return;	/* do not even continue.	*/
    }
  ...
  if (retcode == SD_SUCCESS)
    ...
    for (j = 0; j < dom->num_var_assoc; j++)
      ...
      if (base_var_loop != base_var  &&			/* not LLN0	*/
        ...
        var_array [num_var] = mvlu_flat_var_create (base_var_loop, "ST$Health$stVal");
        num_var++;
        }
      }
  
    assert (num_var <= dom->num_var_assoc - 2);	/* make sure we counted correctly*/
...
ST_INT array_size;
  ...
  if (array_size <= 0)
    {
    MVL_LOG_ERR0 ("Cannot compute 'Health$q' because not enough LN found.");
    mvlu_rd_prim_done (mvluRdVaCtrl, retcode);
    return;	/* do not even continue.	*/
    }
  var_array =   (MVL_VAR_ASSOC **) chk_calloc (array_size, sizeof (MVL_VAR_ASSOC *));
  ...
  if (retcode == SD_SUCCESS)
    ...
    for (j = 0; j < dom->num_var_assoc; j++)
      ...
      if (base_var_loop != base_var  &&			/* not LLN0	*/
        ...
        {	/* this base var is "not" LLN0 or LPHD	*/
        scope_array [num_var] = &mvluRdVaCtrl->rdVaCtrl->va_scope;
        var_array [num_var] = mvlu_flat_var_create (base_var_loop, "ST$Health$q");
        num_var++;
        }
      }
...
ST_INT array_size;
  ...
  if (array_size <= 0)
    {
    MVL_LOG_ERR0 ("Cannot compute 'Health$t' because not enough LN found.");
    mvlu_rd_prim_done (mvluRdVaCtrl, retcode);
    return;	/* do not even continue.	*/
    }
  var_array =   (MVL_VAR_ASSOC **) chk_calloc (array_size, sizeof (MVL_VAR_ASSOC *));
  ...
  if (retcode == SD_SUCCESS)
    ...
    for (j = 0; j < dom->num_var_assoc; j++)
      ...
      if (base_var_loop != base_var  &&			/* not LLN0	*/
        ...
        {	/* this base var is "not" LLN0 or LPHD	*/
        scope_array [num_var] = &mvluRdVaCtrl->rdVaCtrl->va_scope;
        var_array [num_var] = mvlu_flat_var_create (base_var_loop, "ST$Health$t");
        num_var++;
        }
      }
