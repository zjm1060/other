...
ST_INT prim_count;
  ...
  /* If calculation of "prim_count" fails, return error.	*/
  prim_count = ms_rt_compute_prim_count (rt_ctxt->rt_ctrl->rt_first, rt_ctxt->rt_ctrl->rt_num);
  if (prim_count == 0)
    return (SD_FAILURE);
  ...
  /* ms_rt_size_calc also sets ..mvluTypeInfo.prim_count. If it doesn't	*/
  /* match, something is wrong.						*/
  if (prim_count != rt_ctxt->rt_ctrl->rt_first->mvluTypeInfo.prim_count)
    {
    MLOG_ERR2 ("Error building type. Primitive count %d does not match previously computed value %d",
               (ST_INT) rt_ctxt->rt_ctrl->rt_first->mvluTypeInfo.prim_count, prim_count);
    return (SD_FAILURE);
    }
  return (SD_SUCCESS);
  }
