...
#include "../mvl/usr/scl_srvr/61850_h9000.h"
...
ST_CHAR *pChar;
  for (pChar = name; *pChar != '\0'; pChar++)
    {
    if ((!(isalnum(*pChar))) && (*pChar != '_')  && (*pChar != '$'))
      {
      return (SD_FAILURE);
      }
    }
...
static ST_VOID scl_stop_parsing (SX_DEC_CTRL *sxDecCtrl, ST_RET errCode)
  {
  ...
  if (sxDecCtrl->reason == SX_ELEMENT_END)
    ...
    if (!strcmpi(str,"OSI-PSEL"))
      ...
      if (ret == SD_SUCCESS)
        ...
        if (strLen == 0 || asciiToSint32 (strOut, &int32_val) != SD_SUCCESS)
          {
          SXLOG_ERR1 ("Invalid AP-Invoke '%s'", strOut);
          scl_stop_parsing (sxDecCtrl, SX_USER_ERROR);
          }
        else
          {
      ...
      if (ret == SD_SUCCESS)
        ...
        if (strLen == 0 || asciiToSint32 (strOut, &int32_val) != SD_SUCCESS)
          {
          SXLOG_ERR1 ("Invalid AE-Invoke '%s'", strOut);
          scl_stop_parsing (sxDecCtrl, SX_USER_ERROR);
          }
        else
          {
...
ST_CHAR *desc, *h9000_name;
  ...
  if (sxDecCtrl->reason == SX_ELEMENT_START)
   ...
   /* end optional attributes */
	...
	//added by wzg
	ret = scl_get_attr_ptr (sxDecCtrl, "h9000_name", &h9000_name, required);
	if (ret == SD_SUCCESS)
		scl_ld->h9000_name = chk_strdup (h9000_name);	/* Alloc & copy h9000_name string	*/
...
ST_BOOLEAN required = SD_FALSE;
  ...
  if (sxDecCtrl->reason == SX_ELEMENT_START)
	...
	ret |= scl_get_attr_ptr (sxDecCtrl, "desc", &desc, required);//added by wzg
    if (ret != SD_SUCCESS)
      {
      return;
      }
	...
	//added by wzg
	if(doi_desc_info.num<MAX_DOI_DESC_NUM)
	{
		sprintf(doi_desc_info.entry[doi_desc_info.num].ld_name, "%s%s", 
			sclDecCtrl->iedName, sclDecCtrl->scl_ld->inst);
		sprintf(doi_desc_info.entry[doi_desc_info.num].ln_name, "%s%s%s", sclDecCtrl->scl_ln->prefix, 
			sclDecCtrl->scl_ln->lnClass, sclDecCtrl->scl_ln->inst);
		strcpy(doi_desc_info.entry[doi_desc_info.num].doi_name, name);
		strcpy(doi_desc_info.entry[doi_desc_info.num].desc, desc);//need string len assert
		doi_desc_info.entry[doi_desc_info.num].type= SIMPLE_DOI_TYPE;
		doi_desc_info.num++;
	}
	else
		SXLOG_ERR1 ("doi desc too much: max %d", MAX_DOI_DESC_NUM);
...
ST_BOOLEAN required = SD_FALSE;
  ...
  if (sxDecCtrl->reason == SX_ELEMENT_START)
	...
	//added by wzg
	if(doi_desc_info.num > 0) //确保已有DOI定义
	{
		if(doi_desc_info.num<MAX_DOI_DESC_NUM)
		{
			if(SIMPLE_DOI_TYPE==doi_desc_info.entry[doi_desc_info.num-1].type)
				doi_desc_info.entry[doi_desc_info.num-1].type= DOI_SDI_TYPE;//标记上条记录为包含SDI的DOI

			sprintf(doi_desc_info.entry[doi_desc_info.num].ld_name, "%s%s", sclDecCtrl->iedName, sclDecCtrl->scl_ld->inst);
			sprintf(doi_desc_info.entry[doi_desc_info.num].ln_name, "%s%s%s", sclDecCtrl->scl_ln->prefix, 
				sclDecCtrl->scl_ln->lnClass, sclDecCtrl->scl_ln->inst);
			strcpy(doi_desc_info.entry[doi_desc_info.num].doi_name, doi_desc_info.entry[doi_desc_info.num-1].doi_name); //取上条记录的DOI name
			strcpy(doi_desc_info.entry[doi_desc_info.num].sdi_name, name);
			strcpy(doi_desc_info.entry[doi_desc_info.num].desc, desc);//need string len assert
			doi_desc_info.entry[doi_desc_info.num].type= SIMPLE_SDI_TYPE;
			doi_desc_info.num++;
		}
		else
			SXLOG_ERR1 ("doi desc too much: max %d", MAX_DOI_DESC_NUM);
		...
		SXLOG_ERR2 ("SDI %s %s define not in DOI", name, desc);
...
SCL_DO *scl_do;
  ...
  //ret |= scl_get_attr_copy (sxDecCtrl, "desc", scl_do->desc, (sizeof(scl_do->desc)-1), required);
  /* If required attributes parsed OK, make sure "name" is legal.*/
  if (ret == SD_SUCCESS)
    {
