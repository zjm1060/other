...
class _CALLH9000DLL_EXPORT CallIPM:QObject
{
	Q_OBJECT
public:
	CallIPM();//带路径名称
	...
	char* openDbin(QString pathname);
	void ReadGdbaseData ();
	void	test();
	void v4_freadRcd ( NEW_GBRCD *rcd);
	void freadBckRcd ( BACKRCD *rcd);
	void v4_freadLnkRcd ( NEW_LINKRCD *rcd);
	int Openfile_DynamicsTest ();
	int	version4_flag;
	INFO_BUFF		info;
