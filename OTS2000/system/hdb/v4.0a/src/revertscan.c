...
{	
	...
	if(-1==InitSysLog(H9000_MODULE_REVERTSCAN, "RevertScan"))
	{
	...
	for(;;)
	{
		if( ConnectHDB(&lda_rt,&cda_rt,LoginStr)==FALSE)
		{	
	...
	if(SelectIdFromHdb(cda_rt)== FALSE)
	{
	...
	{	
		...
		if(shm_hdb_addr->data_revert.ycnum>=1 && shm_hdb_addr->data_revert.ycnum<MAX_REVERT_NUM)
		{
			...
			{	
				...
				result = SelectRevertFromHdb(cda_rt,&(shm_hdb_addr->data_revert),&(yc_data[curid]));
				if( result == FALSE )
				{	
		...
		if(shm_hdb_addr->data_revert.yxnum>=1 && shm_hdb_addr->data_revert.yxnum<MAX_REVERT_NUM)
		{
			for(curid=0;curid<shm_hdb_addr->data_revert.yxnum;curid++)
			{
				...
				result = SelectYxRevertFromHdb(cda_rt,&(shm_hdb_addr->data_revert),&(yx_data[curid]));
				if( result == FALSE )
				{	
