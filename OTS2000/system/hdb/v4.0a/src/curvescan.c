...
// #ifdef	WINDOWS_OS
// 	HANDLE handle=CreateMutex(NULL, TRUE, "CURVESCAN");
...
//	SetLogLevel(argc, argv);
	...
	for(;;)
	{
		if( ConnectHDB(&lda_cv,&cda_cv,LoginStr)==FALSE )
		{	
	...
	for(;;)
	{
		...
		for(CrtID=0;CrtID<MAX_CRT_NUM;CrtID++)
		{
			...
			else
			{
				...
				else if(rs[CrtID].querytype==0 || rs[CrtID].querytype == 2)
				{
					MySleep(200);
					continue;
				}
			...
			if (rs[CrtID].querytype == 2)	//发电计划曲线
			{
				if (rs[CrtID].num < 1)
				{
					continue;
				...
				for(curid=0; curid<rs[CrtID].num; curid++)
				{
					if (curid == 0)
					{
						result = SelectGenPowerPlanCurveFromHdb(cda_cv, curid, &rs[CrtID]);
					}
					else if (curid == 1)
					{
						temp=rs[CrtID].interval;
						rs[CrtID].interval=ONE_MIN;
						result = SelectYcRecFromHdb(cda_cv, curid, &rs[CrtID]);
						rs[CrtID].interval=temp;
					}
					else
					{
						break;
					...
					if( result == FALSE )
					{	
						printf("SelectGenPowerPlanCurveFromHdb( curid= %d ) error !\n",curid);
						HdbConn = FALSE;
					}
					else
					{	
						printf("\nSelectGenPowerPlanCurveFromHdb( curid= %d ) successfully !\n",curid);
						rscount=rs[CrtID].seg[curid].rscount;
						if(rscount>0)
						{
							ChgFlag[CrtID]=1;
						}
						printf("==== rs return count =  %d \n",rscount);
#ifdef DEBUG
						for(i=0;i<rscount;i++)
						{
					...
					if(ChgFlag[CrtID] == 1)
					{
						if (curid == 0)
						{
							InterGenPowerPlanCurveData(&rs[CrtID], CrtID, curid, rs[CrtID].interval);
						}
						else if (curid == 1)
						{
							InterGenPowerPlanCurveData(&rs[CrtID], CrtID, curid, ONE_MIN);
						}
					}
				}
...
//				if(CrtID != 0)	continue;
				...
				{	
					...
					{
						result = SelectCurveFromHdb(cda_cv,curid,&rs[CrtID]);
					}
					if( result == FALSE )
					{	
	...
	if( DisconnectHDB(&lda_cv,&cda_cv)==FALSE )
...
int InitCurve(struct hdb_curve *p_rs,int CrtID)
{
	...
	p_rs->finishedFlag=shm_hdb_addr->data_curve[CrtID].finishedFlag;
	...
	{
		if (p_rs->querytype != 2 || (p_rs->querytype == 2 && i==1))
		{
		...
		{
			p_rs->seg[i].stnid=shm_hdb_addr->data_curve[CrtID].seg[i].stnid;
		}
	}	
...
{
	struct tm endtime, starttime, lasttime, curtime;
	time_t	ret1,ret2;
	int i, j;
	int rscount;
	...
	for (i=0; i<rscount;)
	{
		StrToTime(p_rs->seg[curid].timestr[i], &curtime, interval, 0);
		if((ret1=mktime(&curtime)) == (ret2=mktime(&lasttime)) )
		{
			CurveData[CrtID][curid][j]=p_rs->seg[curid].data[i];
			CurveTime[CrtID][curid][j]=curtime;
			
			DateAdd(&lasttime, interval);
			i++;
			j++;
		}
		else if (ret1 > ret2)
		{
			...
			CurveTime[CrtID][curid][j]=lasttime;
	...
	if (rscount > 0)
	{
		while ((ret1=mktime(&lasttime)) <= (ret2=mktime(&endtime)))
		{
			if (j>0)
			{
				CurveData[CrtID][curid][j]=CurveData[CrtID][curid][j-1];
