...
int _tmain(int argc, char * argv[])
{
	...
	QSettings settings(myfile,QSettings::IniFormat);
...
//	app_id = HDB_APP;
	...
	QStringList proc_list = settings.childKeys();
	for (int i = 0; i < proc_list.size(); ++i)
	{
		QString procName = settings.value(proc_list.at(i)).toString();
		if (!procName.contains(".exe"))
		{
			procName = procName.append(".exe");
		}
		printf("pause process name=%s\n",procName.toLocal8Bit().data());
		HANDLE ProcessHandle = 0;
		proc_map.insert(procName,ProcessHandle);
	}
...
void * ThreadTrain( void * arg )
{
	...
	for(;;)
	{
		...
		if( GetLocalTrainInfo(&train)==TRUE )
		{
			...
			if( train.task_status == 1 && shm_ots_addr->data_task.task_status != 0)
			{
				if (groups_info.groups_id == -1 || groups_info.groups_role == 1)
				{
				...
				{
					sprintf(sms_str,"此学员不是组长，不能接收任务！");
					BdSms(sms_str);
					data_train.task_status = 0;
					flag_train.task_status = TRUE;
				}
...
void * ThreadScene( void * arg )
{
	...
	for(;;)
	{
		if( shm_ots_addr->data_scene.requestflag == 1 )
		{
			...
			data_scene.scene_id = GetInexistenceIdInHdb(TABLE_SCENE);
