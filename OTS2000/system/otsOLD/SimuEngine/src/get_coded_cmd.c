...
int GetSeqNameFromID(OTS_LINK_DEF *lnk)
{
	...
	int i;
	sprintf(buf_id,"%d.%d.%d.%d.%d.%d",lnk->cmd_code.stn_id,lnk->cmd_code.dev_id,lnk->ptr.data_type,lnk->ptr.pt_id,
		lnk->cmd_code.type_id,lnk->cmd_code.opr);
	sprintf(buf_id_type_any,"%d.%d.%d.%d.%d.%d",lnk->cmd_code.stn_id,lnk->cmd_code.dev_id,lnk->ptr.data_type,lnk->ptr.pt_id,
		0,lnk->cmd_code.opr);
