...
void CLocalOprate::setupUi()
{
	...
	buttonExit = new QPushButton(QString::fromLocal8Bit("退出"));
	DB_FMT data;
	...
	else
	{
		for(int i=0;i<data.rownum;i++)
		{
			...
			scrollArea->verticalScrollBar()->setSingleStep(181);
			scrollArea->verticalScrollBar()->setPageStep(181*2);
			...
			if( SelectRecs(&data_opt) != TRUE)
			{
				HdbLogWrite(data_opt.sql_str,data_opt.sql_error);
	...
	tabWidget->setMinimumHeight(385);
	...
	labelName->setStyleSheet(QString::fromLocal8Bit("QLabel{color: rgb(255, 0, 0);font: 10pt 宋体;}"));
	buttonEnter->setText(QString::fromLocal8Bit("执行此操作"));
	buttonEnter->setAutoRaise(true);
	buttonEnter->setStyleSheet(QString::fromLocal8Bit("QToolButton{color: rgb(0, 0, 255);font: 16pt 宋体;}"));
	QIcon icon;
	buttonExit->setMaximumWidth(75);
	...
	editNameFilter->setMinimumWidth(300);
	editNameFilter->setMaximumWidth(300);
	...
	layout->addWidget(buttonEnter,1,0,Qt::AlignCenter);
	layout->addLayout(layoutFilter,1,1,Qt::AlignRight);
	setLayout(layout);
...
void CLocalOprate::optRun()
{
	...
	data.seg.seg_record.user_id = loginUser->login_user_id;
	...
	WriteRec(m_opt_name.toLocal8Bit().data());
	QMessageBox().information(this,QString::fromLocal8Bit("提示!"),QString::fromLocal8Bit("执行%1成功!").arg(m_opt_name),QMessageBox::Ok);
	FreeDbData(&data,TABLE_RECORD);
}
