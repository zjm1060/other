...
int init(void)
{
	...
	while(fgets(tmp,sizeof(tmp),configFile)!=NULL)
	{
		...
		if (protocolCount>0)
		{
			...
			else if (strcmp(item,"TN_NODE_ID")==0)
			{
				if(1!=sscanf(content, "%d", &protocolRec[protocolPosition].gcom_tn_node_id))
				{
					SendTraceMsg(0,0,"GCOM: TN node id error: %s",tmp);
					fclose(configFile);
					return -1;
				}
				else
					SendTraceMsg(0,0,"GCOM: TN node id: %d", protocolRec[protocolPosition].gcom_tn_node_id);
			}
			else if (strcmp(item,"DSV_NODE_ID")==0)
			{
				if(1!=sscanf(content, "%d", &protocolRec[protocolPosition].gcom_dsv_node_id))
				{
					SendTraceMsg(0,0,"GCOM: DSV node id error: %s",tmp);
					fclose(configFile);
					return -1;
				}
				else
					SendTraceMsg(0,0,"GCOM: DSV node id: %d", protocolRec[protocolPosition].gcom_dsv_node_id);
			}
		}
	}
	fclose(configFile);
...
// 			}
			if(isGCOMMulticasePackage(pkt_data, header->len))
			{
				//check source node id
				if(pkt_data[10]==protocolRec[protocolNo].gcom_tn_node_id)
					protocolRec[protocolNo].shareMemory->header.gcom_tn_hb_recv_flag++;
				else if(pkt_data[10]==protocolRec[protocolNo].gcom_dsv_node_id)
					protocolRec[protocolNo].shareMemory->header.gcom_dsv_hb_recv_flag++;
			}
			else
				handleGCOM(pkt_data,header->len);
...
int isGCOMMulticasePackage(const u_char *pkt_data, int length)//multicast destination address< 01, 00, 23, 00, 00, 01 >.
{
	...
	{
		SendTraceMsg(0,0,"GCOM: HeartBeat package from node %d", pkt_data[10]);
		return TRUE;
	}
	else
		return FALSE;
...
// 		SetPtValueIostById(&point, new_value, iost, update_time, RW_RT_VALUE)		
		BroadCastIndPtState(point, 0, state);
		SendTraceMsg(0,0,"Broadcast %s state %d", entry_name, state);
	}	
}
...
void CheckSnifferActive(void)
{
	...
	for(;;)
	{
		...
		for(i=0;i<protocolCount;i++)
		{
			if(!strcmp(protocolRec[i].protocolName, "VIP"))
			{
				if(protocolRec[i].shareMemory->header.writePoint==readPoint[i]) //no data package received
			...
			else if(!strcmp(protocolRec[i].protocolName, "GCOM"))
			{
// 				SendTraceMsg(0,0,"GCOM: writePoint= %d, readPoint= %d", protocolRec[i].shareMemory->header.writePoint, readPoint[i]);
// 				SendTraceMsg(0,0,"GCOM: TN recv= %d, check= %d", protocolRec[i].gcom_tn_hb_recv_flag, protocolRec[i].gcom_tn_hb_check_flag);
// 				SendTraceMsg(0,0,"GCOM: DSV recv= %d, check= %d", protocolRec[i].gcom_dsv_hb_recv_flag, protocolRec[i].gcom_dsv_hb_check_flag);
				if(protocolRec[i].shareMemory->header.writePoint!=readPoint[i]) //received event package
				{
					UpdateSnifferState(protocolRec[i].protocolName, 0);
				}
				else //no event package received
				{
					if(protocolRec[i].shareMemory->header.gcom_tn_hb_recv_flag==protocolRec[i].shareMemory->header.gcom_tn_hb_check_flag) //no tn heartbeat received
						UpdateSnifferState(protocolRec[i].protocolName, 1);
					else if(protocolRec[i].shareMemory->header.gcom_dsv_hb_recv_flag==protocolRec[i].shareMemory->header.gcom_dsv_hb_check_flag) //no dsv heartbeat received
						UpdateSnifferState(protocolRec[i].protocolName, 1);
					else //ok
						UpdateSnifferState(protocolRec[i].protocolName, 0);
				}
				readPoint[i]= protocolRec[i].shareMemory->header.writePoint;
				protocolRec[i].shareMemory->header.gcom_tn_hb_check_flag= protocolRec[i].shareMemory->header.gcom_tn_hb_recv_flag;
				protocolRec[i].shareMemory->header.gcom_dsv_hb_check_flag= protocolRec[i].shareMemory->header.gcom_dsv_hb_recv_flag;
			}
