...
/* --------  ---  ------   -------------------------------------------  */
/* 05/17/07  zy     22     put H9000 application code in u_mic_dv_ind   */
/*                         and u_mic_dv_end                             */
...
ST_INT		ret;
  ...
  if (dataEntered)
    ...
    if (ret==SD_SUCCESS)
      ...
      prtLocalDataValue (pstMitDataValue->dvName, (union _mi_dv_data_union *)pData,
...
ST_INT		ret;
  ...
  if (dataEntered)
    ...
    if (ret==SD_SUCCESS)
      ...
      getLocalDataValue (pstMitDataValue->dvName, (union _mi_dv_data_union *)pData,
                         pstMitDataValue->dvType);
...
/* modified by yyp for H9000   2011.09.23				*/
		      ...
		      ST_VOID *data_buf, ST_INT mi_type)
{
	...
	MVL_LOG_ERR0 ("############## yyp i get one ind");
	printf("\n############## yyp in u_mic_dv_ind, name=%s, type=%d\n",dv->dv_name,mi_type);	
	...
	case MI_TYPEID_REAL_Q:
		...
		printf("\n############## value= %f\n", drq->Value);	
		ClientMEDataProc (tag, drq->Value, drq->Flags);
	...
	case MI_TYPEID_REAL_Q_TIMETAG:
		...
		ClientMEDataProc (tag, drqt->Value, drqt->Flags);
		break;
	...
	case MI_TYPEID_STATE_Q_TIMETAG:
		...
		ClientSWDataProc (tag, dsqt->Flags);
		break;
	...
	case MI_TYPEID_REAL_EXTENDED:
		...
		ClientMEDataProc (tag, dre->Value, dre->Flags);
		break;
	...
	case MI_TYPEID_STATE_EXTENDED:
		...
		ClientSWDataProc (tag, dse->Flags);
		break;
			    ...
			    MIC_DSTS *mic_dsts, MIC_DSTS_STO *sto)
{
	...
	/* Broadcast received client ME and SW Data for update H9000 Realtime Database */
	ClientDataBD ();
}
