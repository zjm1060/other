...
/************************************************************************/
/*                                                                      */
/* MODULE NAME : iccp_s_main.c (originally mit_main.c)                   	*/
...
void main ( int argc, char *argv[] )
{
	...
	sprintf(infobuf,"H9000通信系统: TASE.2协议(iccp_s_main) Start!\n");
	printf("%s",infobuf);
	WriteDspInfo(infobuf);
...
/*	m_check_list_enable= SD_TRUE; */
	...
	printf("%s",infobuf);
	WriteDspInfo(infobuf);
	if (mitConfigure_xml(MIT_XML_MIT_CFG_FILE_S, &mvlCfg) != SD_SUCCESS)
	{    
		MIT_LOG_ERR0("Configuration File Errors - check log file.");
		sprintf (infobuf,"iccp_s_main: - Configuration File Errors - check log file.\n");
		printf("%s",infobuf);
		WriteDspInfo(infobuf);
		exit(1);
	}
...
void SaveAssocStatus(ST_LONG nTimsav)
{
	...
	for (nCnt = 0; nCnt < DC_cnt; ++nCnt)
	{
		...
		if (sAssocDat[nCnt][0].pAssocInf != (MIT_ASSOC_INFO *)NULL)
		{
			...
			{
				sprintf(infobuf,"iccp: - ASSOCIATION '%s' :0\tNOT ACTICE\t000000\n", sAssocDat[nCnt][0].strAssocNam);
				printf("%s",infobuf);
				WriteDspInfo(infobuf);
			}
			else
			{
				nTASE2_Sta[nCnt] = 1;
				sprintf(infobuf,"iccp: - ASSOCIATION '%s' :0\tACTIVE\t\t111111\n", sAssocDat[nCnt][0].strAssocNam);
				printf("%s",infobuf);
				WriteDspInfo(infobuf);
			}
		}
		if (sAssocDat[nCnt][1].pAssocInf != (MIT_ASSOC_INFO *)NULL)
		{
			...
			{
			/*	nTASE2_Sta[nCnt] = 0; */
				sprintf(infobuf,"iccp: - ASSOCIATION '%s' :1\tNOT ACTICE\t000000\n", sAssocDat[nCnt][1].strAssocNam);
				printf("%s",infobuf);
				WriteDspInfo(infobuf);
			}
			else
			{
				nTASE2_Sta[nCnt] = 1;
				sprintf(infobuf,"iccp: - ASSOCIATION '%s' :1\tACTIVE\t\t111111\n", sAssocDat[nCnt][1].strAssocNam);
				printf("%s",infobuf);
				WriteDspInfo(infobuf);
			}
		}
		if(nTASE2_Sta[nCnt] == 1)
		{
			UpdateChannelFlag(nCnt,(unsigned short)0);
			/* GWRtuAllSend();	*/	/* Gateway data Send All */
		}
		else if(nTASE2_Sta[nCnt] == 0)
			UpdateChannelFlag(nCnt,(unsigned short)1);
	}
	if (pFil != (FILE *)NULL)
	{
...
void Tase2_Service( void )
{
	...
	iccp_s_timeout_flag = 0;
	iccp_s_start_assoctions = 0;
	signal( SIGALRM, Net_Timeout);
/*
	SendSinglePointInformCyc();
*/
	SendDoublePointInformCyc();
	SendMeasureValueNCCyc();
	...
	for(;;)
	{
		...
		if ((nTim - nCycTimSav) > 60)	/*do send all data every 300 seconds */
		{
			sprintf(infobuf,"H9000通信系统: Cyclic Data Send Start!\n");
#ifdef DEBUG
			printf("%s",infobuf);
#endif
			WriteDspInfo(infobuf);
		...
		if ((nTim - nSendTimSav) > 1)  /*do send data every 1 seconds */
		{
			sprintf(infobuf,"H9000通信系统: Change Data Send Start!\n");
#ifdef DEBUG
			printf("%s",infobuf);
#endif
			WriteDspInfo(infobuf);
...
#ifdef DEBUG_SISCO
	if (logcfgx(MIT_XML_MIT_LOG_FILE_S) != SD_SUCCESS)
	{
/*
...
static ST_VOID init_acse()
{
  ...
  /* Added for Marben Stack */
	...
	ret = osicfgx(MIT_XML_OSI_CFG_FILE_S, &mvlCfg);
	if (ret != SD_SUCCESS)
	{
