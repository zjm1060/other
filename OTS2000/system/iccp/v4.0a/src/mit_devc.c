...
/* --------  ---  ------   -------------------------------------------  */
/* 04/12/07  zy     7      Interlock judge added for cmd and setpoint   */
/* 12/12/06  zy     7      Transplanted for H9000 TASE.2 application    */
...
ST_VOID *findDevice (MIT_DEVICE_REF *pstDeviceSet, ST_CHAR *devName);
 ST_VOID    DisplayDeviceYYP (ST_VOID);
...
static ST_VOID displayTagFlags (ST_UCHAR flag)
{
  ...
  {
    sprintf (infobuf, "OPEN_CLOSE_INHIBIT, " );
#ifdef DEBUG
	printf("%s",infobuf);
#endif
	WriteDspInfo(infobuf);
...
#endif
	WriteDspInfo(infobuf);
  sprintf (infobuf,"DeviceSetTag Resp for %s, result=%d", pDevice->device_name, miReq->result);
#ifdef DEBUG
	printf("%s",infobuf);
#endif
	WriteDspInfo(infobuf);
...
MIT_DEVICE *pstCurDevice;
  ...
  printf ("Device Select for %s, send default response.\n", 
           pstCurDevice->devName);
...
{   
  ...
  ST_INT        nDst;
  ST_CHAR    	iccp_devName[MAX_ID_LEN];
  ST_INT16     	cmdstat;
  float			setValue;

	pstCurDevice = (MIT_DEVICE *)dev_ind_ctrl->mis_device->access_info;
	strcpy(iccp_devName, pstCurDevice->devName);
	nDst = 0;

	sprintf(infobuf,"\n---- iccp: - Device Operate for %s\n", iccp_devName);
#ifdef DEBUG
	printf("%s",infobuf);
#endif
	WriteDspInfo(infobuf);
	...
	case MI_TYPEID_DEV_COMMAND:
		...
		sprintf(infobuf,"\n######iccp: - TASE2 received command '%s' Operate = %d\n",
			iccp_devName, cmdstat);
		printf("%s",infobuf);
		WriteDspInfo(infobuf);

		if(ICCPDeviceOperateCmd(nDst,iccp_devName, cmdstat) != 1)
		{
			sprintf(infobuf,"\n*****iccp: - Refused TASE2 command '%s' Operate = %d\n",
				iccp_devName, cmdstat);
			printf("%s",infobuf);
			WriteDspInfo(infobuf);
...
//		sSigTASE2_W.nValBits = ((*pCommand >> 6) & 0x03);
//		Modified by zy 2003.7.6
		sSigTASE2_W.nValBits = *pCommand;
		sSigTASE2_W.nValidity = ((*pCommand >> 4) & 0x03);
*/
		break;

	case MI_TYPEID_DEV_DISCRETE:
		printf("iccp: - TASE2 received discrete value '%s'\n", pstCurDevice->devName);
		pDiscrete = (MI_CONTROL_SETPOINT_DISCRETE *) dev_ind_ctrl->data_buf;
		sprintf(infobuf,"$$ DEV_DISCRETE Operate = %d, MIS_SEND_UNAVAIL_RESP\n", *pDiscrete);
		printf("%s",infobuf);
		WriteDspInfo(infobuf);
		return(MIS_SEND_UNAVAIL_RESP);
		break;
	...
	case MI_TYPEID_DEV_REAL:
		...
		sprintf(infobuf,"\n######iccp: - TASE2 received setpoint '%s' Value = %f\n",
				iccp_devName,setValue);
		printf("%s",infobuf);
		WriteDspInfo(infobuf);

		if(ICCPSetPointCmd(nDst,iccp_devName, setValue)!=1)
		{
			sprintf(infobuf,"\n*****iccp: - Refused TASE2 setpoint '%s' Value = %f\n",
					iccp_devName,setValue);
			printf("%s",infobuf);
			WriteDspInfo(infobuf);
			return(MIS_SEND_UNAVAIL_RESP);
	...
	default:
		printf(infobuf,"$$ Unknown OperateValue Type. MIS_SEND_UNAVAIL_RESP\n");
		printf("%s",infobuf);
		WriteDspInfo(infobuf);
		return(MIS_SEND_UNAVAIL_RESP);
		break;
...
MI_TAG_VALUE *pTagValue;
	...
	pstCurDevice = (MIT_DEVICE *)dev_ind_ctrl->mis_device->access_info;
	sprintf (infobuf,"Device Set Tag for %s, send default response.\n", 
		pstCurDevice->devName);
#ifdef DEBUG
	printf("%s",infobuf);
#endif
...
MIT_DEVICE *pstCurDevice;
  ...
  pstCurDevice = (MIT_DEVICE *)dev_ind_ctrl->mis_device->access_info;
  sprintf (infobuf,"Device Get Tag for %s, send default response.\n", 
          pstCurDevice->devName);
#ifdef DEBUG
	printf("%s",infobuf);
#endif
...
ST_BOOLEAN bFound = SD_FALSE;
int count;
   printf("#############kkkk DisplayDeviceYYP\n");
  /* Get the association name and see if it's active			*/

  pstAssociationInfo = getAssociationName (buffer);
    pstMitMicVcc = findMitMicVcc (pstAssociationInfo->remoteName);
  printf("i am in 1111  pstMitMicVcc = %d\n", pstMitMicVcc );
  pstCurDevice = pstMitMicVcc->pstDeviceSet;
   printf("i am in 222  pstCurDevice = %d\n", pstCurDevice ); 
  count=0;
  while (pstCurDevice && !bFound && count<3)
    {
      printf("#############yyp name=%s  device = %d,devset=%d\n",pstCurDevice->devName,pstCurDevice->pstDevice,pstCurDevice);
      pstCurDevice = (MIT_DEVICE_REF *) list_get_next 
                 ((ST_VOID *)pstDeviceSet, (ST_VOID *)pstCurDevice);
      count++;
