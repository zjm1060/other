...
*/
	if( rw_flag & RW_UPDATE_TIME )
	{
		...
		case	TIME_TYPE:
			common_ptr.var_ptr->time.update_time= change_time;
			break;
...
/*modified by wzg 20080818
			...
			if(value.anlg.status.lmt_state==ANA_ALM_HI_RECOVER 
				|| value.anlg.status.lmt_state==ANA_ALM_LO_RECOVER)
				value.anlg.status.lmt_state= ANA_ALM_NORMAL;
			else if(value.anlg.status.lmt_state==ANA_ALM_HH_RECOVER)
				value.anlg.status.lmt_state= ANA_ALM_HI_WARN;
			else if(value.anlg.status.lmt_state==ANA_ALM_LL_RECOVER) 
				value.anlg.status.lmt_state= ANA_ALM_LO_WARN;
			Swap2Byte(&value.anlg.status);
			common_ptr.var_ptr->anlg.value.status= value.anlg.status;
		}
		else
...
int SetTimePtVarById( POINTER *point, ENTRY_TIME_VAL time_var )
{
	...
	SwapDateTimeByte(&time_var.update_time);
	Swap4Byte(&time_var.value_sec);
	Swap4Byte(&time_var.coords.x);
	Swap4Byte(&time_var.coords.y);
	Swap4Byte(&time_var.coords.z);
	Swap2Byte(&time_var.value_ms);
	Swap2Byte(&time_var.status);
	common_ptr.var_ptr->time= time_var;
	common_ptr.fixed_ptr->update_src= point->update_src;
	return 0;
}
