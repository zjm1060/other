...
int GetEntryLongnameById( POINTER *point, char *entryLongname )
{
	...
	if(GetExLangState())
		strcpy( entryLongname, common_ptr.fixed_ptr->entryExLongname );
	else
#endif
...
int	GetPtIoGroupCommentById( POINTER *point, char *ioGroupComment )
{
	...
	if(io_group>=MAX_IOGROUP_NUM)
	{
		SendWarnMsg(1, "ioGroup %d dose not support ioGroupComment, only 0~255: pt %d.%d.%d.%d",
			io_group, point->stn_id, point->dev_id, point->data_type, point->pt_id);
		return -1;
	}
	
	if(-1==GetLcuIoGroupInfoById(point->stn_id, point->dev_id, (UCHAR)io_group, &ioGroup_info))
	{
		ioGroupComment[0]=0;
		return -1;
	}
...
#ifdef EXTEND_LANGUAGE_SYSTEM
	if(GetExLangState())
		strcpy(ioGroupComment, ioGroup_info.exComment);
	else
#endif
		strcpy(ioGroupComment, ioGroup_info.comment);
...
int	GetPtIoGroupCommentByNameStr( char *ptNameStr, char *ioGroupComment )
{
	...
	if(-1==GetPtIdByNameStr(ptNameStr, &pt))
		return -1;
	return GetPtIoGroupCommentById(&pt, ioGroupComment);
}
...
int	GetPtIoGroupStatusById( POINTER *point, UINT *ioGroupStatus )
{
	...
	*ioGroupStatus= ioGroup_info.ioGroupStatus;
	return 0;
}
...
int	GetPtIoGroupStatusByNameStr( char *ptNameStr, UINT *ioGroupStatus )
{
	...
	if(-1==GetPtIdByNameStr(ptNameStr, &pt))
		return -1;
	return GetPtIoGroupStatusById(&pt, ioGroupStatus);
}
...
int GetPairPtById( POINTER *point, POINTER *pair_pt )
{
	...
	*pair_pt= common_ptr.fixed_ptr->pair_pt;
	...
	if(pair_pt->stn_id==0 || pair_pt->dev_id==0 || pair_pt->data_type==0)
		return -1;
	return 0;
}
...
int	GetPtUpdateTimeById( POINTER *point, DATE_TIME *update_time )
{
	...
	case	TIME_TYPE:
		*update_time= common_ptr.var_ptr->time.update_time;
		break;
...
#ifdef EXTEND_LANGUAGE_SYSTEM
	if(GetExLangState())
		strcpy(str, db_ptr->msg_def[procType].group[alm_sel].Exdef[state]);
	else
#endif
...
int GetPtCmdMsgStrById( POINTER *point, UCHAR cmdType, USHORT cmd_opt, CHAR *str )
{
	...
	if(cmdType==C_DATA_POLLING||cmdType==C_LCU_PARAM || cmdType==DPS_IMP_PARAM_SET
		|| cmdType==DPS_ANA_PARAM_SET ||cmdType==DPS_IOATTR_SET ||  cmdType==DPS_IOST_SET )
		cmd_sel= 0;   // modify by chi.hl 2012.2.20
	else
	{
		if( -1==GetCommonPtrById(point, &common_ptr) )
			return -1;
...
#ifdef EXTEND_LANGUAGE_SYSTEM
			if(GetExLangState())
				strcpy(str, db_ptr->cmd_msg[i].group[cmd_sel].Exdef[cmd_opt]);
			else
#endif
...
int	GetTimePtStatusById( POINTER *point, COMMAND_STATUS *status )
{
	...
	{
		sprintf(err_str, "Not time type: %d", point->data_type);
		Err_log(err_str);
		return -1;
	}
	...
	*status= common_ptr.var_ptr->time.status;
	Swap2Byte(status);
	return 0;
}
...
int	GetTimePtStatusByNameStr( char *ptNameStr, COMMAND_STATUS *status )
{
	...
	if( common_ptr.point.data_type != TIME_TYPE )
	{
		sprintf(err_str, "Not time type: %d", common_ptr.point.data_type);
		Err_log(err_str);
		return -1;
	}
...
int	GetTimePtSecValueById( POINTER *point, GULONG *second )
{
	...
	*second= common_ptr.var_ptr->time.value_sec;
	Swap4Byte(second);
	return 0;
}
...
int	GetTimePtMsValueById( POINTER *point, USHORT *ms )
{
	...
	*ms= common_ptr.var_ptr->time.value_ms;
	Swap2Byte(ms);
	return 0;
}
...
int	GetTimePtUsrNameById( POINTER *point, char *usr_name )
{
	...
	strcpy(usr_name, common_ptr.var_ptr->time.usr_name);
	return 0;
}
...
int	GetTimePtHostIdById( POINTER *point, UCHAR *host_id )
{
	...
	*host_id= common_ptr.var_ptr->time.host_id;
	return 0;
}
...
int	GetTimePtCoordinateById( POINTER *point, TIME_PT_COORDS *coordinate )
{
	...
	coordinate->x= common_ptr.var_ptr->time.coords.x;
	coordinate->y= common_ptr.var_ptr->time.coords.y;
	coordinate->z= common_ptr.var_ptr->time.coords.z;
	Swap4Byte(&coordinate->x);
	Swap4Byte(&coordinate->y);
	Swap4Byte(&coordinate->z);
	return 0;
}
...
int GetPtDataByStr( char *nameStr, FLOAT *fValue, UINT *chan_fail, POINTER *point )
{
	...
	char	err_str[128], ptNameStr[256], *ptr=0;
	int		i=0, j=0;

	if(!nameStr[0])
	{
		sprintf(err_str, "GetPtDataByStr: parameter is empty string");
		Err_log(err_str);
		return -1;
	}
	...
	if(!ptr)
	{
		...
		case	OUTS_TYPE:
		case	SYSMSG_TYPE:
			ReadEntryById(&common_ptr.point, &common_all);
			*fValue= (FLOAT)common_all.var.ind.status.state;
			*chan_fail= common_all.var.ind.status.chan_fail;
			break;
		...
		case	CALC_TYPE:
			ReadEntryById(&common_ptr.point, &common_all);
			*fValue= (FLOAT)(common_all.var.anlg.value.data/1000.);
			*chan_fail= common_all.var.anlg.value.status.chan_fail;
			break;
		
		case	IMP_TYPE:
			ReadEntryById(&common_ptr.point, &common_all);
			*fValue= (FLOAT)common_all.var.imp.entry.d_accum;
			*chan_fail= common_all.var.imp.entry.status.chan_fail;
			break;
			
		default:
			sprintf(err_str, "Unsupported data type: %s", nameStr);
			Err_log(err_str);
			return -1;
		}
		return 0;
	...
	if( common_ptr.point.data_type != CHARA_TYPE )
	{
		sprintf(err_str, "Only chara type is supported: %s", nameStr);
		Err_log(err_str);
		return -1;
	}
	
	*chan_fail= 0;
	if(!strcmp(ptr, "MAX_VALUE"))
	{
		*fValue= common_ptr.var_ptr->chara.chara_data.max_value;
		Swap4Byte(fValue);
	}
	else if(!strcmp(ptr, "MIN_VALUE"))
	{
		*fValue= common_ptr.var_ptr->chara.chara_data.min_value;
		Swap4Byte(fValue);
	}
	else
	{
		sprintf(err_str, "Unsupported chara value type: %s", nameStr);
		Err_log(err_str);
		return -1;
	}
