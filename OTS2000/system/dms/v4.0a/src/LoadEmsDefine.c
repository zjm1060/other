...
BOOL IsFileEndStr(char *str)
{
	return !strcmp(str, "END") || !strcmp(str, "END\n");
}
...
/*	printf("\nconfig_info size: %d", sizeof(config_info));*/
	...
	while( NULL != fgets(str, sizeof(str), fp) )
	{
		if( (strstr(str, "[") && strstr(str, "]")) || IsFileEndStr(str) )	
			break;
	...
	while( NULL != fgets(str, sizeof(str), fp) )
	{
		...
		if(strbuf[0][0]==0 || strbuf[0][0]=='!' || strbuf[0][0]=='#')
			continue;
	...
	while( NULL != fgets(str, sizeof(str), fp) )
	{
		...
		for(j=0; j<(str_num-1); j++)
		{
			...
			if(strbuf[j+1][0]==0 || strbuf[j+1][0]=='!' || strbuf[j+1][0]=='#')
				continue;
...
/*deleted by wzg 20080909 unused*/
	/* KEYWORD=="TIME_SERVER" 
...
*/
	...
	while( NULL != fgets(str, sizeof(str), fp) )
	{
		...
		if(config_info.plc_mode_num>=MAX_ALL_LCU_NUM)
		{
			sprintf(err_str, "Too much plc in %s: %s", keyword, str);
			Err_log(err_str);
			fclose(fp);
		...
		config_info.plc_mode[config_info.plc_mode_num].stn_id= stn_id;
		config_info.plc_mode[config_info.plc_mode_num].dev_id= dev_id;
		config_info.plc_mode[config_info.plc_mode_num].mode= (UCHAR)value;
		config_info.plc_mode_num++;
	...
	config_info.info_rec_table= FALSE;
	...
	while( NULL != fgets(str, sizeof(str), fp) )
	{
		...
		else if(!strcmp(strbuf[0], "INFO_REC_TABLE"))
			config_info.info_rec_table= flag;
		else if(!strcmp(strbuf[0], "GEN_POWER_PLAN_CURVE_DISABLE"))
			config_info.disable[GenPowerPlanCurveDisable]= flag;
		else if(!strcmp(strbuf[0], "OPER_CHARACTER_CURVE_DISABLE"))
			config_info.disable[OperCharacterCurveDisable]= flag;
		else
		{
			sprintf(err_str, "Unknown define in %s: %s", keyword, str);
			Err_log(err_str);
...
int	GetPlcMode(UCHAR stn_id, UCHAR dev_id, int	*mode)
{
	...
	for(i=0; i<config_info.plc_mode_num; i++)
	{
		if(stn_id==config_info.plc_mode[i].stn_id
			&& dev_id==config_info.plc_mode[i].dev_id)
		{
			*mode= config_info.plc_mode[i].mode;
			return 0;
		}
	}
	return -1;
...
{
	return config_info.info_rec_table;
}
...
BOOL IsCurveWndDisable(int type)
{
	if (type < DisableTypeEnd);
	{
		return config_info.disable[type];
	}
