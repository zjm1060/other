...
void	PrintTimeVar( DMS_COMMON *common_aa );
...
/*	if(-1==InitSysLog(H9000_MODULE_DBREAD, "DBREAD"))
	{
		printf("\nFail to InitSysLog!");
		exit(-1);
	}
...
LOOP:
	...
	if(strlen(pt_name)<=1 || strlen(pt_name)>50)
	{
		Sleep(1000);
		goto LOOP;
	}
...
void WriteCharaPoint(POINTER *point)
{
	...
	if(-1==GetCharaPtDataById( point, &chara_data ))
	{
		printf("\nNo this point in database" );
		return;
	}
	...
	printf( DMS_STR_MODY_ITEM_SELECT );
	printf( DMS_STR_MODY_CHARA_MAX_VALUE );
	printf( DMS_STR_MODY_CHARA_MIN_VALUE );
	printf( DMS_STR_MODY_CHARA_AVG_VALUE );
	printf( DMS_STR_MODY_CHARA_STD_VALUE );
	printf( DMS_STR_MODY_CHARA_MAX_RATE );
	printf( DMS_STR_MODY_CHARA_MIN_RATE );
	printf( DMS_STR_NO_SELECT );
	printf( DMS_STR_MODY_CHARA_INPUT );
	...
	{
	case	1:
		chara_data.max_value= fvalue;
		GetSysTime(&time);
		SetCharaPtDataById( point, chara_data, time );
		break;
	case	2:
		chara_data.min_value= fvalue;
		GetSysTime(&time);
		SetCharaPtDataById( point, chara_data, time );
		break;
	case	3:
		chara_data.avg_value= fvalue;
		GetSysTime(&time);
		SetCharaPtDataById( point, chara_data, time );
		break;
	case	4:
		chara_data.std_value= fvalue;
		GetSysTime(&time);
		SetCharaPtDataById( point, chara_data, time );
		break;
	case	5:
		chara_data.max_rate= fvalue;
		GetSysTime(&time);
		SetCharaPtDataById( point, chara_data, time );
		break;
	case	6:
		chara_data.min_rate= fvalue;
		GetSysTime(&time);
		SetCharaPtDataById( point, chara_data, time );
		break;
...
void	PrintCharaVar( DMS_COMMON *common_aa )
{
	...
	/*CHARA_DATA*/
	printf("\nmax_no= %d, min_no= %d, max_rate_no= %d, min_rate_no= %d \
...
void	PrintTimeVar( DMS_COMMON *common_aa )
{
	...
	/*USR_NAME AND HOST_ID*/
	printf("\nusr_name= %s, host_id= %d", 
		common_aa->var.time.usr_name, common_aa->var.time.host_id);
	
	/*UPDATE_TIME*/
	GetTimetoStr(&common_aa->var.time.update_time, str);
	printf("\nupdate_time= %s", str);
	
	/*CMD_STATUS AND SECOND AND MS*/
	printf("\ncmd_opr= %d, cmd_type= %d, cmd_result= %d, \
		\ncmd_src= %d, value_sec= %d, value_ms= %d",
		common_aa->var.time.status.opr, common_aa->var.time.status.state,
		common_aa->var.time.status.result, common_aa->var.time.status.src,
		common_aa->var.time.value_sec, common_aa->var.time.value_ms);
	
	/*COORDS*/
	printf("\nx= %f, y= %f, z= %f",
	   common_aa->var.time.coords.x, common_aa->var.time.coords.y,
	   common_aa->var.time.coords.z);
}
