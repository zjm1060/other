...
int GetStnLongnameById( UCHAR stnId, char *stnLongname )
{
	...
	if((STATION_ENTRY*)-1 == stn_ptr)
		return -1;
			...
			if(GetExLangState())
				strcpy(stnLongname, db_ptr->Station_Union.StationEntry[stnId-1].stnExLongname);
			else
#endif
...
#ifdef EXTEND_LANGUAGE_SYSTEM
			if(GetExLangState())
				strcpy(stnComment, db_ptr->Station_Union.StationEntry[stnId-1].stnExComment);
			else
#endif
...
int GetStnTypeById( UCHAR stnId, UCHAR *stnType )
{
	...
	for(i=0; i<stn_num; i++)
	{
		if(stnId == db_ptr->Station_Union.StationEntry[i].stnId)
		{
			*stnType= db_ptr->Station_Union.StationEntry[i].stnType;
			return 0;
		}
	}
...
int GetGroupNameById( UCHAR stnId, UCHAR groupId, char *groupName )
{
	...
	for(i=0; i<stn_num; i++)
	{
		if(stnId == db_ptr->Station_Union.StationEntry[i].stnId)
		{
			grp_num= db_ptr->Station_Union.StationEntry[i].grp_num;
			grp_offset= db_ptr->Station_Union.StationEntry[i].grp_offset;
