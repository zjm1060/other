...
typedef struct
{
	...
	float	P_Yggd_ChangeValue;   /*投入单机负荷平衡的机组有功给定的变化值*/
...
typedef struct
{
	...
	int		CommandSendSbo;		/*机组曾经下令标志，用于判断机组是否需要重新下令*/
} T_UNIT;     /*机组工况*/
...
char DB_NAME_AGC_JZJX[MAX_UNIT_NUMB+1][NAME_SIZE];
char DB_NAME_AP_RtValue[MAX_UNIT_NUMB+1][NAME_SIZE];
...
char DB_NAME_QCUNRUN_UPLMT[4][NAME_SIZE];    /*全厂有功禁运上下限，省调要求上送3个，added by 冯迅 for 瀑布沟 2010-918*/
char DB_NAME_QCUNRUN_DNLMT[4][NAME_SIZE];	
...
char DB_NAME_AGC_RTFR[FREQUENCY_NUM+1][NAME_SIZE]; /*当前母线频率值*/
char DB_NAME_AGC_FRLOW_ERR[NAME_SIZE];	/*AGC频率低减负荷闭锁*/
char DB_NAME_AGC_FRHIGH_ERR[NAME_SIZE];	/*AGC频率高增负荷闭锁*/
...
char DB_NAME_VIBRATING[MAX_UNIT_NUMB+1][NAME_SIZE]; /*机组跨越振动区状态*/
char DB_NAME_UNIT_OVERTIME[MAX_UNIT_NUMB+1][NAME_SIZE]; /*机组有功调节超时*/
char DB_NAME_UNITP_BALANCE[MAX_UNIT_NUMB+1][NAME_SIZE]; /*机组符合平衡功能*/
char DB_NAME_ADJ_STEP[MAX_UNIT_NUMB+1][NAME_SIZE];  /*机组有功单次分配调节步长*/
